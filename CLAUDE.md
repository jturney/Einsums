...
Einsums is a C++20 tensor algebra library that analyzes each contraction's index pattern to select optimal operations: BLAS calls, a packed-GEMM backend, or generic algorithms.
It has two einsum implementations that make this choice differently.
The templated `TensorAlgebra::einsum` takes compile-time index types and chooses its route at compile time.
The string-spec einsum used by ComputeGraph, `RuntimeTensor`, and Python parses its spec at run time and walks a route cascade on each call, because passes must be able to rewrite a spec that lives on the graph as data.
A few components rely on C++23 features through the CXX23 compatibility module, which supplies C++20 fallbacks.
It uses a modular CMake build system with Python bindings generated by the apiary codegen tool (a conda-forge package; `cmake/Einsums_SetupApiary.cmake` builds it from source if absent) and a deferred-execution ComputeGraph with optimization passes.
...
Indices for the templated `TensorAlgebra::einsum` are compile-time structs created with `MAKE_INDEX(x)`; string specs use the same letters as runtime data.
Either way, the index letters drive contraction analysis.
...
Bindings are generated by the apiary tool from `APIARY_*` annotations in headers (see `APIARY_EXPOSE` / `APIARY_MODULE` / `APIARY_INSTANTIATE_AS` / `APIARY_INSTANTIATE_BOOLS` in `ComputeGraph/Operations.hpp`).
