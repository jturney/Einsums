...
EINSUMS_PYBIND_MODULE("gpu")
EINSUMS_PYBIND_GETTER("available_device_memory")
    // clang-format on
    EINSUMS_EXPORT size_t available_device_memory();

/// Set the mock device memory limit (only effective on mock backend).
/// Has no effect when a real GPU is present.
EINSUMS_EXPORT void set_mock_device_memory_limit(size_t bytes);
...
EINSUMS_PYBIND_EXPOSE
EINSUMS_PYBIND_MODULE("gpu")
EINSUMS_PYBIND_GETTER("device_name")
    // clang-format on
    [[nodiscard]] EINSUMS_EXPORT std::string device_name();
