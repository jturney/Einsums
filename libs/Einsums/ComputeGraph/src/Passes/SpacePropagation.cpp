...
bool SpacePropagation::run(Graph &graph) {
    ...
    // driver calls run() once per subgraph and reset_stats() runs only once per
    // apply, so `_num_inferred > 0` would report this graph as having inferred
    // something whenever ANY earlier subgraph did.
    std::size_t const num_inferred_at_entry = _num_inferred;
