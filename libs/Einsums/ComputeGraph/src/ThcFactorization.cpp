...
ThcFactorization::ThcFactorization(std::string tag, RuntimeTensorView<double> three_index, RuntimeTensorView<double> collocation,
    ...
    : _tag(std::move(tag)), _name(std::move(name)), _three_index(std::move(three_index)), _collocation(std::move(collocation)),
...
expected<FactorizationPlan, std::string> ThcFactorization::propose(Graph const &graph, TensorId tensor) const {
    ...
    std::size_t const basis = _collocation.dim(0);
    std::size_t const grid  = _collocation.dim(1);
    if (_three_index.dim(1) != basis || _three_index.dim(2) != basis) {
        return unexpected(fmt::format("the three-index tensor is [{}, {}, {}] and the collocation matrix has {} basis functions", naux,
                                      _three_index.dim(1), _three_index.dim(2), basis));
    }
    if (grid == 0) {
        return unexpected(std::string{"the collocation matrix has no grid points"});
    }
    for (std::size_t axis = 0; axis < 4; ++axis) {
        if (handle->dims[axis] != basis) {
            return unexpected(fmt::format("the tagged tensor is [{}] and the fit produces a tensor over {} basis functions on every axis",
                                          fmt::join(handle->dims, ", "), basis));
        }
    }
    ...
    // Five factors, two of them under the name of ONE tensor mentioned four times. The pass
    // reads factors under one name as one buffer, which is what keeps a collocation matrix from
    // being stored four times; and the chain is what a plan naming a factor LIST exists for,
    // since there is no way to write it as a split of two.
    std::string const grid_space         = grid_space_name();
    auto const        collocation_factor = [&](std::string basis_letter, std::string grid_letter) {
        return FactorTensor{.name    = "X",
                            .letters = {std::move(basis_letter), std::move(grid_letter)},
                            .dims    = {basis, grid},
                            .spaces  = {std::string{}, grid_space},
                            .dtype   = packed_gemm::ScalarType::Float64};
    };
    plan.factors.push_back(collocation_factor("m", "P"));
    plan.factors.push_back(collocation_factor("n", "P"));
    plan.factors.push_back(FactorTensor{.name    = "Z",
                                        .letters = {"P", "Q"},
                                        .dims    = {grid, grid},
                                        .spaces  = {grid_space, grid_space},
                                        .dtype   = packed_gemm::ScalarType::Float64});
    plan.factors.push_back(collocation_factor("p", "Q"));
    plan.factors.push_back(collocation_factor("q", "Q"));
    ...
    RuntimeTensorView<double> const collocation   = _collocation;
    ...
    plan.emit_setup = [three_index, collocation, naux, basis, grid, threshold, residual_key,
                       reference_key](Graph &parent, Graph &body, std::vector<TensorId> const &factors) {
        auto *X = static_cast<RuntimeTensor<double> *>(parent.tensor(factors[0]).tensor_ptr);
        ...
        auto &gram      = body.declare_runtime_tensor<double>("thc_gram", {grid, grid}, /*intermediate=*/true);
        ...
        auto &partial   = body.declare_runtime_tensor<double>("thc_partial", {naux, basis, grid}, /*intermediate=*/true);
        ...
        auto &rebuilt   = body.declare_runtime_tensor<double>("thc_rebuilt", {naux, basis, grid}, /*intermediate=*/true);
        auto &residual  = body.declare_runtime_tensor<double>("thc_residual", {naux, basis, basis}, /*intermediate=*/true);
...

        {
            ...
            // X is the caller's, copied into the factor the pass declared, which is what makes
            // it an interface tensor of the transformed graph: a bind at a new geometry hands
            // over a new collocation matrix and everything below refits from it.
            permute("m,P <- m,P", 0.0, X, 1.0, collocation);

            // S = (X^T X) elementwise squared. Symmetric and positive semi-definite by the
            // Schur product theorem, which is what lets the guarded inverse below be the same
            // one a metric fit uses.
            einsum("m,P ; m,Q -> P,Q", &gram, *X, *X);
            direct_product(1.0, gram, gram, 0.0, &metric);
            ...
            // B~[A,P] = sum_mn B[A,m,n] X[m,P] X[n,P]. The second contraction carries P through
            // both operands and the output, which is the grid index riding along rather than
            // being summed: that is what makes the fit a fit of one function per grid point.
            einsum("A,m,n ; n,P -> A,m,P", &partial, three_index, *X);
            einsum("A,m,P ; m,P -> A,P", &projected, partial, *X);
            ...
            // C[A,P] X[m,P] X[n,P], and the residual against B is a quantity this library can
            // form where the four-index error is one it cannot.
            einsum("A,R ; R,P -> A,P", &weights, projected, inverse);
            einsum("A,P ; m,P -> A,m,P", &rebuilt, weights, *X);
            permute("A,m,n <- A,m,n", 0.0, &residual, 1.0, three_index);
            einsum("A,m,P ; n,P -> A,m,n", 1.0, &residual, -1.0, rebuilt, *X);
