...
void einsum(EinsumFormatString spec, typename AType::ValueType c_pf, CType *C, typename AType::ValueType ab_pf, AType const &A,
    ...
    auto [b_id, b_slot] = ctx.get_slot(B);
    auto [c_id, c_slot] = ctx.get_slot(*C);
    detail::capture_string_einsum<T>(ctx, parsed, c_pf, ab_pf, conj_a, conj_b, {a_id, a_slot, &A.impl()}, {b_id, b_slot, &B.impl()},
                                     {c_id, c_slot, &C->impl()});
}
...
void einsum(EinsumFormatString spec, typename CType::ValueType c_pf, CType *C,
    ...
    auto [b_id, b_slot] = ctx.get_slot(B);
    auto [c_id, c_slot] = ctx.get_slot(*C);
    detail::capture_mixed_string_einsum<typename CType::ValueType, typename AType::ValueType, typename BType::ValueType>(
        ctx, parsed, c_pf, ab_pf, conj_a, conj_b, {a_id, a_slot, &A.impl()}, {b_id, b_slot, &B.impl()}, {c_id, c_slot, &C->impl()});
}
