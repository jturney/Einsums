...
class APIARY_EXPOSE APIARY_MODULE("graph") APIARY_HOLDER(std::shared_ptr) EINSUMS_EXPORT ScaleAbsorption : public OptimizerPass {
  public:
    ...
    /// Every feature today: only a real live factor folds, and readers and writers are resolved by buffer.
    ...
    /// The absorbed scale becomes the consumer's prefactor, which the consumer was already applying.
