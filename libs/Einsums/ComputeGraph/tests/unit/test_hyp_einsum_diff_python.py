...
def _draw_permutation_operator(draw, c_idx, extent):
    ...
    # that already happened to share one found an operator on a tenth of draws
    # and never once reached the four-letter shapes, because C is short and the
    # extents are drawn independently. Zero stays drawable: an operator over a
    # zero-extent axis still has to apply the destination prefactor once.
    common = draw(st.sampled_from([0, 1, 2, 2, 3]))
...
def _einsum_problem(draw):
    ...
    return (a_idx, b_idx, c_idx, extent, alias_ab,
            ...
            draw(st.booleans()), draw(st.booleans()), draw(st.booleans()), draw(st.booleans()),
            draw(st.booleans()),
            # operator: appended LAST so the pinned @example tuples, which
            # predate it, keep working through the *rest unpacking below.
            _draw_permutation_operator(draw, c_idx, extent))
...
def _run_einsum_diff(prob, exact):
    ...
    operator = rest[1] if len(rest) > 1 else None
