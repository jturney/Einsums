...
The ComputeGraph provides 16+ built-in optimization passes. The default pipeline
(``create_default()``) includes all safe passes in the recommended order. Some
modify the graph (graph-transforming), others only analyze and report
(analysis-only).
...
.. code-block:: cpp
   ...
   // Default pipeline (all 11 built-in passes in recommended order):
...
- **Einsum**: ``c_prefactor=0`` → ``c_prefactor=α``
- **Gemm**:  ``beta=0`` → ``beta=α``
- **Permute**: ``beta=0`` → ``beta=α``
...
Propagates: if node A is folded, nodes depending only on A's outputs are also
foldable.

.. note::

   This pass has side effects — it executes folded nodes during the pass itself.
   Included in ``create_default()`` and safe for Pipeline loop bodies.
...
with no consumers. Useful after CSE or other passes eliminate consumers — the
original producer may become dead.
...
``graph.execute()``. Current rules: scale preserves, axpy/axpby with
same-descriptor operands preserves, rank-2 self-contraction (``AᵀA``/
``AAᵀ``) produces symmetric, permute of symmetric stays symmetric.
...
candidates for in-place operation where the consumer overwrites the
intermediate's buffer instead of allocating a separate output.
...
instead of N, which is a substantial win for workloads that issue many
small contractions (stacked attention heads, per-sample transforms,
Kronecker-factored updates, batched chemistry kernels). See
:doc:`gemm_batching` for a full walk-through and timing numbers.

**Candidate:** 2D×2D→2D einsum with exactly one link index. Capture
...
Everything in the key must match exactly — alpha/beta compared
...
(complex alpha/beta assumed real — matches the capture path).
...
- The permute is a pure axis reorder: ``alpha == 1``, ``beta == 0``,
  ``c_indices`` is a duplicate-free permutation of ``a_indices``.
- The consumer einsum has a populated ``EinsumDescriptor::indices``
  shared state (string-captured einsums — the only form after the
  tuple-overload removal).

Reports ``num_candidates()`` (detected pairs) and ``num_rewrites()``
(pairs that passed the safety filter and were actually fused).
...
Most ``DiskRead`` nodes have no predecessors — they load data from files that
exist independently of the graph. By moving them to the beginning, the pass
...
**Result**: ``T = B1 + B2; R += A*T`` — saves one matrix multiply per additional term.
...
allocated during ``execute()``, not during the pass itself — enabling
...
- **Only frees intermediates** — user-provided tensors (inputs/outputs) are never freed
- **Size threshold** — only frees tensors above 1 MB by default (configurable via constructor). Small tensors are kept alive to avoid alloc/free overhead in loops.
...
- **Loop overhead** — for re-executed graphs, large intermediates are freed and re-allocated each iteration. The overhead (~1ms for 100MB) is negligible vs compute time, and the memory savings (not keeping dead intermediates alive) are significant.
...
Removes redundant communication — e.g., back-to-back allreduce of the same
tensor without intervening modification.
...
.. code-block:: text
   ...
   13. GPUPlacement             — decide CPU vs GPU per node (if GPU)
   14. TransferInsertion        — insert H2D/D2H transfer nodes (if GPU)
   15. TransferElimination      — remove redundant transfers (if GPU)
   16. GPUDiagnostics           — report GPU placement statistics (if GPU)
   17. StreamAssignment         — assign GPU streams for async (if GPU)
   18. InputSlicing             — carve distributed tensor inputs (if MPI)
   19. SUMMAExpansion           — expand distributed GEMMs (if MPI)
   20. CommunicationInsertion   — insert allreduce/broadcast (if MPI)
   21. CommunicationElimination — remove redundant communication (if MPI)
   22. CommunicationScheduling  — overlap communication with compute (if MPI)
   23. FreeInsertion            — insert Free nodes at last-consumer
   24. MemoryPlanning           — analyze tensor liveness
   25. ContractionPlanning      — multi-objective contraction ordering
   26. InplaceOptimization      — detect in-place candidates
