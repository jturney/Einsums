...
//
// This file is *not* generated. It owns the runtime startup/shutdown
...
// set, and that's pulled in from a tiny autogenerated header
...
// file linted, formatted, and IDE-indexed like ordinary C++ source.
...
// Autogenerated header: declares ``einsums_pybind_register_<Module>(py::module_ &)``
// for every enabled module and defines the inline aggregator
// ``einsums_pybind_register_all(py::module_ &)`` that calls them in turn.
...
// spellings are runtime-internal — keep this mapping in lockstep with
...
PYBIND11_MODULE(_core, m) {
    ...
    // Initialize the einsums runtime once, using the user's einsums.rc
    // settings (or defaults if rc was never touched). einsums.rc is a
    // pure-Python sibling and importing it does not recurse back into
    // _core.
    if (!einsums::is_running()) {
