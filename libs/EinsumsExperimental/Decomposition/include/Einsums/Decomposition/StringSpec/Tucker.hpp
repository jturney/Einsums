    ...
    for_sequence<TRank>([&](auto i) {
        ...
        for (size_t element = 0; element < elements; element++) {
            ...
            for (size_t n = 0; n < full_idx; n++) {
                ...
                TType &source = subscript_tensor(*old_tensor_buffer, source_combination);
                TType &target = subscript_tensor(*new_tensor_buffer, target_combination);
...
auto initialize_tucker(std::vector<TTensor, Alloc> &folds, Ranks &ranks) -> BufferVector<TTensor> {
    ...
    for_sequence<Rank>([&](auto i) {
        ...
        factors.emplace_back(Tensor{U.value()(All, Range{0, rank})});
    });
...
auto tucker_ho_svd(Tensor<TType, TRank> const &tensor, Ranks &ranks) -> std::tuple<Tensor<TType, TRank>, BufferVector<Tensor<TType, 2>>> {
    return tucker_ho_svd(tensor, ranks, BufferVector<Tensor<TType, 2>>());
}
...
auto tucker_ho_oi(Tensor<TType, TRank> const &tensor, Ranks &ranks, int n_iter_max = 100, double tolerance = 1.e-8)
    ...
    while (iter < n_iter_max) {
        ...
        for_sequence<TRank>([&](auto i) {
            ...
            BufferAllocator<Tensor<TType, TRank>> allocator;
