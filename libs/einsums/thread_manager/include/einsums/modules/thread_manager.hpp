...
class thread_manager {
  ...
  public:
    ...
    using pool_vector              = std::vector<pool_type>;
     ...
     thread_manager(einsums::util::runtime_configuration &rtcfg_, notification_policy_type &notifier);
    ~thread_manager();

    void init();
    void create_pools();

    //! FIXME move to private and add --einsums:printpools cmd line option
    ...
    runtime_state status() const {
        einsums::runtime_state result(runtime_state::last_valid_runtime);

        for (auto &pool_iter : pools_) {
            einsums::runtime_state s = pool_iter->get_state();
    ...
    /// \brief return the number of einsums-threads with the given state
    ///
  ...
  public:
    ...
    hwloc_bitmap_ptr get_pool_numa_bitmap(const std::string &pool_name) const { return get_pool(pool_name).get_numa_domain_bitmap(); }
    ...
    void init_tss(std::size_t global_thread_num) { set_global_thread_num_tss(global_thread_num); }
  ...
  public:
    ...
    std::int64_t get_thread_count_unknown(bool reset) {
        return get_thread_count(thread_schedule_state::unknown, execution::thread_priority::default_, std::size_t(-1), reset);
    }
    std::int64_t get_thread_count_active(bool reset) {
        return get_thread_count(thread_schedule_state::active, execution::thread_priority::default_, std::size_t(-1), reset);
    }
    std::int64_t get_thread_count_pending(bool reset) {
        return get_thread_count(thread_schedule_state::pending, execution::thread_priority::default_, std::size_t(-1), reset);
    }
    std::int64_t get_thread_count_suspended(bool reset) {
        return get_thread_count(thread_schedule_state::suspended, execution::thread_priority::default_, std::size_t(-1), reset);
    }
    std::int64_t get_thread_count_terminated(bool reset) {
        return get_thread_count(thread_schedule_state::terminated, execution::thread_priority::default_, std::size_t(-1), reset);
    }
    std::int64_t get_thread_count_staged(bool reset) {
        return get_thread_count(thread_schedule_state::staged, execution::thread_priority::default_, std::size_t(-1), reset);
    }
