...
#include <einsums/timing/detail/timestamp.hpp>
...
void check_num_high_priority_queues(std::size_t num_threads, std::size_t num_high_priority_queues) {
...
///////////////////////////////////////////////////////////////////////////
thread_manager::thread_manager(einsums::util::runtime_configuration &rtcfg, notification_policy_type &notifier)
    : rtcfg_(rtcfg), notifier_(notifier) {
    // Add callbacks local to thread_manager.
    notifier.add_on_start_thread_callback([this]([[maybe_unused]] std::size_t local_thread_num, std::size_t global_thread_num,
                                                 [[maybe_unused]] char const *pool_name,
                                                 [[maybe_unused]] char const *postfix) { this->init_tss(global_thread_num); });
    notifier.add_on_stop_thread_callback([this]([[maybe_unused]] std::size_t local_thread_num,
                                                [[maybe_unused]] std::size_t global_thread_num, [[maybe_unused]] char const *pool_name,
                                                [[maybe_unused]] char const *postfix) { this->deinit_tss(); });
    notifier.add_on_start_thread_callback(
        [](std::size_t local_thread_num, [[maybe_unused]] std::size_t global_thread_num, char const *pool_name,
           [[maybe_unused]] char const *postfix) { einsums::resource::get_partitioner().assign_pu(pool_name, local_thread_num); });
    notifier.add_on_stop_thread_callback(
        [](std::size_t local_thread_num, [[maybe_unused]] std::size_t global_thread_num, char const *pool_name,
           [[maybe_unused]] char const *postfix) { einsums::resource::get_partitioner().unassign_pu(pool_name, local_thread_num); });
}
...
void thread_manager::create_pools() {
    ...
    std::size_t const max_idle_loop_count =
        einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.max_idle_loop_count", EINSUMS_IDLE_LOOP_COUNT_MAX);
    std::size_t const max_busy_loop_count =
        einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.max_busy_loop_count", EINSUMS_BUSY_LOOP_COUNT_MAX);

    std::int64_t const max_thread_count =
        einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.max_thread_count", EINSUMS_THREAD_QUEUE_MAX_THREAD_COUNT);
    std::int64_t const min_tasks_to_steal_pending = einsums::detail::get_entry_as<std::int64_t>(
        rtcfg_, "einsums.thread_queue.min_tasks_to_steal_pending", EINSUMS_THREAD_QUEUE_MIN_TASKS_TO_STEAL_PENDING);
    std::int64_t const min_tasks_to_steal_staged = einsums::detail::get_entry_as<std::int64_t>(
        rtcfg_, "einsums.thread_queue.min_tasks_to_steal_staged", EINSUMS_THREAD_QUEUE_MIN_TASKS_TO_STEAL_STAGED);
    std::int64_t const min_add_new_count = einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.min_add_new_count",
                                                                                       EINSUMS_THREAD_QUEUE_MIN_ADD_NEW_COUNT);
    std::int64_t const max_add_new_count = einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.max_add_new_count",
                                                                                       EINSUMS_THREAD_QUEUE_MAX_ADD_NEW_COUNT);
    std::int64_t const min_delete_count =
        einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.min_delete_count", EINSUMS_THREAD_QUEUE_MIN_DELETE_COUNT);
    std::int64_t const max_delete_count =
        einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.max_delete_count", EINSUMS_THREAD_QUEUE_MAX_DELETE_COUNT);
    std::int64_t const max_terminated_threads = einsums::detail::get_entry_as<std::int64_t>(
        rtcfg_, "einsums.thread_queue.max_terminated_threads", EINSUMS_THREAD_QUEUE_MAX_TERMINATED_THREADS);
    std::int64_t const init_threads_count = einsums::detail::get_entry_as<std::int64_t>(rtcfg_, "einsums.thread_queue.init_threads_count",
                                                                                        EINSUMS_THREAD_QUEUE_INIT_THREADS_COUNT);
    double const       max_idle_backoff_time =
        einsums::detail::get_entry_as<double>(rtcfg_, "einsums.max_idle_backoff_time", EINSUMS_IDLE_BACKOFF_TIME_MAX);

    std::ptrdiff_t small_stacksize  = rtcfg_.get_stack_size(execution::thread_stacksize::small_);
    std::ptrdiff_t medium_stacksize = rtcfg_.get_stack_size(execution::thread_stacksize::medium);
    ...
    thread_queue_init_parameters thread_queue_init(max_thread_count, min_tasks_to_steal_pending, min_tasks_to_steal_staged,
                                                   min_add_new_count, max_add_new_count, min_delete_count, max_delete_count,
                                                   max_terminated_threads, init_threads_count, max_idle_backoff_time, small_stacksize,
    ...
    for (size_t i = 0; i != num_pools; i++) {
        ...
        if (i == 0) {
            if (name != rp.get_default_pool_name()) {
                throw std::invalid_argument("Trying to instantiate pool " + name +
                                            " as first thread pool, but first thread pool must be named " + rp.get_default_pool_name());
            }
        }

        thread_pool_init_parameters thread_pool_init(name, i, scheduler_mode, num_threads_in_pool, thread_offset, notifier_,
                                                     rp.get_affinity_data(), max_idle_loop_count, max_busy_loop_count);

        std::size_t numa_sensitive = einsums::detail::get_entry_as<std::size_t>(rtcfg_, "einsums.numa_sensitive", 0);

        switch (sched_type) {
        case resource::user_defined: {
            auto                              pool_func = rp.get_pool_creator(i);
            std::unique_ptr<thread_pool_base> pool(pool_func(thread_pool_init, thread_queue_init));
            pools_.push_back(std::move(pool));
            break;
        }
        case resource::unspecified: {
            throw std::invalid_argument("cannot instantiate a thread-manager if the thread-pool" + name +
                                        " has an unspecified scheduler type");
        }
        case resource::local: {
            // instantiate the scheduler
            using local_sched_type = einsums::threads::detail::local_queue_scheduler<>;

            local_sched_type::init_parameter_type init(thread_pool_init.num_threads_, thread_pool_init.affinity_data_, thread_queue_init,
            ...
            std::unique_ptr<thread_pool_base> pool(
                new einsums::threads::detail::scheduled_thread_pool<local_sched_type>(std::move(sched), thread_pool_init));
            pools_.push_back(std::move(pool));
            break;
        }
        ...
        case resource::local_priority_fifo: {
            ...
            std::size_t num_high_priority_queues = einsums::detail::get_entry_as<std::size_t>(
                rtcfg_, "einsums.thread_queue.high_priority_queues", thread_pool_init.num_threads_);
            check_num_high_priority_queues(thread_pool_init.num_threads_, num_high_priority_queues);
            ...
            local_sched_type::init_parameter_type init(thread_pool_init.num_threads_, thread_pool_init.affinity_data_,
                                                       num_high_priority_queues, thread_queue_init, "core-local_priority_queue_scheduler");
        ...
        case resource::local_priority_lifo: {
            ...
            pools_.push_back(std::move(pool));
#else
            throw einsums::detail::command_line_error("Command line option --einsums:queuing=local-priority-lifo is not configured in "
        ...
        case resource::static_: {
            // instantiate the scheduler
            using local_sched_type = einsums::threads::detail::static_queue_scheduler<>;
        ...
        case resource::static_priority: {
            ...
            // instantiate the scheduler
            using local_sched_type = einsums::threads::detail::static_priority_queue_scheduler<>;

            local_sched_type::init_parameter_type init(thread_pool_init.num_threads_, thread_pool_init.affinity_data_,
                                                       num_high_priority_queues, thread_queue_init, "core-static_priority_queue_scheduler");
        ...
        case resource::abp_priority_fifo: {
            ...
            pools_.push_back(std::move(pool));
#else
            throw einsums::detail::command_line_error("Command line option --einsums:queuing=abp-priority-fifo is not configured in "
                                                      "this build. Please make sure 128bit atomics are available.");
#endif
            break;
        }
        ...
        case resource::abp_priority_lifo: {
            ...
            pools_.push_back(std::move(pool));
#else
            throw einsums::detail::command_line_error("Command line option --einsums:queuing=abp-priority-lifo is not configured in "
                                                      "this build. Please make sure 128bit atomics are available.");
#endif
            break;
        }
        ...
        case resource::shared_priority: {
            // instantiate the scheduler
            using local_sched_type = einsums::threads::detail::shared_priority_queue_scheduler<>;
            local_sched_type::init_parameter_type init(thread_pool_init.num_threads_, {1, 1, 1}, thread_pool_init.affinity_data_,
                                                       thread_queue_init, "core-shared_priority_queue_scheduler");
...
thread_pool_base &thread_manager::get_pool(std::string const &pool_name) const {
    ...
    if (pool_name == "default" || pool_name == resource::get_partitioner().get_default_pool_name()) {
        return default_pool();
    }

    // now check the other pools - no need to check pool 0 again, so ++begin
    auto pool = std::find_if(++pools_.begin(), pools_.end(),
                             [&pool_name](pool_type const &itp) -> bool { return (itp->get_pool_name() == pool_name); });
    ...
    EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "thread_manager::get_pool",
...
///////////////////////////////////////////////////////////////////////////
std::int64_t thread_manager::get_thread_count(thread_schedule_state state, execution::thread_priority priority, std::size_t num_thread,
                                              bool reset) {
...
bool thread_manager::enumerate_threads(util::detail::function<bool(thread_id_type)> const &f, thread_schedule_state state) const {
    std::lock_guard<mutex_type> lk(mtx_);
    bool                        result = true;
...
#ifdef EINSUMS_HAVE_THREAD_IDLE_RATES
...
#ifdef EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS
...
bool thread_manager::run() {
    ...
    for (auto &pool_iter : pools_) {
        ...
        if (pool_iter->get_os_thread_count() != 0 || pool_iter->has_reached_state(runtime_state::running)) {
    ...
    EINSUMS_LOG(info, "run: running");
    return true;
}

void thread_manager::stop(bool blocking) {
    EINSUMS_LOG(info, "stop: blocking({})", blocking ? "true" : "false");
...
void thread_manager::wait() {
    einsums::util::yield_while(
        []() { return einsums::threads::detail::get_global_activity_count() > (threads::detail::get_self_ptr() != nullptr ? 1 : 0); });
}

void thread_manager::suspend() {
    EINSUMS_ASSERT(!threads::detail::get_self_ptr());
