...
//   stability
// - einsums thread ids are used before std::thread ids are used for the tree index
// - Waiting is done with einsums's yield_while, spinning until the expected result (yielding done
//   after some time)
//
...
namespace einsums {
    ...
    namespace detail {
        ...
        class EINSUMS_EXPORT barrier_algorithm_base
        {
        public:
    ...
    public:
        ...
        //                 EINSUMS_MOVE(f). Starts the first phase. [Note: If
        ...
        explicit barrier(std::ptrdiff_t expected, Completion completion = Completion())
          ...
          , base(this->expected)
        {
            EINSUMS_ASSERT(expected >= 0);
            EINSUMS_ASSERT(expected <= (max) ());
        }

        EINSUMS_NON_COPYABLE(barrier);
        ...
        {
            EINSUMS_ASSERT(update <= expected);
        ...
        //                 EINSUMS_MOVE(arrival) until the phase completion step
                ...
                0.0)) const
        {
            ...
            if (do_busy_wait &&
                einsums::util::detail::yield_while_timeout(
                    poll, busy_wait_timeout, "barrier::wait", false))
            {
                return;
            }

            einsums::util::yield_while(poll, "barrier::wait", true);
        }
