...
enum class async_rw_mutex_access_type { read, readwrite };
...
struct async_rw_mutex_shared_state {
    ...
    mutex_type                                                                                            mtx{};
    einsums::detail::small_vector<einsums::util::detail::unique_function<void(shared_state_ptr_type)>, 1> continuations{};
    ...
    T &get_value() {
        einsums::util::yield_while([this]() { return !value_set.load(std::memory_order_acquire); });
        EINSUMS_ASSERT(value);
        // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
        return *value;
    }
    ...
    void set_next_state(std::shared_ptr<async_rw_mutex_shared_state> state) {
        ...
        EINSUMS_ASSERT(!next_state);
        EINSUMS_ASSERT(state);
...
struct async_rw_mutex_shared_state<void> {
    ...
    einsums::detail::small_vector<einsums::util::detail::unique_function<void(shared_state_ptr_type)>, 1> continuations{};
         ...
         async_rw_mutex_shared_state()                                                   = default;
         async_rw_mutex_shared_state(async_rw_mutex_shared_state &&)                     = delete;
    auto operator=(async_rw_mutex_shared_state &&) -> async_rw_mutex_shared_state      & = delete;
         async_rw_mutex_shared_state(async_rw_mutex_shared_state const &)                = delete;
    auto operator=(async_rw_mutex_shared_state const &) -> async_rw_mutex_shared_state & = delete;
...
struct async_rw_mutex_access_wrapper<ReadWriteT, ReadT, async_rw_mutex_access_type::read> {
  ...
  public:
         ...
         async_rw_mutex_access_wrapper(shared_state_type state) : state(EINSUMS_MOVE(state)) {}
         async_rw_mutex_access_wrapper(async_rw_mutex_access_wrapper &&)                     = default;
    auto operator=(async_rw_mutex_access_wrapper &&) -> async_rw_mutex_access_wrapper      & = default;
         async_rw_mutex_access_wrapper(async_rw_mutex_access_wrapper const &)                = default;
    auto operator=(async_rw_mutex_access_wrapper const &) -> async_rw_mutex_access_wrapper & = default;

    auto get() const -> ReadT & {
        EINSUMS_ASSERT(state);
...
struct async_rw_mutex_access_wrapper<ReadWriteT, ReadT, async_rw_mutex_access_type::readwrite> {
  private:
    static_assert(!std::is_void_v<ReadWriteT>, "Cannot mix void and non-void type in async_rw_mutex_access_wrapper wrapper "
                                               "(ReadWriteT is void, ReadT is non-void)");
    static_assert(!std::is_void_v<ReadT>, "Cannot mix void and non-void type in async_rw_mutex_access_wrapper wrapper (ReadT "
  ...
  public:
    ...
    auto operator=(async_rw_mutex_access_wrapper const &) -> async_rw_mutex_access_wrapper & = delete;
...
struct async_rw_mutex_access_wrapper<void, void, async_rw_mutex_access_type::read> {
  ...
  public:
                                   async_rw_mutex_access_wrapper() = delete;
    explicit                       async_rw_mutex_access_wrapper(shared_state_type state) : state(std::move(state)) {}
                                   async_rw_mutex_access_wrapper(async_rw_mutex_access_wrapper &&)                     = default;
...
template <typename ReadWriteT = void, typename ReadT = ReadWriteT, typename Allocator = einsums::detail::internal_allocator<>>
...
class async_rw_mutex<void, void, Allocator> {
...
#if defined(EINSUMS_NVHPC_VERSION)
  public:
#endif
  ...
  public:
    ...
    explicit        async_rw_mutex(allocator_type const &alloc = {}) : alloc(alloc) {}
    ...
    sender<async_rw_mutex_access_type::read> read() {
        if (prev_access == async_rw_mutex_access_type::readwrite) {
            ...
            // state.
            if (EINSUMS_LIKELY(shared_prev_state)) {
  ...
  private:
    ...
    struct sender {
        ...
        using completion_signatures =
            einsums::execution::experimental::completion_signatures<einsums::execution::experimental::set_value_t(access_type),
        ...
        struct operation_state {
            ...
            operation_state(R_ &&r, shared_state_weak_ptr_type prev_state, shared_state_ptr_type state)
                : r(std::forward<R_>(r)), prev_state(std::move(prev_state)), state(std::move(state)) {}
            ...
            friend void tag_invoke(einsums::execution::experimental::start_t, operation_state &os) noexcept {
                EINSUMS_ASSERT_MSG(os.state, "async_rw_lock::sender::operation_state state is empty, was the sender "
                                             "already started?");

                auto continuation = [&os](shared_state_ptr_type state) mutable {
                    try {
                        einsums::execution::experimental::set_value(std::move(os.r), access_type{std::move(state)});
                    } catch (...) {
                        einsums::execution::experimental::set_error(std::move(os.r), std::current_exception());
                    }
                };
        ...
        template <typename R>
        friend auto tag_invoke(einsums::execution::experimental::connect_t, sender &&s, R &&r) {
            return operation_state<R>{std::forward<R>(r), std::move(s.prev_state), std::move(s.state)};
        }
    };
...
class async_rw_mutex {
  private:
    static_assert(!std::is_void_v<ReadWriteT>, "Cannot mix void and non-void type in async_rw_mutex (ReadWriteT is void, ReadT is "
                                               "non-void)");
    static_assert(!std::is_void_v<ReadT>, "Cannot mix void and non-void type in async_rw_mutex (ReadT is void, ReadWriteT is "
                                          "non-void)");
  ...
  public:
    ...
    explicit async_rw_mutex(U &&u, allocator_type const &alloc = {}) : value(std::forward<U>(u)), alloc(alloc) {}
                    async_rw_mutex(async_rw_mutex &&)      = default;
    async_rw_mutex &operator=(async_rw_mutex &&)           = default;
                    async_rw_mutex(async_rw_mutex const &) = delete;
    async_rw_mutex &operator=(async_rw_mutex const &)      = delete;
    ...
    sender<async_rw_mutex_access_type::read> read() {
        if (prev_access == async_rw_mutex_access_type::readwrite) {
            ...
            if (EINSUMS_LIKELY(shared_prev_state)) {
                ...
                state->set_value(EINSUMS_MOVE(value));
            }
        }
    ...
    sender<async_rw_mutex_access_type::readwrite> readwrite() {
        auto shared_prev_state = EINSUMS_MOVE(state);
        state                  = std::allocate_shared<shared_state_type, allocator_type>(alloc);
        prev_access            = async_rw_mutex_access_type::readwrite;
  ...
  private:
    ...
    struct sender {
        ...
        template <typename R>
        friend auto tag_invoke(einsums::execution::experimental::connect_t, sender const &s, R &&r) {
            if constexpr (AccessType == async_rw_mutex_access_type::readwrite) {
                static_assert(sizeof(R) == 0, "senders returned from async_rw_mutex::readwrite are not l-lvalue "
                                              "connectable");
            }
    ...
    EINSUMS_NO_UNIQUE_ADDRESS value_type     value;
    EINSUMS_NO_UNIQUE_ADDRESS allocator_type alloc;
