...
#include <einsums/modules/execution_base.hpp>
#include <einsums/modules/memory.hpp>
#include <einsums/modules/thread_support.hpp>
...
struct stop_state;
EINSUMS_EXPORT void intrusive_ptr_add_ref(stop_state *p);
EINSUMS_EXPORT void intrusive_ptr_release(stop_state *p);
...
struct stop_state {
  ...
  public:
    stop_state() : state_(token_ref_increment) {}

    bool stop_requested() const noexcept { return stop_requested(state_.load(std::memory_order_acquire)); }

    // Returns: false if a stop request was not made and there are no
    //      associated stop_source objects; otherwise, true.
    bool stop_possible() const noexcept { return stop_possible(state_.load(std::memory_order_acquire)); }

    EINSUMS_EXPORT bool request_stop() noexcept;

    void add_source_count() { state_.fetch_add(stop_state::source_ref_increment, std::memory_order_relaxed); }

    void remove_source_count() { state_.fetch_sub(stop_state::source_ref_increment, std::memory_order_acq_rel); }

    EINSUMS_EXPORT bool add_callback(stop_callback_base *cb) noexcept;
    EINSUMS_EXPORT void remove_callback(stop_callback_base *cb) noexcept;

  private:
    static bool is_locked(std::uint64_t state) noexcept { return (state & stop_state::locked_flag) != 0; }

    static bool stop_requested(std::uint64_t state) noexcept { return (state & stop_state::stop_requested_flag) != 0; }
    ...
    EINSUMS_EXPORT bool lock_and_request_stop() noexcept;
    ...
    EINSUMS_EXPORT bool lock_if_not_stopped(stop_callback_base *cb) noexcept;

  public:
    // Effect: locks the state
    EINSUMS_EXPORT void lock() noexcept;
  ...
  private:
    ...
    friend EINSUMS_EXPORT void intrusive_ptr_add_ref(stop_state *p);
    friend EINSUMS_EXPORT void intrusive_ptr_release(stop_state *p);

    std::atomic<std::uint64_t>               state_;
    stop_callback_base                      *callbacks_ = nullptr;
    einsums::threads::detail::thread_id_type signalling_thread_;
};
...
class stop_token {
  public:
    ...
    stop_token(stop_token const &rhs) noexcept : state_(rhs.state_) {}
    ...
    [[nodiscard]] bool stop_possible() const noexcept { return !!state_ && state_->stop_possible(); }
  ...
  private:
    ...
    stop_token(einsums::memory::intrusive_ptr<detail::stop_state> const &state) : state_(state) {}

  private:
    einsums::memory::intrusive_ptr<detail::stop_state> state_;
};
...
class stop_source {
  public:
    ...
    //      state.
    stop_source() : state_(new detail::stop_state, false) { state_->add_source_count(); }

    // Postconditions: stop_possible() is false and stop_requested() is
    //      false. [Note: No resources are allocated for the state.]
    explicit stop_source(nostopstate_t) noexcept {}

    stop_source(stop_source const &rhs) noexcept : state_(rhs.state_) {
        if (state_)
...
class [[nodiscard]] stop_callback : private detail::stop_callback_base {
  public:
    ...
    explicit stop_callback(stop_token const &st, CB &&cb) noexcept(std::is_nothrow_constructible<Callback, CB>::value)
        : callback_(std::forward<CB>(cb)), state_(st.state_) {
        if (state_)
            state_->add_callback(this);
    }

    template <typename CB, typename Enable = typename std::enable_if<std::is_constructible<Callback, CB>::value>::type>
    explicit stop_callback(stop_token &&st, CB &&cb) noexcept(std::is_nothrow_constructible<Callback, CB>::value)
        : callback_(std::forward<CB>(cb)), state_(std::move(st.state_)) {
        if (state_)
            state_->add_callback(this);
    }
