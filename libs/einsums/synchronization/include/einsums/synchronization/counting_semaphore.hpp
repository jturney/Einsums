...
// this semaphore.
template <std::ptrdiff_t LeastMaxValue = PTRDIFF_MAX, typename Mutex = einsums::concurrency::detail::spinlock>
class counting_semaphore {
  public:
    EINSUMS_NON_COPYABLE(counting_semaphore);
  ...
  public:
    ...
    //  Throws          Nothing.
    explicit counting_semaphore(std::ptrdiff_t value) : sem_(value) {}
    ...
    bool try_acquire_until(einsums::chrono::steady_time_point const &abs_time) {
        std::unique_lock<mutex_type> l(mtx_);
        return sem_.wait_until(l, abs_time, 1);
    }

    bool try_acquire_for(einsums::chrono::steady_duration const &rel_time) { return try_acquire_until(rel_time.from_now()); }
...
class binary_semaphore : public counting_semaphore<1, Mutex> {
  public:
    EINSUMS_NON_COPYABLE(binary_semaphore);

  public:
    binary_semaphore(std::ptrdiff_t value = 1) : counting_semaphore<1, Mutex>(value) {}
