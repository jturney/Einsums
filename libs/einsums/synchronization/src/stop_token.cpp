...
namespace einsums::detail {
    ...
    void stop_state::lock() noexcept
    {
            ...
            std::memory_order_acquire, std::memory_order_relaxed))
        {
            ...
            {
                einsums::execution::this_thread::detail::yield_k(k, "stop_state::lock");
    ...
    bool stop_state::lock_and_request_stop() noexcept
    {
            ...
            std::memory_order_acquire, std::memory_order_relaxed))
        {
            ...
            for (std::size_t k = 0; is_locked(old_state); ++k)
            {
                einsums::execution::this_thread::detail::yield_k(
    ...
    bool stop_state::lock_if_not_stopped(stop_callback_base* cb) noexcept
    {
            ...
            std::memory_order_acquire, std::memory_order_relaxed))
        {
            ...
            for (std::size_t k = 0; is_locked(old_state); ++k)
            {
                einsums::execution::this_thread::detail::yield_k(k, "stop_state::add_callback");
                old_state = state_.load(std::memory_order_acquire);
    ...
    void stop_state::remove_callback(stop_callback_base* cb) noexcept
    {
        ...
        // on another thread.
        if (signalling_thread_ == einsums::threads::detail::get_self_id())
        {
    ...
    bool stop_state::request_stop() noexcept
    {
        ...
        EINSUMS_ASSERT(stop_requested(state_.load(std::memory_order_acquire)));

        signalling_thread_ = einsums::threads::detail::get_self_id();
