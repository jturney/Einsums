...
#ifdef _MSC_VER
#    if defined(_M_IX86)
#        define EINSUMS_LOCKFREE_DCAS_ALIGNMENT
#    elif defined(_M_X64) || defined(_M_IA64)
#        define EINSUMS_LOCKFREE_DCAS_ALIGNMENT __declspec(align(16))
#    endif
...
#        define EINSUMS_LOCKFREE_DCAS_ALIGNMENT __attribute__((aligned(16)))
...
#if !defined(EINSUMS_LOCKFREE_DCAS_ALIGNMENT)
#    define EINSUMS_LOCKFREE_DCAS_ALIGNMENT
#endif

namespace einsums::concurrency::detail {
struct EINSUMS_LOCKFREE_DCAS_ALIGNMENT uint128_type {
    std::uint64_t left;
    std::uint64_t right;

    bool operator==(volatile uint128_type const &rhs) const { return (left == rhs.left) && (right == rhs.right); }

    bool operator!=(volatile uint128_type const &rhs) const { return !(*this == rhs); }
};

template <class Left, class Right>
struct EINSUMS_LOCKFREE_DCAS_ALIGNMENT tagged_ptr_pair {
    using compressed_ptr_pair_t = uint128_type;
    using compressed_ptr_t      = std::uint64_t;
    using tag_t                 = std::uint16_t;

    union EINSUMS_LOCKFREE_DCAS_ALIGNMENT cast_unit {
    ...
    static Left *extract_left_ptr(volatile compressed_ptr_pair_t const &i) { return reinterpret_cast<Left *>(i.left & ptr_mask); }

    static Right *extract_right_ptr(volatile compressed_ptr_pair_t const &i) { return reinterpret_cast<Right *>(i.right & ptr_mask); }
    ...
    static void pack_ptr_pair(compressed_ptr_pair_t &pair, Left *lptr, Right *rptr, IntegralL ltag, IntegralR rtag) {
        cast_unit ret;
