...
class contiguous_index_queue {
    static_assert(sizeof(T) <= 4, "contiguous_index_queue assumes at most 32 bit indices to fit two indices in an at "
                                  "most 64 bit struct");
    static_assert(std::is_integral_v<T>, "contiguous_index_queue only works with integral indices");
    ...
    struct range {
        ...
        range(T first, T last) noexcept : first(first), last(last) {}
  ...
  public:
    ...
    constexpr void reset(T first, T last) noexcept {
        ...
        EINSUMS_ASSERT(first <= last);
    }

    /// \brief Construct a new contiguous_index_queue.
    ///
    /// Construct a new queue with an empty range.
    constexpr contiguous_index_queue() noexcept : initial_range{}, current_range{} {}
    ...
    constexpr contiguous_index_queue(T first, T last) noexcept : initial_range{}, current_range{} { reset(first, last); }
    ...
    constexpr contiguous_index_queue(contiguous_index_queue<T> const &other) : initial_range{other.initial_range}, current_range{} {
    ...
    constexpr bool empty() noexcept { return current_range.data_.load(std::memory_order_relaxed).empty(); }
