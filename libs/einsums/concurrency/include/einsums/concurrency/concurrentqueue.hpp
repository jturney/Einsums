...
static const thread_id_t invalid_thread_id2 = 0xFFFF'FFFFU; // Not technically guaranteed to be invalid, but is never used in practice. Note
...
#            define MOODYCAMEL_CPP11_THREAD_LOCAL_SUPPORTED // tentatively enabled for now; years ago several users report having problems
                                                            // with it on
#        endif
#    endif
#endif
...
#        define MOODYCAMEL_ALIGNED_TYPE_LIKE(T, obj) typename detail::Vs2013Aligned<std::alignment_of<obj>::value, T>::type
...
#        define MOODYCAMEL_ALIGNED_TYPE_LIKE(T, obj) alignas(alignof(obj)) typename detail::identity<T>::type
#    endif
#endif
} // namespace einsums::concurrency::detail
...
class ConcurrentQueue {
  public:
    using producer_token_t = ::einsums::concurrency::detail::ProducerToken;
    using consumer_token_t = ::einsums::concurrency::detail::ConsumerToken;
...
#    pragma warning(disable : 4309) // static_cast: Truncation of constant value
    ...
    static const size_t MAX_SUBQUEUE_SIZE =
        (detail::const_numeric_max<size_t>::value - static_cast<size_t>(Traits::MAX_SUBQUEUE_SIZE) < BLOCK_SIZE)
...
#    pragma warning(pop)
  ...
  public:
    ...
    bool try_dequeue(consumer_token_t &token, U &item) {
        ...
        // consumer dictates the rotation speed of everyone else, more or less If you see that the global offset has changed, you must reset
        // your consumption counter and move to your designated place If there's no items where you're supposed to be, keep moving until you
        // find a producer with some items If the global offset has not changed but you've run out of items to consume, move over from your
    ...
    static constexpr bool is_lock_free() {
        return detail::static_is_lock_free<bool>::value == 2 && detail::static_is_lock_free<size_t>::value == 2 &&
               detail::static_is_lock_free<std::uint32_t>::value == 2 && detail::static_is_lock_free<index_t>::value == 2 &&
               detail::static_is_lock_free<void *>::value == 2 &&
               detail::static_is_lock_free<typename detail::thread_id_converter<detail::thread_id_t>::thread_id_numeric_size_t>::value == 2;
    }
  ...
  private:
    ...
    struct ProducerBase : public detail::ConcurrentQueueProducerTypelessBase {
        ...
        inline size_t size_approx() const {
            ...
            return detail::circular_less_than(head, tail) ? static_cast<size_t>(tail - head) : 0;
        }
    ...
    struct ExplicitProducer : public ProducerBase {
        explicit ExplicitProducer(ConcurrentQueue *parent_)
            ...
            size_t poolBasedIndexSize = detail::ceil_to_pow_2(parent_->initialBlockPoolSize) >> 1;
        ...
        ~ExplicitProducer() {
            ...
            if (this->tailBlock != nullptr) { // Note this means there must be a block index too
                ...
                if ((this->headIndex.load(std::memory_order_relaxed) & static_cast<index_t>(BLOCK_SIZE - 1)) != 0) {
                    ...
                    while (detail::circular_less_than<index_t>(pr_blockIndexEntries[i].base + BLOCK_SIZE,
                                                               this->headIndex.load(std::memory_order_relaxed))) {
                        i = (i + 1) & (pr_blockIndexSize - 1);
                    }
                    assert(
                        detail::circular_less_than<index_t>(pr_blockIndexEntries[i].base, this->headIndex.load(std::memory_order_relaxed)));
        ...
        bool dequeue(U &element) {
            ...
            if (detail::circular_less_than<index_t>(this->dequeueOptimisticCount.load(std::memory_order_relaxed) - overcommit, tail)) {
                ...
                if ((detail::likely)(detail::circular_less_than<index_t>(myDequeueCount - overcommit, tail))) {
        ...
        bool MOODYCAMEL_NO_TSAN enqueue_bulk(It itemFirst, size_t count) {
            ...
            if (blockBaseDiff > 0) {
                ...
                while (blockBaseDiff > 0) {
                    ...
                    auto head = this->headIndex.load(std::memory_order_relaxed);
                    assert(!detail::circular_less_than<index_t>(currentTailIndex, head));
                    bool full = !detail::circular_less_than<index_t>(head, currentTailIndex + BLOCK_SIZE) ||
                                (MAX_SUBQUEUE_SIZE != detail::const_numeric_max<size_t>::value &&
                                 (MAX_SUBQUEUE_SIZE == 0 || MAX_SUBQUEUE_SIZE - BLOCK_SIZE < currentTailIndex - head));
                    ...
                    MOODYCAMEL_TRY {
                        while (currentTailIndex != stopIndex) {
                            ...
                            new ((*this->tailBlock)[currentTailIndex])
                                T(detail::nomove_if<!MOODYCAMEL_NOEXCEPT_CTOR(T, decltype(*itemFirst),
                                                                              new (static_cast<T *>(nullptr))
                                                                                  T(detail::deref_noexcept(itemFirst)))>::eval(*itemFirst));
                            ++currentTailIndex;
                            ++itemFirst;
                        }
                    }
                    MOODYCAMEL_CATCH(...) {
                        ...
                        if (!detail::is_trivially_destructible<T>::value) {
                            ...
                            while (true) {
                                ...
                                if (detail::circular_less_than<index_t>(constructedStopIndex, stopIndex)) {
            ...
            MOODYCAMEL_CONSTEXPR_IF(
                !MOODYCAMEL_NOEXCEPT_CTOR(T, decltype(*itemFirst), new (static_cast<T *>(nullptr)) T(detail::deref_noexcept(itemFirst)))) {
            ...
            if (detail::circular_less_than<size_t>(0, desiredCount)) {
                ...
                if (detail::circular_less_than<size_t>(0, actualCount)) {
                    ...
                    do {
                        ...
                        endIndex   = detail::circular_less_than<index_t>(firstIndex + static_cast<index_t>(actualCount), endIndex)
                                         ? firstIndex + static_cast<index_t>(actualCount)
                                         : endIndex;
                        auto block = localBlockIndex->entries[indexIndex].block;
                        // NOLINTNEXTLINE(bugprone-assignment-in-if-condition)
                        if (MOODYCAMEL_NOEXCEPT_ASSIGN(T, T &&, detail::deref_noexcept(itemFirst) = std::move((*(*block)[index])))) {
      ...
      private:
        ...
        bool new_block_index(size_t numberOfFilledSlotsToExpose) {
            ...
            auto newBlockIndexEntries =
                reinterpret_cast<BlockIndexEntry *>(detail::align_for<BlockIndexEntry>(newRawPtr + sizeof(BlockIndexHeader)));
...
#    pragma warning(pop)
      ...
      private:
        ...
        bool new_block_index() {
            ...
            auto entries = reinterpret_cast<BlockIndexEntry *>(detail::align_for<BlockIndexEntry>(raw + sizeof(BlockIndexHeader)));
            auto index   = reinterpret_cast<BlockIndexEntry **>(
                detail::align_for<BlockIndexEntry *>(reinterpret_cast<char *>(entries) + sizeof(BlockIndexEntry) * entryCount));
            if (prev != nullptr) {
  ...
  private:
    ...
    friend void einsums::concurrency::detail::swap(typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP &,
    ...
    inline void populate_initial_implicit_producer_hash() {
        MOODYCAMEL_CONSTEXPR_IF(INITIAL_IMPLICIT_PRODUCER_HASH_SIZE == 0) {
            ...
            for (size_t i = 0; i != INITIAL_IMPLICIT_PRODUCER_HASH_SIZE; ++i) {
                initialImplicitProducerHashEntries[i].key.store(detail::invalid_thread_id, std::memory_order_relaxed);
            }
            hash->prev = nullptr;
        ...
        while (true) {
            ...
            if (newCount >= (mainHash->capacity >> 1) && !implicitProducerHashResizeInProgress.test_and_set(std::memory_order_acquire)) {
                ...
                if (newCount >= (mainHash->capacity >> 1)) {
                    ...
                    newHash->entries =
                        reinterpret_cast<ImplicitProducerKVP *>(detail::align_for<ImplicitProducerKVP>(raw + sizeof(ImplicitProducerHash)));
                    for (size_t i = 0; i != newCapacity; ++i) {
                        new (newHash->entries + i) ImplicitProducerKVP;
                        newHash->entries[i].key.store(detail::invalid_thread_id, std::memory_order_relaxed);
                    }
...
#    endif
        ...
        for (; hash != nullptr; hash = hash->prev) {
            ...
            do {
                index &= hash->capacity - 1u;
                probedKey = id;
                if (hash->entries[index].key.compare_exchange_strong(probedKey, detail::invalid_thread_id2, std::memory_order_seq_cst,
                                                                     std::memory_order_relaxed)) {
                    break;
                }
                ++index;
            } while (probedKey != detail::invalid_thread_id); // Can happen if the hash has changed but we weren't put back in it yet, or if
                                                              // we weren't added to this hash in the first place
        }
    ...
    static inline void *aligned_malloc(size_t size) {
        MOODYCAMEL_CONSTEXPR_IF(std::alignment_of<TAlign>::value <= std::alignment_of<detail::max_align_t>::value)
