...
#include <einsums/concurrency/detail/freelist.hpp>
...
enum deque_status_type { stable, rpush, lpush };
...
struct deque_node //-V690
{
    ...
    deque_node() : left(nullptr), right(nullptr), data() {}

    deque_node(deque_node const &p) : left(p.left.load(std::memory_order_relaxed)), right(p.right.load(std::memory_order_relaxed)) {}
...
struct deque_anchor //-V690
{
  ...
  public:
    ...
    deque_anchor(deque_anchor const &p) : pair_(p.pair_.load(std::memory_order_relaxed)) {}

    deque_anchor(pair const &p) : pair_(p) {}

    deque_anchor(node *lptr, node *rptr, tag_t status = stable, tag_t tag = 0) : pair_(pair(lptr, rptr, status, tag)) {}

    pair lrs(std::memory_order mo = std::memory_order_acquire) const { return pair_.load(mo); }

    node *left(std::memory_order mo = std::memory_order_acquire) const { return pair_.load(mo).get_left_ptr(); }

    node *right(std::memory_order mo = std::memory_order_acquire) const { return pair_.load(mo).get_right_ptr(); }

    tag_t status(std::memory_order mo = std::memory_order_acquire) const { return pair_.load(mo).get_left_tag(); }

    tag_t tag(std::memory_order mo = std::memory_order_acquire) const { return pair_.load(mo).get_right_tag(); }

    bool cas(deque_anchor &expected, deque_anchor const &desired, std::memory_order mo = std::memory_order_acq_rel) {
        return pair_.compare_exchange_strong(expected.load(std::memory_order_acquire), desired.load(std::memory_order_acquire), mo);
    }

    bool cas(pair &expected, deque_anchor const &desired, std::memory_order mo = std::memory_order_acq_rel) {
        return pair_.compare_exchange_strong(expected, desired.load(std::memory_order_acquire), mo);
    }

    bool cas(deque_anchor &expected, pair const &desired, std::memory_order mo = std::memory_order_acq_rel) {
        return pair_.compare_exchange_strong(expected.load(std::memory_order_acquire), desired, mo);
    }

    bool cas(pair &expected, pair const &desired, std::memory_order mo = std::memory_order_acq_rel) {
        return pair_.compare_exchange_strong(expected, desired, mo);
    }

    bool operator==(deque_anchor const &rhs) const {
        return pair_.load(std::memory_order_acquire) == rhs.pair_.load(std::memory_order_acquire);
    }

    bool operator!=(deque_anchor const &rhs) const { return !(*this == rhs); }

    bool operator==(pair const &rhs) const { return pair_.load(std::memory_order_acquire) == rhs; }
...
struct deque {
  ...
  public:
    ...
    using pool = typename std::conditional<std::is_same<freelist_t, caching_freelist_t>::value, caching_freelist<node, node_allocator>,
  ...
  private:
    ...
    static constexpr std::size_t padding_size = BOOST_LOCKFREE_CACHELINE_BYTES - sizeof(anchor); //-V103
    ...
    void stabilize_left(anchor_pair &lrs) {
        ...
        if (prevnext.get_ptr() != lrs.get_left_ptr()) {
            ...
            // problem.
            if (!prev.get_ptr()->left.compare_exchange_strong(prevnext, node_pointer(lrs.get_left_ptr(), prevnext.get_tag() + 1)))
                return;
        }
        // Try to update the anchor, modifying the status and ABA tag.
        anchor_.cas(lrs, anchor_pair(lrs.get_left_ptr(), lrs.get_right_ptr(), stable, lrs.get_right_tag() + 1));
    }
    ...
    void stabilize_right(anchor_pair &lrs) {
        ...
        if (prevnext.get_ptr() != lrs.get_right_ptr()) {
            ...
            // Attempt the CAS, incrementing the tag to protect from the ABA
            // problem.
            if (!prev.get_ptr()->right.compare_exchange_strong(prevnext, node_pointer(lrs.get_right_ptr(), prevnext.get_tag() + 1)))
                return;
        }
        // Try to update the anchor, modifying the status and ABA tag.
        anchor_.cas(lrs, anchor_pair(lrs.get_left_ptr(), lrs.get_right_ptr(), stable, lrs.get_right_tag() + 1));
  ...
  public:
    ...
    bool empty() const { return anchor_.lrs(std::memory_order_relaxed).get_left_ptr() == nullptr; }
    ...
    bool push_left(T data) {
        ...
        while (true) {
            ...
            if (lrs.get_left_ptr() == nullptr) {
                ...
                // element.
                if (anchor_.cas(lrs, anchor_pair(n, n, lrs.get_left_tag(), lrs.get_right_tag() + 1)))
                    return true;
            }
