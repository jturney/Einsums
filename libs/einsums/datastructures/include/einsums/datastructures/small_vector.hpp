...
// the einsums one being significantly worse.
#if 1    // !defined(EINSUMS_HAVE_CXX17_MEMORY_RESOURCE)
...
namespace einsums::detail {
    ...
    public:
        EINSUMS_NO_UNIQUE_ADDRESS Allocator allocator_;
    };
    ...
    struct memory_storage final
    {
        ...
        memory_storage(memory_storage&& rhs) noexcept
          ...
          , resource_(EINSUMS_MOVE(rhs.resource_))
        ...
        EINSUMS_NO_UNIQUE_ADDRESS allocator_memory_resource<T, Allocator> resource_;
    ...
    public:
        ...
        small_vector(small_vector&& rhs) noexcept
          : storage_(rhs.storage_)
          , data_(EINSUMS_MOVE(rhs.data_), storage_.allocator_)
        {
        }
        ...
        small_vector& operator=(small_vector&& rhs) noexcept
        {
            if (this != &rhs)
            {
                ...
                // reconstruct the memory management infrastructure for
                // the new instance
                storage_ = EINSUMS_MOVE(rhs.storage_);

                // fill the new instance with the moved rhs data
                new (&data_) data_type(EINSUMS_MOVE(rhs.data_), storage_.allocator_);
            }
            return *this;
        }
        ...
        {
            return data_.insert(pos, EINSUMS_MOVE(value));
        }
        ...
        void push_back(value_type&& value) { data_.push_back(EINSUMS_MOVE(value)); }
        ...
        void swap(small_vector& other) noexcept
        {
            ...
            small_vector tmp = EINSUMS_MOVE(*this);
            *this = EINSUMS_MOVE(other);
            other = EINSUMS_MOVE(tmp);
        }
...
namespace std {
    ...
    template <typename T, std::size_t Size, typename Allocator>
    void swap(einsums::detail::small_vector<T, Size, Allocator>& lhs,
        einsums::detail::small_vector<T, Size, Allocator>& rhs)
    {
        lhs.swap(rhs);
    }
}    // namespace std
