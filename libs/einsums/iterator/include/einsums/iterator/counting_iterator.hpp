...
#include <einsums/iterator/iterator_adaptor.hpp>
...
namespace einsums::util {
    ...
    namespace detail {
        ...
        struct counting_iterator_base
        {
            ...
            using base_traversal =
                ::einsums::detail::lazy_conditional<std::is_integral<Incrementable>::value,
                    ::einsums::detail::type_identity<std::random_access_iterator_tag>,
                    iterator_category<Incrementable>>;

            using traversal =
                ::einsums::detail::lazy_conditional_t<std::is_void<CategoryOrTraversal>::value,
                    base_traversal, ::einsums::detail::type_identity<CategoryOrTraversal>>;
            ...
            using difference = ::einsums::detail::lazy_conditional_t<std::is_void<Difference>::value,
                base_difference, ::einsums::detail::type_identity<Difference>>;
    ...
    public:
        ...
        EINSUMS_HOST_DEVICE explicit counting_iterator(Incrementable x)
          : base_type(x)
        {
        }

    private:
        EINSUMS_HOST_DEVICE typename base_type::reference dereference() const
        {
    ...
    private:
        template <typename Iterator>
        EINSUMS_HOST_DEVICE bool equal(Iterator const& rhs) const
        {
            return this->base() == rhs.base();
        }

        EINSUMS_HOST_DEVICE void increment() { ++this->base_reference(); }

        EINSUMS_HOST_DEVICE void decrement() { --this->base_reference(); }

        template <typename Distance>
        EINSUMS_HOST_DEVICE void advance(Distance n)
        {
        ...
        EINSUMS_HOST_DEVICE typename base_type::difference_type distance_to(
    ...
    EINSUMS_HOST_DEVICE inline counting_iterator<Incrementable> make_counting_iterator(Incrementable x)
    {
