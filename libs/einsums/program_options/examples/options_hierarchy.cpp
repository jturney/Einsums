...
class OptionsHierarchy {
  ...
  private:
    ...
    void SetConfigOnlyOptions() {
        ...
        store(po::command_line_parser(1, argv).allow_unregistered().options(config_only_options).run(), results);
        notify(results);
    }
    ...
    void ParseCommandLine(int argc, char *argv[]) {
        ...
        store(po::command_line_parser(argc, argv).allow_unregistered().options(cmd_opts).positional(positional_options).run(), results);
        notify(results);
    }
    ...
    void ParseEnvironment() {
        store(po::parse_environment(common_options, std::bind(&OptionsHierarchy::EnvironmentMapper, this, std::placeholders::_1)), results);
        notify(results);
    }
