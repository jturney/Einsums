...
#include <einsums/program_options.hpp>
...
int einsums_main(variables_map &vm) {
...
int main(int argc, char *argv[]) {
    std::vector<std::string> cfg = {"einsums.commandline.rethrow_errors!=1"};

    char help_option[] = "--einsums:help";
    ...
    einsums::init_params init_args;
    init_args.desc_cmdline = cmdline;
    init_args.cfg          = cfg;

    // This test should just run without crashing
    EINSUMS_TEST(true);
    return einsums::init(einsums_main, argc + 1, newargv.data(), init_args);
}
