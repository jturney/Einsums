...
void test_unicode_to_unicode() {
    ...
    EINSUMS_TEST(vm["foo"].as<wstring>() == L"\x044F");
    EINSUMS_TEST_EQ(parsed.options[0].original_tokens.size(), std::size_t(1));
    EINSUMS_TEST(parsed.options[0].original_tokens[0] == L"--foo=\x044F");
}
...
void test_unicode_to_native() {
    std::codecvt<wchar_t, char, mbstate_t> *facet = new einsums::program_options::detail::utf8_codecvt_facet;
    ...
    EINSUMS_TEST_EQ(vm["foo"].as<string>(), "\xD1\x8F");
}
...
void check_value(const woption &option, const char *name, const wchar_t *value) {
    ...
    EINSUMS_TEST(option.value.front() == value);
}
...
void test_command_line() {
    ...
    desc.add_options()("foo,f", new untyped_value(), "")
        ...
        ("bar,b", value<std::string>(), "")("baz", new untyped_value())("qux,plug*", new untyped_value());
    ...
    EINSUMS_TEST_EQ(a4.size(), std::size_t(5));
