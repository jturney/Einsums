...
void check_value(const option &option, const char *name, const char *value) {
    EINSUMS_TEST_EQ(option.string_key, name);
    EINSUMS_TEST_EQ(option.value.size(), std::size_t(1));
    EINSUMS_TEST_EQ(option.value.front(), value);
}
...
void test_many_different_options() {
    ...
    desc.add_options()("foo,f", new untyped_value(), "")("bar,b", value<std::string>(), "")("car,voiture", new untyped_value())(
        "dog,dawg", new untyped_value())("baz", new untyped_value())("plug*", new untyped_value());
...
void test_not_crashing_with_empty_string_values() {
    ...
    variables_map vm;
    store(parse_command_line(sizeof(cmdline4) / sizeof(const char *), const_cast<char **>(cmdline4), desc2), vm);
}

void test_multitoken() {
    const char         *cmdline5[] = {"", "-p7", "-o", "1", "2", "3", "-x8"};
    options_description desc3;
    desc3.add_options()(",p", value<string>())(",o", value<string>()->multitoken())(",x", value<string>());
    vector<option> a5 =
        parse_command_line(sizeof(cmdline5) / sizeof(const char *), const_cast<char **>(cmdline5), desc3, 0, additional_parser).options;
    EINSUMS_TEST_EQ(a5.size(), 3u);
    check_value(a5[0], "-p", "7");
    EINSUMS_TEST_EQ(a5[1].value.size(), std::size_t(3));
    EINSUMS_TEST_EQ(a5[1].string_key, "-o");
    EINSUMS_TEST_EQ(a5[1].value[0], "1");
    EINSUMS_TEST_EQ(a5[1].value[1], "2");
    EINSUMS_TEST_EQ(a5[1].value[2], "3");
...
void test_multitoken_and_multiname() {
    ...
    options_description desc;
    desc.add_options()("bar,b", value<string>())("foo,fee,f", value<string>()->multitoken())("fizbaz,baz,z", value<string>());
    ...
    EINSUMS_TEST_EQ(parsed_options.size(), 5u);
    check_value(parsed_options[0], "foo", "one");
    check_value(parsed_options[1], "bar", "two");
    EINSUMS_TEST_EQ(parsed_options[2].string_key, "foo");
    EINSUMS_TEST_EQ(parsed_options[2].value.size(), std::size_t(2));
    EINSUMS_TEST_EQ(parsed_options[2].value[0], "three");
    EINSUMS_TEST_EQ(parsed_options[2].value[1], "four");
...
void test_multitoken_vector_option() {
    ...
    desc4.add_options()("multitoken,multi-token,m", value<std::vector<std::string>>()->multitoken(), "values")("file", value<std::string>(),
                                                                                                               "the file to process");
    ...
    vector<option> a6         = command_line_parser(sizeof(cmdline6) / sizeof(const char *), const_cast<char **>(cmdline6))
                            ...
                            .run()
                            .options;
    EINSUMS_TEST_EQ(a6.size(), 2u);
    EINSUMS_TEST_EQ(a6[0].value.size(), std::size_t(2));
    EINSUMS_TEST_EQ(a6[0].string_key, "multitoken");
    EINSUMS_TEST_EQ(a6[0].value[0], "token1");
    EINSUMS_TEST_EQ(a6[0].value[1], "token2");
    EINSUMS_TEST_EQ(a6[1].string_key, "file");
    EINSUMS_TEST_EQ(a6[1].value.size(), std::size_t(1));
    EINSUMS_TEST_EQ(a6[1].value[0], "some_file");
}
...
void test_config_file(const char *config_file) {
    ...
    EINSUMS_TEST_EQ(a1.size(), std::size_t(7));
    ...
    EINSUMS_TEST_EQ(a2.size(), std::size_t(7));
...
void test_environment() {
    ...
    EINSUMS_TEST_EQ(p.options.size(), std::size_t(1));
    EINSUMS_TEST_EQ(p.options[0].string_key, "foo");
    EINSUMS_TEST_EQ(p.options[0].value.size(), std::size_t(1));
    EINSUMS_TEST_EQ(p.options[0].value[0], "1");
...
void test_unregistered() {
    ...
    EINSUMS_TEST_EQ(a1.size(), std::size_t(3));
    EINSUMS_TEST_EQ(a1[0].string_key, "foo");
    EINSUMS_TEST_EQ(a1[0].unregistered, true);
    EINSUMS_TEST_EQ(a1[0].value.size(), std::size_t(1));
    EINSUMS_TEST_EQ(a1[0].value[0], "12");
    EINSUMS_TEST_EQ(a1[1].string_key, "bar");
    EINSUMS_TEST_EQ(a1[1].unregistered, true);
    EINSUMS_TEST_EQ(a1[2].string_key, "");
    EINSUMS_TEST_EQ(a1[2].unregistered, false);

    vector<string> a2 = collect_unrecognized(a1, include_positional);
    EINSUMS_TEST_EQ(a2[0], "--foo=12");
    EINSUMS_TEST_EQ(a2[1], "--bar");
    EINSUMS_TEST_EQ(a2[2], "1");
    ...
    EINSUMS_TEST_EQ(a3.size(), std::size_t(2));
...
int main() {
    ...
    // write config data to file
    {
        ...
        f.write(config_file_content, (sizeof(config_file_content) / sizeof(config_file_content[0])) - 1);
    }
