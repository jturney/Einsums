...
using einsums::program_options::detail::cmdline;
...
/* Parses the syntax description in 'syntax' and initialized
   'cmd' accordingly'
   The "einsums::program_options" in parameter type is needed because CW9
...
void test_cmdline(const char *syntax, command_line_style::style_t style, const test_case *cases) {
    for (int i = 0; cases[i].input; ++i) {
        ...
        EINSUMS_TEST_EQ(status, cases[i].expected_status);
        EINSUMS_TEST_EQ(result, cases[i].expected_result);
    }
}
...
void test_additional_parser() {
    ...
    // the long_names() API function was introduced in Boost V1.68
    EINSUMS_TEST_EQ(result.size(), std::size_t(3));
    EINSUMS_TEST_EQ(result[0].string_key, "response-file");
    EINSUMS_TEST_EQ(result[0].value[0], "config");
    EINSUMS_TEST_EQ(result[1].string_key, "foo");
    EINSUMS_TEST_EQ(result[1].value[0], "1");
    // the long_names() API function was introduced in Boost V1.68
    EINSUMS_TEST_EQ(result[2].string_key, "bar");
    EINSUMS_TEST_EQ(result[2].value[0], "11");
...
void test_style_parser() {
    ...
    EINSUMS_TEST_EQ(result.size(), std::size_t(2));
    EINSUMS_TEST_EQ(result[0].string_key, "foo");
    EINSUMS_TEST_EQ(result[0].value[0], "1");
    EINSUMS_TEST_EQ(result[1].string_key, "bar");
    EINSUMS_TEST_EQ(result[1].value[0], "1");
}
...
void test_unregistered() {
    ...
    vector<option> result = cmd.run();
    EINSUMS_TEST_EQ(result.size(), std::size_t(5));
    // --foo=1
    EINSUMS_TEST_EQ(result[0].string_key, "foo");
    EINSUMS_TEST_EQ(result[0].unregistered, true);
    EINSUMS_TEST_EQ(result[0].value[0], "1");
    // --bar
    EINSUMS_TEST_EQ(result[1].string_key, "bar");
    EINSUMS_TEST_EQ(result[1].unregistered, true);
    EINSUMS_TEST(result[1].value.empty());
    // '1' is considered a positional option, not a value to
    // --bar
    EINSUMS_TEST(result[2].string_key.empty());
    EINSUMS_TEST_EQ(result[2].position_key, 0);
    EINSUMS_TEST_EQ(result[2].unregistered, false);
    EINSUMS_TEST_EQ(result[2].value[0], "1");
    // -b
    EINSUMS_TEST_EQ(result[3].string_key, "-b");
    EINSUMS_TEST_EQ(result[3].unregistered, true);
    EINSUMS_TEST(result[3].value.empty());
    // -biz
    EINSUMS_TEST_EQ(result[4].string_key, "-b");
    EINSUMS_TEST_EQ(result[4].unregistered, true);
    EINSUMS_TEST_EQ(result[4].value[0], "iz");
    ...
    EINSUMS_TEST_EQ(result.size(), std::size_t(3));
    EINSUMS_TEST_EQ(result[0].string_key, "help");
    EINSUMS_TEST_EQ(result[0].unregistered, false);
    EINSUMS_TEST(result[0].value.empty());
    EINSUMS_TEST_EQ(result[1].string_key, "-c");
    EINSUMS_TEST_EQ(result[1].unregistered, true);
    EINSUMS_TEST(result[1].value.empty());
    EINSUMS_TEST_EQ(result[2].string_key, "magic");
    EINSUMS_TEST_EQ(result[2].unregistered, false);
    EINSUMS_TEST_EQ(result[2].value[0], "c");
