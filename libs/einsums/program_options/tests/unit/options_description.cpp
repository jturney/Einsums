...
void test_approximation()
{
    ...
    EINSUMS_TEST_EQ(desc.find("fo", true).long_name(), "foo");

    EINSUMS_TEST_EQ(desc.find("all", true).long_name(), "all");
    EINSUMS_TEST_EQ(desc.find("all-ch", true).long_name(), "all-chroots");
    ...
    EINSUMS_TEST_EQ(desc2.find("config", true).long_name(), "config");
    EINSUMS_TEST_EQ(desc2.find("config-value", true).long_name(), "config-value");
}
...
void test_approximation_with_multiname_options() {
    ...
    EINSUMS_TEST_EQ(desc.find("fo", true).long_name(), "qux");

    EINSUMS_TEST_EQ(desc.find("all", true).long_name(), "everything");
    EINSUMS_TEST_EQ(desc.find("all-ch", true).long_name(), "chroots");

    EINSUMS_TEST_EQ(desc.find("foo", false, false, false).long_names().second, std::size_t(1));
    EINSUMS_TEST_EQ(desc.find("foo", false, false, false).long_names().first[0], "foo");

    EINSUMS_TEST_EQ(desc.find("fe", false, false, false).long_names().second, std::size_t(2));
    EINSUMS_TEST_EQ(desc.find("fe", false, false, false).long_names().first[0], "fe");
    EINSUMS_TEST_EQ(desc.find("baz", false, false, false).long_names().first[1], "baz");

    EINSUMS_TEST_EQ(desc.find("baz", false, false, false).long_names().second, std::size_t(2));
    EINSUMS_TEST_EQ(desc.find("baz", false, false, false).long_names().first[0], "fizbaz");
    EINSUMS_TEST_EQ(desc.find("baz", false, false, false).long_names().first[1], "baz");
}
...
void test_long_names_for_option_description() {
    ...
    EINSUMS_TEST_EQ(desc.find("qux", false, false, false).long_names().second, std::size_t(2));
    EINSUMS_TEST_EQ(desc.find("qux", false, false, false).long_names().first[0], "qux");
    EINSUMS_TEST_EQ(desc.find("qux", false, false, false).long_names().first[1], "fo");
}
...
void test_multiname_option_formatting() {
    ...
    stringstream ss;
    ss << desc;
    EINSUMS_TEST_EQ(ss.str(), "  --foo arg             a multiple-name option\n");
}
...
void test_formatting_description_length() {
    ...
    }
    {
        ...
        stringstream ss;
        ss << desc;
        EINSUMS_TEST_EQ(ss.str(), "  --an-option-that-encroaches-description arg\n"
...
void test_long_default_value() {
    ...
    stringstream ss;
    ss << desc;
    EINSUMS_TEST_EQ(ss.str(), "  -c [ --cfgfile ] arg (=/usr/local/etc/myprogramXXXXXXXXX/configuration.conf)\n"
                              "                                        the configfile\n");
}
...
void test_word_wrapping() {
    ...
    desc.add_options()("prefix", value<string>()->default_value("/h/proj/tmp/dispatch"), "root path of the dispatch installation");
    desc.add_options()("opt1", "this_is_a_sufficiently_long_text_to_require_word-wrapping_but_cannot_be_wrapped");
    desc.add_options()("opt2", "this_is_a_sufficiently long_text_to_require_word-wrapping");
    desc.add_options()("opt3", "this_is_a sufficiently_long_text_to_require_word-wrapping_but_will_not_be_wrapped");

    stringstream ss;
    ss << desc;
    EINSUMS_TEST_EQ(ss.str(), "Supported options:\n"
