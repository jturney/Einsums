...
void test_ambiguous() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(e.alternatives().size(), std::size_t(2));
        EINSUMS_TEST_EQ(e.get_option_name(), "-c");
        EINSUMS_TEST_EQ(e.alternatives()[0], "cfgfile");
        EINSUMS_TEST_EQ(e.alternatives()[1], "output");
    }
}
...
void test_ambiguous_long() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(e.alternatives().size(), std::size_t(2));
        EINSUMS_TEST_EQ(e.get_option_name(), "--output");
        EINSUMS_TEST_EQ(e.alternatives()[0], "output");
        EINSUMS_TEST_EQ(e.alternatives()[1], "output");
    }
}
...
void test_ambiguous_multiple_long_names() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(e.alternatives().size(), std::size_t(2));
        EINSUMS_TEST_EQ(e.get_option_name(), "--foo");
        EINSUMS_TEST_EQ(e.alternatives()[0], "cfgfile");
        EINSUMS_TEST_EQ(e.alternatives()[1], "output");
    }
}
...
void test_unknown_option() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(e.get_option_name(), "-f");
        EINSUMS_TEST_EQ(string(e.what()), "unrecognised option '-f'");
    }
}
...
void test_multiple_values() {
    ...
    try {
        ...
        //
        EINSUMS_TEST_EQ(e.get_option_name(), "--cfgfile");
        EINSUMS_TEST_EQ(string(e.what()), "option '--cfgfile' only takes a single argument");
    }
}
...
void test_multiple_occurrences() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(e.get_option_name(), "--cfgfile");
        EINSUMS_TEST_EQ(string(e.what()), "option '--cfgfile' cannot be specified more than once");
    }
}
...
void test_multiple_occurrences_with_different_names() {
    ...
    try {
        ...
        EINSUMS_TEST((e.get_option_name() == "--cfgfile") || (e.get_option_name() == "--config-file"));
        EINSUMS_TEST((string(e.what()) == "option '--cfgfile' cannot be specified more than once") ||
...
void test_missing_value() {
    ...
    try {
        ...
        EINSUMS_TEST_EQ(static_cast<std::underlying_type_t<invalid_syntax::kind_t>>(e.kind()),
                        static_cast<std::underlying_type_t<invalid_syntax::kind_t>>(invalid_syntax::missing_parameter));
        EINSUMS_TEST_EQ(e.tokens(), "--cfgfile");
    }
}
