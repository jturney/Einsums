...
EINSUMS_EXPORT std::wstring from_8_bit(const std::string &s, const std::codecvt<wchar_t, char, std::mbstate_t> &cvt);

/** Converts from wchar_t string into local 8 bit encoding into using
    the specified locale facet. */
EINSUMS_EXPORT std::string to_8_bit(const std::wstring &s, const std::codecvt<wchar_t, char, std::mbstate_t> &cvt);

/** Converts 's', which is assumed to be in UTF8 encoding, into wide
    string. */
EINSUMS_EXPORT std::wstring from_utf8(const std::string &s);

/** Converts wide string 's' into string in UTF8 encoding. */
EINSUMS_EXPORT std::string to_utf8(const std::wstring &s);

/** Converts wide string 's' into local 8 bit encoding determined by
    the current locale. */
EINSUMS_EXPORT std::string to_local_8_bit(const std::wstring &s);

/** Converts 's', which is assumed to be in local 8 bit encoding, into wide
    string. */
EINSUMS_EXPORT std::wstring from_local_8_bit(const std::string &s);
...
/** Convert the input string into internal encoding used by
    ...
    */
EINSUMS_EXPORT std::string to_internal(const std::string &);
/** @overload */
EINSUMS_EXPORT std::string to_internal(const std::wstring &);
