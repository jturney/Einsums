...
#include <einsums/program_options/eof_iterator.hpp>
...
/** Standalone parser for config files in ini-line format.
    ...
    - Section names are given in brackets.
     ...
     class.
 */
class EINSUMS_EXPORT common_config_file_iterator : public eof_iterator<common_config_file_iterator, option> {
  public:
    common_config_file_iterator() : m_allow_unregistered(false) { found_eof(); }
...
#if defined(EINSUMS_MSVC) && EINSUMS_MSVC <= 1900
    void decrement() {}
...
class basic_config_file_iterator : public common_config_file_iterator {
  public:
     ...
     */
    basic_config_file_iterator(std::basic_istream<Char> &is, const std::set<std::string> &allowed_options, bool allow_unregistered = false);
...
struct null_deleter {
    void EINSUMS_STATIC_CALL_OPERATOR(void const *) {}
};

template <class Char>
basic_config_file_iterator<Char>::basic_config_file_iterator(std::basic_istream<Char> &is, const std::set<std::string> &allowed_options,
