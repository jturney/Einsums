...
template <class Char>
basic_command_line_parser<Char>::basic_command_line_parser(const std::vector<std::basic_string<Char>> &xargs)
    : detail::cmdline(to_internal(xargs)) {
}

template <class Char>
basic_command_line_parser<Char>::basic_command_line_parser(int argc, const Char *const argv[])
    : detail::cmdline(to_internal(std::vector<std::basic_string<Char>>(argv + 1, argv + argc))), m_desc() {
}

template <class Char>
basic_command_line_parser<Char> &basic_command_line_parser<Char>::options(const options_description &desc) {
...
template <class Char>
basic_command_line_parser<Char> &basic_command_line_parser<Char>::positional(const positional_options_description &desc) {
...
template <class Char>
basic_command_line_parser<Char> &basic_command_line_parser<Char>::extra_style_parser(style_parser s) {
...
template <class Char>
basic_parsed_options<Char> parse_command_line(int argc, const Char *const argv[], const options_description &desc, int style,
                                              std::function<std::pair<std::string, std::string>(const std::string &)> ext) {
    return basic_command_line_parser<Char>(argc, argv).options(desc).style(style).extra_parser(ext).run();
}
...
std::vector<std::basic_string<Char>> collect_unrecognized(const std::vector<basic_option<Char>> &options,
    ...
    for (std::size_t i = 0; i < options.size(); ++i) {
        if (options[i].unregistered || (mode == include_positional && options[i].position_key != -1)) {
            std::copy(options[i].original_tokens.begin(), options[i].original_tokens.end(), std::back_inserter(result));
        }
    }
    return result;
}
