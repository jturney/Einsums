...
extern EINSUMS_EXPORT std::string arg;
...
/* If v.size() > 1, throw validation_error.
   ...
   otherwise. */
template <class Char>
const std::basic_string<Char> &get_single_string(const std::vector<std::basic_string<Char>> &v, bool allow_empty = false) {
...
EINSUMS_EXPORT void check_first_occurrence(const std::any &value);
} // namespace validators
...
void validate(std::any &v, const std::vector<std::basic_string<Char>> &xs, T *, long) {
    ...
    try {
        v = std::any(einsums::string_util::from_string<T>(s));
    } catch (const einsums::string_util::detail::bad_lexical_cast &) {
        throw invalid_option_value(s);
    }
}

EINSUMS_EXPORT void validate(std::any &v, const std::vector<std::string> &xs, bool *, int);

EINSUMS_EXPORT void validate(std::any &v, const std::vector<std::wstring> &xs, bool *, int);
...
// value_semantic.cpp
EINSUMS_EXPORT void validate(std::any &v, const std::vector<std::string> &xs, std::string *, int);
EINSUMS_EXPORT void validate(std::any &v, const std::vector<std::wstring> &xs, std::string *, int);
...
void validate(std::any &v, const std::vector<std::basic_string<Char>> &s, std::vector<T> *, int) {
    ...
    for (std::size_t i = 0; i < s.size(); ++i) {
        ...
        } catch (const einsums::string_util::detail::bad_lexical_cast & /*e*/) {
...
template <class T, class Char>
void validate(std::any &v, const std::vector<std::basic_string<Char>> &s, std::optional<T> *, int) {
    validators::check_first_occurrence(v);
...
template <class T, class Char>
void typed_value<T, Char>::xparse(std::any &value_store, const std::vector<std::basic_string<Char>> &new_tokens) const {
...
typed_value<T> *value() {
    ...
    return einsums::program_options::value<T>(nullptr);
}
