...
invalid_option_value::invalid_option_value(const std::string &bad_value) : validation_error(validation_error::invalid_option_value) {
    set_substitute("value", bad_value);
}

#ifndef BOOST_NO_STD_WSTRING
invalid_option_value::invalid_option_value(const std::wstring &bad_value) : validation_error(validation_error::invalid_option_value) {
    set_substitute("value", convert_value(bad_value));
}
#endif

invalid_bool_value::invalid_bool_value(const std::string &bad_value) : validation_error(validation_error::invalid_bool_value) {
    set_substitute("value", bad_value);
}
...
error_with_option_name::error_with_option_name(const std::string &template_, const std::string &option_name,
    ...
    : error(template_), m_option_style(option_style), m_error_template(template_) {
...
string error_with_option_name::get_canonical_option_prefix() const {
    ...
    throw std::logic_error("error_with_option_name::m_option_style can only be one of [0, allow_dash_for_short, "
...
void error_with_option_name::substitute_placeholders(const string &error_template) const {
    ...
    for (const auto &substitution_default : m_substitution_defaults) {
        ...
        if (substitutions.count(substitution_default.first) == 0 || substitutions[substitution_default.first].length() == 0) {
            replace_token(substitution_default.second.first, substitution_default.second.second);
        }
    }
