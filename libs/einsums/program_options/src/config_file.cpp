...
common_config_file_iterator::common_config_file_iterator(const std::set<std::string> &allowed_options, bool allow_unregistered)
    : allowed_options(allowed_options), m_allow_unregistered(allow_unregistered) {
...
void common_config_file_iterator::add_option(const char *name) {
    ...
    if (*s.rbegin() == '*') {
        ...
        if (bad_prefixes)
            throw error("options '" + string(name) + "' and '" + *i + "*' will both match the same arguments from the configuration file");
...
#if defined(__COMO_VERSION__) && __COMO_VERSION__ >= 4303 || (defined(__sgi) && defined(_COMPILER_VERSION) && _COMPILER_VERSION >= 741)
template <>
