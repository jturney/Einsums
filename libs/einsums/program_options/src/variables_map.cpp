...
void store(const parsed_options &options, variables_map &xm, bool utf8) {
    ...
    // 'store'.
    EINSUMS_ASSERT(options.description);
...
abstract_variables_map::abstract_variables_map() : m_next(nullptr) {
}

abstract_variables_map::abstract_variables_map(const abstract_variables_map *next) : m_next(next) {
}
...
variables_map::variables_map(const abstract_variables_map *next) : abstract_variables_map(next) {
}
