...
#include <einsums/program_options/detail/convert.hpp>
#include <einsums/program_options/environment_iterator.hpp>
...
basic_parsed_options<wchar_t>::basic_parsed_options(const parsed_options &po)
    : description(po.description), utf8_encoded_options(po), m_options_prefix(po.m_options_prefix) {
    for (const auto &option : po.options)
        options.push_back(woption_from_option(option));
}

template <class Char>
basic_parsed_options<Char> parse_config_file(std::basic_istream<Char> &is, const options_description &desc, bool allow_unregistered) {
    ...
    for (const auto &option : options) {
        ...
        if (d.long_name().empty())
            throw error("abbreviated option names are not permitted in options configuration files");
    ...
    copy(detail::basic_config_file_iterator<Char>(is, allowed_options, allow_unregistered), detail::basic_config_file_iterator<Char>(),
         back_inserter(result.options));
    // Convert char strings into desired type.
    return basic_parsed_options<Char>(result);
}

template EINSUMS_EXPORT basic_parsed_options<char> parse_config_file(std::basic_istream<char> &is, const options_description &desc,
                                                                     bool allow_unregistered);

template EINSUMS_EXPORT basic_parsed_options<wchar_t> parse_config_file(std::basic_istream<wchar_t> &is, const options_description &desc,
                                                                        bool allow_unregistered);

template <class Char>
basic_parsed_options<Char> parse_config_file(const char *filename, const options_description &desc, bool allow_unregistered) {
    // Parser return char strings
...
template EINSUMS_EXPORT basic_parsed_options<char> parse_config_file(const char *filename, const options_description &desc,
                                                                     bool allow_unregistered);

template EINSUMS_EXPORT basic_parsed_options<wchar_t> parse_config_file(const char *filename, const options_description &desc,
                                                                        bool allow_unregistered);
