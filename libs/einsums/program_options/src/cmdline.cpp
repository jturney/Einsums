...
cmdline::cmdline(int argc, const char *const *argv) {
    init(vector<string>(argv + 1, argv + static_cast<std::size_t>(argc) + static_cast<std::size_t>(!argc)));
}
...
void cmdline::check_style(int style) const {
    ...
    if (allow_some_long && !(style & long_allow_adjacent) && !(style & long_allow_next))
        error = "einsums::program_options misconfiguration: choose one or other of "
    ...
    for (std::size_t i = 0; i < result.size(); ++i) {
        ...
        if (min_tokens < max_tokens && opt.value.size() < max_tokens) {
            ...
            for (; can_take_more && j < result.size(); --can_take_more, ++j) {
                ...
                EINSUMS_ASSERT(opt2.value.size() == 1);

                opt.value.push_back(opt2.value[0]);

                EINSUMS_ASSERT(opt2.original_tokens.size() == 1);
...
void cmdline::finish_option(option &opt, vector<string> &other_tokens, const vector<style_parser> &style_parsers) {
    ...
    try {
        ...
        const option_description *xd =
            m_desc->find_nothrow(opt.string_key, is_style_active(allow_guessing), is_style_active(long_case_insensitive),
                                 is_style_active(short_case_insensitive));
        ...
        if (present_tokens >= min_tokens) {
            ...
            for (; !other_tokens.empty() && min_tokens--;) {
                ...
                for (std::size_t i = 0; followed_option.empty() && i < style_parsers.size(); ++i) {
...
vector<option> cmdline::parse_long_option(vector<string> &args) {
    ...
    if (tok.size() >= 3 && tok[0] == '-' && tok[1] == '-') {
        ...
        if (p != tok.npos) {
            ...
            if (adjacent.empty())
                throw invalid_command_line_syntax(invalid_command_line_syntax::empty_adjacent_parameter, name, name,
...
vector<option> cmdline::parse_disguised_long_option(vector<string> &args) {
    ...
    if (tok.size() >= 2 && ((tok[0] == '-' && tok[1] != '-') || ((m_style & allow_slash_for_short) && tok[0] == '/'))) {
        try {
            if (m_desc->find_nothrow(tok.substr(1, tok.find('=') - 1), is_style_active(allow_guessing),
