...
option_description::option_description(const char *names, const value_semantic *s) : m_value_semantic(s) {
...
option_description &option_description::set_names(const char *_names) {
    ...
    EINSUMS_ASSERT(!m_long_names.empty() && "No option names were specified");
...
options_description_easy_init::options_description_easy_init(options_description *owner) : owner(owner) {
}
...
// NOLINTEND(bugprone-easily-swappable-parameters)
{
    ...
    EINSUMS_ASSERT(m_min_description_length < m_line_length - 1);
}

// NOLINTBEGIN(bugprone-easily-swappable-parameters)
options_description::options_description(const std::string &caption, unsigned line_length, unsigned min_description_length)
    // NOLINTEND(bugprone-easily-swappable-parameters)
    : m_caption(caption), m_line_length(line_length), m_min_description_length(min_description_length) {
    // we require a space between the option and description parts, so add 1.
    EINSUMS_ASSERT(m_min_description_length < m_line_length - 1);
}
...
void format_paragraph(std::ostream &os, std::string par, std::size_t indent, std::size_t line_length) {
    ...
    // indent.
    EINSUMS_ASSERT(indent < line_length);
    line_length -= indent;
    ...
    if (par_indent == string::npos) {
        ...
        // this EINSUMS_ASSERT may fail due to user error or
        // environment conditions!
        EINSUMS_ASSERT(par_indent < line_length);
    ...
    if (par.size() < line_length) {
        ...
        while (line_begin < par_end) // paragraph lines
        {
            ...
            string::const_iterator line_end =
                line_begin + static_cast<string::const_iterator::difference_type>((remaining < line_length) ? remaining : line_length);
            ...
            if ((*(line_end - 1) != ' ') && ((line_end < par_end) && (*line_end != ' '))) {
                ...
                string::const_iterator last_space =
                    find(reverse_iterator<string::const_iterator>(line_end), reverse_iterator<string::const_iterator>(line_begin), ' ')
                        .base();
...
void format_description(std::ostream &os, const std::string &desc, std::size_t first_column_width, std::size_t line_length) {
    // we need to use one char less per line to work correctly if actual
    // console has longer lines
    EINSUMS_ASSERT(line_length > 1);
    if (line_length > 1) {
        --line_length;
    }

    // line_length must be larger than first_column_width
    // this EINSUMS_ASSERT may fail due to user error or environment
    // conditions!
    EINSUMS_ASSERT(line_length > first_column_width);
...
void format_one(std::ostream &os, const option_description &opt, std::size_t first_column_width, std::size_t line_length) {
    stringstream ss;
