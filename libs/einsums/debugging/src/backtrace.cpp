...
#        define EINSUMS_HAVE_EXECINFO
#        define EINSUMS_HAVE_DLFCN
#        if defined(__GNUC__) && !defined(__clang__)
#            define EINSUMS_HAVE_UNWIND
#        endif
#    endif

#    if defined(__GNUC__) && !defined(__bgq__)
#        define EINSUMS_HAVE_ABI_CXA_DEMANGLE
#    endif

#    ifdef EINSUMS_HAVE_EXECINFO
#        include <execinfo.h>
#    endif

#    ifdef EINSUMS_HAVE_ABI_CXA_DEMANGLE
#        include <cxxabi.h>
#    endif

#    ifdef EINSUMS_HAVE_DLFCN
#        include <dlfcn.h>
#    endif
#    ifdef EINSUMS_HAVE_UNWIND
#        include <unwind.h>
#    endif
...
namespace einsums::debug::detail::stacktrace {
#    if defined(EINSUMS_HAVE_EXECINFO) && defined(EINSUMS_HAVE_UNWIND)
struct TraceData {
    TraceData(void **array, std::size_t size) : array_(array), size_(size), cfa_(0), count_(std::size_t(-1)) {}
...
_Unwind_Reason_Code trace_callback(_Unwind_Context *ctx, void *ptr) {
    ...
    TraceData &d = *(reinterpret_cast<TraceData *>(ptr));
...
#    elif defined(EINSUMS_HAVE_EXECINFO)
...
#    if defined(EINSUMS_HAVE_EXECINFO)
...
#    if defined(EINSUMS_HAVE_DLFCN) && defined(EINSUMS_HAVE_ABI_CXA_DEMANGLE)
...
#        endif
        ...
        if (info.dli_sname) {
#        if defined(EINSUMS_HAVE_STACKTRACES_DEMANGLE_SYMBOLS)
            int   status    = 0;
