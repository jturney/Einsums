...
ipaddr::ipaddr(std::uint32_t a) : data_(reinterpret_cast<const uint8_t *>(&ipdata_)), ipdata_(a) {
}
...
mem_crc32::mem_crc32(void const *a, std::size_t len) : addr_(reinterpret_cast<const uint64_t *>(a)), len_(len) {
}

std::ostream &operator<<(std::ostream &os, mem_crc32 const &p) {
    std::uint64_t const *uintBuf = static_cast<std::uint64_t const *>(p.addr_);
    os << "Memory:";
    os << " address " << ptr(p.addr_) << " length " << hex<6>(p.len_) << " CRC32:" << hex<8>(detail::crc32(p.addr_, p.len_)) << "\n";

    for (std::size_t i = 0; i < (std::min)(size_t(std::ceil(static_cast<double>(p.len_) / 8.0)), std::size_t(128)); i++) {
        os << hex<16>(*uintBuf++) << " ";
    }
    return os;
}
