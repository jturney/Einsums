...
// an aid for einsums development.
// ------------------------------------------------------------
...
//
// static einsums::debug::detail::enable_print<true> spq_deb("SUBJECT");
//
...
#define EINSUMS_DETAIL_DP_LAZY(printer, Expr) printer.eval([&] { return Expr; })
#define EINSUMS_DETAIL_DP(printer, Expr)                                                                               \
    if constexpr (printer.is_enabled()) {                                                                              \
        printer.Expr;                                                                                                  \
    };

#define EINSUMS_DETAIL_NS_DEBUG einsums::debug::detail
...
struct dec_impl {
    constexpr explicit dec_impl(T const &v) : data_(v) {}

    T const &data_;

    friend auto operator<<(std::ostream &os, dec_impl<N, T> const &d) -> std::ostream & {
...
auto dec(T const &v) -> dec_impl<N, T> {
...
// ------------------------------------------------------------------
struct ptr {
    EINSUMS_EXPORT ptr(void const *v);
    EINSUMS_EXPORT ptr(std::uintptr_t v);

    void const *data_;

    EINSUMS_EXPORT friend auto operator<<(std::ostream &os, ptr const &d) -> std::ostream &;
};
...
// ------------------------------------------------------------------
template <typename Int>
EINSUMS_EXPORT void print_hex(std::ostream &os, Int v, int n);
...
struct hex_impl<N, T, std::enable_if_t<!std::is_pointer<T>::value>> {
    constexpr explicit hex_impl(T const &v) : data_(v) {}

    T const &data_;

    friend auto operator<<(std::ostream &os, hex_impl<N, T> const &d) -> std::ostream & {
...
template <typename Int>
EINSUMS_EXPORT void print_ptr(std::ostream &os, Int v, int n);
...
constexpr auto hex(T const &v) -> hex_impl<N, T> {
...
struct bin_impl {
    constexpr explicit bin_impl(T const &v) : data_(v) {}

    T const &data_;

    friend auto operator<<(std::ostream &os, bin_impl<N, T> const &d) -> std::ostream & {
...
constexpr auto bin(T const &v) -> bin_impl<N, T> {
...
// ------------------------------------------------------------------
EINSUMS_EXPORT void print_str(std::ostream &os, char const *v, int n);
...
struct str {
    ...
    friend auto operator<<(std::ostream &os, str<N> const &d) -> std::ostream & {
...
// ------------------------------------------------------------------
struct ipaddr {
    EINSUMS_EXPORT ipaddr(void const *a);
    EINSUMS_EXPORT ipaddr(std::uint32_t a);

    std::uint8_t const *data_;
    std::uint32_t const ipdata_;

    EINSUMS_EXPORT friend auto operator<<(std::ostream &os, ipaddr const &p) -> std::ostream &;
};
...
struct current_time_print_helper {
    EINSUMS_EXPORT friend auto operator<<(std::ostream &os, current_time_print_helper const &) -> std::ostream &;
};
...
// ------------------------------------------------------------------
struct mem_crc32 {
    EINSUMS_EXPORT mem_crc32(void const *a, std::size_t len);

    std::uint64_t const *addr_;
    std::size_t const    len_;

    EINSUMS_EXPORT friend auto operator<<(std::ostream &os, mem_crc32 const &p) -> std::ostream &;
};
...
struct hostname_print_helper {
    EINSUMS_EXPORT auto get_hostname() const -> char const *;
    EINSUMS_EXPORT auto guess_rank() const -> int;

    EINSUMS_EXPORT friend auto operator<<(std::ostream &os, hostname_print_helper const &h) -> std::ostream &;
};

///////////////////////////////////////////////////////////////////////
EINSUMS_EXPORT void register_print_info(void (*)(std::ostream &));
EINSUMS_EXPORT void generate_prefix(std::ostream &os);
...
struct empty_timed_var {
    [[nodiscard]] constexpr auto trigger() const -> bool { return false; }

    [[nodiscard]] constexpr auto elapsed() const -> double { return 0; }
};
...
struct timed_var {
    ...
    timed_var(double const &delay, Args const &...args)
        : time_start_(std::chrono::steady_clock::now()), time_check_(time_start_), delay_(delay), message_(args...) {}

    auto trigger() const -> bool {
        auto   now      = std::chrono::steady_clock::now();
        double elapsed_ = std::chrono::duration_cast<std::chrono::duration<double>>(now - time_check_).count();
    ...
    auto elapsed() const -> double {
        return std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() - time_start_)
            .count();
    }

    friend auto operator<<(std::ostream &os, timed_var<Args...> const &ti) -> std::ostream & {
...
struct enable_print<false> {
    ...
    [[nodiscard]] constexpr auto is_enabled() const -> bool { return false; }
    ...
    constexpr auto declare_variable(Args const &...) const -> bool {
        return true;
    }
    ...
    template <typename... Args>
    constexpr auto make_timer(double const, Args const &...) const -> empty_timed_var {
        return empty_timed_var{};
    }

    template <typename Expr>
    constexpr auto eval(Expr const &) -> bool {
        return true;
    }
};
...
struct enable_print<true> {
  ...
  public:
    constexpr enable_print() : _prefix("") {}

    constexpr enable_print(char const *p) : _prefix(p) {}

    [[nodiscard]] constexpr auto is_enabled() const -> bool { return true; }

    template <typename... Args>
    constexpr void debug(Args const &...args) const {
        debug_impl(_prefix, args...);
    }

    template <typename... Args>
    constexpr void warning(Args const &...args) const {
        warning_impl(_prefix, args...);
    }

    template <typename... Args>
    constexpr void trace(Args const &...args) const {
        trace_impl(_prefix, args...);
    }

    template <typename... Args>
    constexpr void error(Args const &...args) const {
        error_impl(_prefix, args...);
    }

    template <typename... Args>
    auto scope(Args const &...args) -> scoped_var<Args...> {
        return scoped_var<Args...>(_prefix, args...);
    }

    template <typename... T, typename... Args>
    void timed(timed_var<T...> const &init, Args const &...args) const {
        if (init.trigger()) {
            timed_impl(_prefix, init, args...);
        }
    }
    ...
    template <typename T, typename... Args>
    auto declare_variable(Args const &...args) const -> T {
        return T(args...);
    }
    ...
    template <typename... Args>
    constexpr auto make_timer(double const delay, const Args... args) const -> timed_var<Args...> {
