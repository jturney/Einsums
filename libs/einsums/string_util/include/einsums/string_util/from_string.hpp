...
auto check_out_of_range(U const &value) -> T {
...
auto from_string(std::basic_string<Char> const &v) -> T {
    T target;
    try {
...
auto from_string(std::basic_string<Char> const &v, U &&default_value) -> T {
    T target;
    try {
        from_string_impl<T>::call(v, target);
        return target;
    } catch (...) {
        return EINSUMS_FORWARD(U, default_value);
    }
}

template <typename T>
auto from_string(std::string const &v) -> T {
    T target;
    try {
        from_string_impl<T>::call(v, target);
    } catch (...) {
...
template <typename T, typename U>
auto from_string(std::string const &v, U &&default_value) -> T {
    T target;
    try {
        from_string_impl<T>::call(v, target);
        return target;
