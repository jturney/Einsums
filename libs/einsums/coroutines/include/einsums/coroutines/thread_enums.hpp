 ...
 *
 * The \a thread_schedule_state enuerator encodes the current state of a
 * \a thread instance.
 */
enum class thread_schedule_state : std::int8_t {
    ...
    suspended = 3,               /*!< thread has been suspended (waiting for synchronization event, but
                                  *   still known and under control of the thread-manager) */
    terminated = 4,              /*!< thread has been stopped and may be garbage collected */
    staged = 5,                  /*!< this is not a real thread state, but allows to reference staged task
                                  * descriptions, which eventually will be converted into thread objects */
    pending_do_not_schedule = 6, /*!< this is not a real thread state, but allos to create a thread in
                                  * pending state without scheduling it (internal, do not use) */
    pending_boost = 7            /*!< this is not a real thread state, but allows us to suspend a thread
                                  * in a pending state without high priority rescheduling */
};
// clang-format on

EINSUMS_EXPORT char const *get_thread_state_name(thread_schedule_state state);
...
EINSUMS_EXPORT char const *get_thread_state_ex_name(thread_restart_state state);

/// \cond NOINTERNAL
// special type storing both state in one tagged structure
using thread_state = threads::detail::combined_tagged_state<thread_schedule_state, thread_restart_state>;
/// \endcond

/// Get the readable string representing the name of the given
/// thread_state constant.
EINSUMS_EXPORT char const *get_thread_state_name(thread_state state);

} // namespace einsums::threads::detail

namespace einsums::execution {
// clang-format off
    /// This enumeration lists all possible thread-priorities for einsums threads.
...
EINSUMS_EXPORT char const *get_stack_size_enum_name(thread_stacksize size);
} // namespace detail
...
struct thread_schedule_hint {
    ...
    constexpr thread_schedule_hint() noexcept : hint(-1), mode(thread_schedule_hint_mode::none) {}
    ...
    constexpr explicit thread_schedule_hint(std::int16_t thread_hint) noexcept
        : hint(thread_hint), mode(thread_schedule_hint_mode::thread) {}
    ...
    constexpr thread_schedule_hint(thread_schedule_hint_mode mode, std::int16_t hint) noexcept : hint(hint), mode(mode) {}

    /// \cond NOINTERNAL
    bool operator==(thread_schedule_hint const &rhs) const noexcept { return mode == rhs.mode && hint == rhs.hint; }
...
template <>
struct fmt::formatter<einsums::threads::detail::thread_schedule_state> : fmt::formatter<char const *> {
    template <typename FormatContext>
    auto format(einsums::threads::detail::thread_schedule_state const &t, FormatContext &ctx) const {
        return fmt::formatter<char const *>::format(einsums::threads::detail::get_thread_state_name(t), ctx);
    }
};

template <>
struct fmt::formatter<einsums::threads::detail::thread_restart_state> : fmt::formatter<char const *> {
    template <typename FormatContext>
    auto format(einsums::threads::detail::thread_restart_state const &t, FormatContext &ctx) const {
        return fmt::formatter<char const *>::format(einsums::threads::detail::get_thread_state_ex_name(t), ctx);
    }
};

template <>
struct fmt::formatter<einsums::execution::thread_priority> : fmt::formatter<char const *> {
    template <typename FormatContext>
    auto format(einsums::execution::thread_priority const &t, FormatContext &ctx) const {
        return fmt::formatter<char const *>::format(einsums::execution::detail::get_thread_priority_name(t), ctx);
    }
};

template <>
struct fmt::formatter<einsums::execution::thread_stacksize> : fmt::formatter<char const *> {
    template <typename FormatContext>
    auto format(einsums::execution::thread_stacksize const &t, FormatContext &ctx) const {
        return fmt::formatter<char const *>::format(einsums::execution::detail::get_stack_size_enum_name(t), ctx);
    }
};
