...
#include <einsums/coroutines/detail/swap_context.hpp> //for swap hints
...
class context_base : public default_context_impl<CoroutineImpl> {
  ...
  public:
    ...
    context_base(std::ptrdiff_t stack_size, thread_id_type id)
        : base_type(stack_size), _caller(), _state(ctx_ready), _exit_state(ctx_exit_not_requested),
    ...
    auto get_thread_id() const -> thread_id_type { return _thread_id; }
  ...
  protected:
    ...
    void do_return(context_exit_status status, std::exception_ptr &&info) noexcept {
        ...
        EINSUMS_ASSERT(m_state == ctx_running);
        _type_info   = EINSUMS_MOVE(info);
        _state       = ctx_exited;
        _exit_status = status;
#if defined(EINSUMS_HAVE_ADDRESS_SANITIZER)
        this->start_yield_fiber(&this->asan_fake_stack, _caller);
