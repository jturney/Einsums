...
#        define EINSUMS_COROUTINE_CREATE_CONTEXT(ctx)                                                                  \
            einsums::threads::coroutines::detail::posix::pth::check_(pth_uctx_create(&(ctx)))
...
#        define EINSUMS_COROUTINE_MAKE_CONTEXT(ctx, stack, size, startfunc, startarg, exitto)                          \
            einsums::threads::coroutines::detail::posix::ucontext::make_context(ctx, stack, size, startfunc, startarg, \
                                                                                exitto)
...
class ucontext_context_impl_base : detail::context_impl_base {
  public:
    ...
    ucontext_context_impl_base() { EINSUMS_COROUTINE_CREATE_CONTEXT(m_ctx); }
    ~ucontext_context_impl_base() { EINSUMS_COROUTINE_DESTROY_CONTEXT(m_ctx); }
  ...
  private:
     ...
     */
    friend void swap_context(ucontext_context_impl_base &from, ucontext_context_impl_base const &to, default_hint) {
        [[maybe_unused]] int error = EINSUMS_COROUTINE_SWAP_CONTEXT(&from.m_ctx, &to.m_ctx);
        EINSUMS_ASSERT(error == 0);
    }

  protected:
    EINSUMS_COROUTINE_DECLARE_CONTEXT(m_ctx);
};
...
class ucontext_context_impl : public ucontext_context_impl_base {
  ...
  public:
    ...
    explicit ucontext_context_impl(std::ptrdiff_t stack_size = -1)
        : _stack_size(stack_size == -1 ? this->default_stack_size : stack_size), _stack(nullptr),
          _funp(&trampoline<CoroutineImpl>) {}
    ...
    void init() {
        ...
        [[maybe_unused]] int error = EINSUMS_COROUTINE_MAKE_CONTEXT(&m_ctx, _stack, _stack_size, _funp, this, nullptr);
...
#    endif
            [[maybe_unused]] int error =
                EINSUMS_COROUTINE_MAKE_CONTEXT(&m_ctx, _stack, _stack_size, _funp, this, nullptr);
            EINSUMS_ASSERT(error == 0);
        }
    }
