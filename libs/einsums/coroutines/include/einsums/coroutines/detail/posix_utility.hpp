    ...
    if (use_guard_pages) {
        ...
        if (r != 0) {
            std::string error_message =
                "mprotect on a stack allocation failed with errno " + std::to_string(errno) + " (" + std::strerror(errno) + ")";
    ...
    if (real_stack == MAP_FAILED) {
        if (ENOMEM == errno && use_guard_pages) {
            char const *error_message = "mmap failed to allocate thread stack due to insufficient resources. "
                                        ...
                                        "configuration option einsums.stacks.use_guard_pages=0 may reduce memory "
                                        "consumption.";
            throw std::runtime_error(error_message);
        }

        std::string error_message =
            "mmap failed to allocate thread stack with errno " + std::to_string(errno) + " (" + std::strerror(errno) + ")";
        throw std::runtime_error(error_message);
    }
...
inline bool reset_stack(void *stack, std::size_t size) {
    ...
    if ((reinterpret_cast<void *>(0xDEAD'BEEF'DEAD'BEEFull)) != *watermark) {
        ...
        if (r != 0) {
            std::string error_message =
                "madvise on a stack allocation failed with errno " + std::to_string(errno) + " (" + std::strerror(errno) + ")";
            throw std::runtime_error(error_message);
        }
        return true;
    }
...
inline void free_stack(void *stack, std::size_t size) {
    ...
    if (r != 0) {
        std::string error_message =
            "munmap failed to deallocate thread stack with errno " + std::to_string(errno) + " (" + std::strerror(errno) + ")";
        throw std::runtime_error(error_message);
    }
}
