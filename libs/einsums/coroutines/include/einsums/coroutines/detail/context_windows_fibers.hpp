...
class fibers_context_impl_base : detail::context_impl_base {
  public:
     ...
     * default.
     */
    friend void swap_context(fibers_context_impl_base &from, const fibers_context_impl_base &to, default_hint) noexcept {
        if (!is_fiber()) {
...
class fibers_context_impl : public fibers_context_impl_base {
  ...
  public:
     ...
     */
    explicit fibers_context_impl(std::ptrdiff_t stacksize) : stacksize_(stacksize == -1 ? std::ptrdiff_t(default_stack_size) : stacksize) {}
