...
class x86_linux_context_impl : public x86_linux_context_impl_base {
  public:
    ...
    explicit x86_linux_context_impl(std::ptrdiff_t stack_size = -1)
        : m_stack_size(stack_size == -1 ? static_cast<std::ptrdiff_t>(default_stack_size) : stack_size), m_stack(nullptr) {}
    ...
    void init() {
        ...
        if (0 != (m_stack_size % EXEC_PAGESIZE)) {
            throw std::runtime_error(fmt::format("stack size of {} is not page aligned, page size is {}", m_stack_size, EXEC_PAGESIZE));
        }

        if (0 >= m_stack_size) {
            throw std::runtime_error(fmt::format("stack size of {} is invalid", m_stack_size));
        }
        ...
        m_sp = (static_cast<void **>(m_stack) + static_cast<std::size_t>(m_stack_size) / sizeof(void *)) - context_size;
...
#    endif
    ...
    std::ptrdiff_t get_available_stack_space() { return get_stack_ptr() - reinterpret_cast<std::size_t>(m_stack) - context_size; }
