...
class stackless_coroutine {
  ...
  public:
    ...
    stackless_coroutine(functor_type &&f, thread_id_type id, std::ptrdiff_t /*stack_size*/ = default_stack_size)
        : _f(std::move(f)), _state(ctx_ready), _id(id)
    ...
    thread_id_type get_thread_id() const { return _id; }
  ...
  public:
    ...
    std::ptrdiff_t get_available_stack_space() { return (std::numeric_limits<std::ptrdiff_t>::max)(); }
