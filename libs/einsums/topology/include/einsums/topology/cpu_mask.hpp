...
#if defined(EINSUMS_HAVE_MORE_THAN_64_THREADS) || EINSUMS_MAX_CPU_COUNT > 64
#    include <bitset>
#endif
...
using mask_type      = std::bitset<EINSUMS_MAX_CPU_COUNT>;
using mask_cref_type = std::bitset<EINSUMS_MAX_CPU_COUNT> const &;
...
inline std::size_t find_first(mask_cref_type mask) {
    if (mask.any()) {
        for (std::size_t i = 0; i != EINSUMS_MAX_CPU_COUNT; ++i) {
            if (mask[i])
                return i;
        }
    }
...
struct from_string_impl<einsums::threads::detail::mask_type, void> {
    ...
    static void call(std::basic_string<Char> const &value, einsums::threads::detail::mask_type &target) {
        ...
        string_util::trim(value_trimmed);
        ...
        constexpr auto const to_mask = [](unsigned char const c) {
            ...
            throw std::out_of_range(fmt::format("from_string<mask_type>: got invalid hexadecimal character (\"{}\")", c));
        };
