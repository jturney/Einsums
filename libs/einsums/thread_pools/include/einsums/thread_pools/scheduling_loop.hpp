...
#    include <einsums/threading_base/detail/tracy.hpp>
...
///////////////////////////////////////////////////////////////////////
inline void write_state_log(scheduler_base const &scheduler, std::size_t num_thread, thread_id_ref_type const &thrd,
                            thread_schedule_state const old_state, thread_schedule_state const new_state) {
    EINSUMS_LOG(debug,
                "scheduling_loop state change: pool({}), scheduler({}), worker_thread({}), thread({}), "
                "description({}), old state({}), new state({})",
                *scheduler.get_parent_pool(), scheduler, num_thread, thrd, get_thread_id_data(thrd)->get_description(),
                get_thread_state_name(old_state), get_thread_state_name(new_state));
}

inline void write_state_log_warning(scheduler_base const &scheduler, std::size_t num_thread, thread_id_ref_type const &thrd,
...
class switch_status {
  public:
    switch_status(thread_id_ref_type const &t, thread_state prev_state)
        : thread_(t), prev_state_(prev_state), next_thread_id_(nullptr),
          need_restore_state_(get_thread_id_data(thread_)->set_state_tagged(thread_schedule_state::active, prev_state_, orig_state_)) {}
    ...
    thread_state operator=(thread_result_type &&new_state) {
        prev_state_     = thread_state(new_state.first, prev_state_.state_ex(), prev_state_.tag() + 1);
    ...
    // einsums-thread in the meantime.
...
struct idle_collect_rate {
    idle_collect_rate(std::int64_t &tfunc_time, std::int64_t &exec_time)
        : start_timestamp_(einsums::chrono::detail::timestamp()), tfunc_time_(tfunc_time), exec_time_(exec_time) {}

    void collect_exec_time(std::int64_t timestamp) { exec_time_ += einsums::chrono::detail::timestamp() - timestamp; }
    void take_snapshot() {
        if (tfunc_time_ == std::int64_t(-1)) {
            start_timestamp_ = einsums::chrono::detail::timestamp();
            tfunc_time_      = 0;
            exec_time_       = 0;
        } else {
            tfunc_time_ = einsums::chrono::detail::timestamp() - start_timestamp_;
        }
    }
...
struct exec_time_wrapper {
     exec_time_wrapper(idle_collect_rate &idle_rate) : timestamp_(einsums::chrono::detail::timestamp()), idle_rate_(idle_rate) {}
...
struct tfunc_time_wrapper {
     tfunc_time_wrapper(idle_collect_rate &idle_rate) : idle_rate_(idle_rate) {}
...
struct is_active_wrapper {
     is_active_wrapper(bool &is_active) : is_active_(is_active) { is_active = true; }
...
struct scheduling_counters {
    ...
    scheduling_counters(std::int64_t &executed_threads, std::int64_t &executed_thread_phases, std::int64_t &tfunc_time,
                        std::int64_t &exec_time, std::int64_t &idle_loop_count, std::int64_t &busy_loop_count, bool &is_active)
        // NOLINTEND(bugprone-easily-swappable-parameters)
        : executed_threads_(executed_threads), executed_thread_phases_(executed_thread_phases), tfunc_time_(tfunc_time),
          exec_time_(exec_time), idle_loop_count_(idle_loop_count), busy_loop_count_(busy_loop_count), is_active_(is_active) {}
...
struct scheduling_callbacks {
    ...
    // NOLINTBEGIN(bugprone-easily-swappable-parameters)
    explicit scheduling_callbacks(callback_type &&outer, callback_type &&inner = callback_type(),
                                  std::int64_t max_idle_loop_count = EINSUMS_IDLE_LOOP_COUNT_MAX,
                                  std::int64_t max_busy_loop_count = EINSUMS_BUSY_LOOP_COUNT_MAX)
        // NOLINTEND(bugprone-easily-swappable-parameters)
        : outer_(std::move(outer)), inner_(std::move(inner)), max_idle_loop_count_(max_idle_loop_count),
...
void scheduling_loop(std::size_t num_thread, SchedulingPolicy &scheduler, scheduling_counters &counters, scheduling_callbacks &params) {
    std::atomic<einsums::runtime_state> &this_state = scheduler.get_state(num_thread);
    ...
    einsums::execution::this_thread::detail::agent_storage *context_storage = einsums::execution::this_thread::detail::get_agent_storage();
    ...
    while (true) {
        ...
        // Get the next einsums thread from the queue
        ...
        bool enable_stealing = scheduler.SchedulingPolicy::has_scheduler_mode(::einsums::threads::scheduler_mode::enable_stealing);

        // stealing staged threads is enabled only after normal stealing has
        // failed for a while
        bool enable_stealing_staged = enable_stealing && idle_loop_count > params.max_idle_loop_count_ / 2;

        if (EINSUMS_LIKELY(thrd || scheduler.SchedulingPolicy::get_next_thread(num_thread, running, thrd, enable_stealing))) {
            tfunc_time_wrapper tfunc_time_collector(idle_rate);
            EINSUMS_ASSERT(get_thread_id_data(thrd)->get_scheduler_base() == &scheduler);
            ...
            // Only pending einsums threads will be executed.
            // Any non-pending einsums threads are leftovers from a set_state()
            // call for a previously pending einsums thread (see comments above).
            thread_state          state     = get_thread_id_data(thrd)->get_state();
            thread_schedule_state state_val = state.state();

            if (EINSUMS_LIKELY(thread_schedule_state::pending == state_val)) {
...

                {
                    ...
                    if (EINSUMS_LIKELY(thrd_stat.is_valid() && thrd_stat.get_previous() == thread_schedule_state::pending)) {
                        write_state_log(scheduler, num_thread, thrd, thrd_stat.get_previous(), thread_schedule_state::active);
                        ...
                        // store the returned state in the thread
                        {
                            ...
                            util::itt::task task = thrdptr->get_description().get_task_itt(thread_domain);
                            ...
                            einsums::detail::external_timer::scoped_timer profiler(thrdptr->get_timer_data());
...
#    if defined(EINSUMS_HAVE_TRACY)
                            ...
                            task_annotation << "einsums/task:" << thrd << "/"
                                            << (desc.kind() == einsums::detail::thread_description::data_type::data_type_description
                                                    ? desc.get_description()
                                                    : "<unknown>");
                            auto task_annotation_str = EINSUMS_MOVE(task_annotation).str();
...
#    endif
                        ...
                        write_state_log(scheduler, num_thread, thrd, thread_schedule_state::active, thrd_stat.get_previous());
                        ...
                        // executing this einsums-thread, we just continue with
                        // the next one
                        thrd_stat.disable_restore();
                        write_state_log_warning(scheduler, num_thread, thrd, state_val, "no execution");
                        continue;
                    }

                    // store and retrieve the new state in the thread
                    if (EINSUMS_UNLIKELY(!thrd_stat.store_state(state))) {
                        // some other worker-thread got in between and changed
                        // the state of this thread, we just continue with
                        // the next one
                        write_state_log_warning(scheduler, num_thread, thrd, state_val, "no state change");
                        continue;
                    }
                ...
                // Re-add this work item to our list of work items if the einsums
                // thread should be re-scheduled. If the einsums thread is suspended
                // now we just keep it in the map of threads.
                if (EINSUMS_UNLIKELY(state_val == thread_schedule_state::pending)) {
                    if (EINSUMS_LIKELY(next_thrd == nullptr)) {
                        // schedule other work
                        scheduler.SchedulingPolicy::wait_or_add_new(num_thread, running, idle_loop_count, enable_stealing_staged, added);
                    }

                    // schedule this thread again, make sure it ends up at
                    // the end of the queue
                    scheduler.SchedulingPolicy::schedule_thread_last(
                        std::move(thrd), execution::thread_schedule_hint(static_cast<std::int16_t>(num_thread)), true);
                    scheduler.SchedulingPolicy::do_some_work(num_thread);
                } else if (EINSUMS_UNLIKELY(state_val == thread_schedule_state::pending_boost)) {
                    ...
                    if (EINSUMS_LIKELY(next_thrd == nullptr)) {
                        ...
                        if (EINSUMS_UNLIKELY(busy_loop_count > params.max_busy_loop_count_)) {
                            next_thrd = std::move(thrd);
                        } else {
                            // schedule other work
                            scheduler.SchedulingPolicy::wait_or_add_new(num_thread, running, idle_loop_count, enable_stealing_staged,
                                                                        added);
                    ...
                    } else if (EINSUMS_LIKELY(next_thrd != thrd)) {
                        // schedule this thread again immediately with
                        // boosted priority
            ...
            } else if (EINSUMS_UNLIKELY(thread_schedule_state::active == state_val)) {
                auto *thrdptr = get_thread_id_data(thrd);
                EINSUMS_LOG(warn,
                            "pool({}), scheduler({}), worker_thread({}), thread({}), "
                            "description({}), rescheduling",
                            *scheduler.get_parent_pool(), scheduler, num_thread, thrdptr->get_thread_id(), thrdptr->get_description());
                ...
                scheduler.SchedulingPolicy::schedule_thread(
                    std::move(thrd), execution::thread_schedule_hint(static_cast<std::int16_t>(num_thread)), true, priority);
                scheduler.SchedulingPolicy::do_some_work(num_thread);
            }

            // Remove the mapping from thread_map_ if einsums thread is terminated, this will
            // delete the einsums thread.
            if (EINSUMS_LIKELY(state_val == thread_schedule_state::terminated)) {
#ifdef EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS
        ...
        // if nothing else has to be done either wait or terminate
            ...
            if (scheduler.SchedulingPolicy::wait_or_add_new(num_thread, running, idle_loop_count, enable_stealing_staged, added)) {
                // Clean up terminated threads before trying to exit
                bool can_exit = !running && scheduler.SchedulingPolicy::cleanup_terminated(num_thread, true) &&
                ...
                if (this_state.load() == runtime_state::pre_sleep) {
                    ...
                    can_exit = can_exit && scheduler.SchedulingPolicy::get_thread_count(
                                               thread_schedule_state::suspended, execution::thread_priority::default_, num_thread) == 0;
            ...
            if (!params.inner_.empty()) {
                params.inner_();
                context_storage = einsums::execution::this_thread::detail::get_agent_storage();
            }
        }

        if (scheduler.custom_polling_function() == einsums::threads::detail::polling_status::busy) {
            idle_loop_count = 0;
        }

        // something went badly wrong, give up
        if (EINSUMS_UNLIKELY(this_state.load() == runtime_state::terminating))
            break;
        ...
        if (busy_loop_count > params.max_busy_loop_count_) {
            ...
            if (may_exit) {
                EINSUMS_ASSERT(this_state.load() != runtime_state::pre_sleep);

                {
                    bool can_exit = !running && scheduler.SchedulingPolicy::cleanup_terminated(true) &&
                                    scheduler.SchedulingPolicy::get_thread_count(thread_schedule_state::suspended,
