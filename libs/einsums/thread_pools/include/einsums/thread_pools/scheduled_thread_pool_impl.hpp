...
#include <einsums/thread_pools/scheduling_loop.hpp>
...
#include <algorithm>
#include <atomic>
#ifdef EINSUMS_HAVE_MAX_CPU_COUNT
#    include <bitset>
#endif
#include <cstddef>
#include <cstdint>
...
struct manage_active_thread_count {
     manage_active_thread_count(std::atomic<long> &counter) : counter_(counter) {}
...
struct init_tss_helper {
    ...
    init_tss_helper(scheduled_thread_pool<Scheduler> &pool, std::size_t local_thread_num, std::size_t global_thread_num)
        // NOLINTEND(bugprone-easily-swappable-parameters)
        : pool_(pool), local_thread_num_(local_thread_num), global_thread_num_(global_thread_num) {
        pool.notifier_.on_start_thread(local_thread_num_, global_thread_num_, pool_.get_pool_id().name().c_str(), "");
        pool.sched_->Scheduler::on_start_thread(local_thread_num_);
    }
    ~init_tss_helper() {
        pool_.sched_->Scheduler::on_stop_thread(local_thread_num_);
        pool_.notifier_.on_stop_thread(local_thread_num_, global_thread_num_, pool_.get_pool_id().name().c_str(), "");
    }
...
///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
scheduled_thread_pool<Scheduler>::scheduled_thread_pool(std::unique_ptr<Scheduler> sched, thread_pool_init_parameters const &init)
    : thread_pool_base(init), sched_(std::move(sched)), thread_count_(0), max_idle_loop_count_(init.max_idle_loop_count_),
      max_busy_loop_count_(init.max_busy_loop_count_), shutdown_check_count_(init.shutdown_check_count_) {
...
void scheduled_thread_pool<Scheduler>::print_pool(std::ostream &os) {
    os << "[pool \"" << id_.name() << "\", #" << id_.index() //-V128
       ...
       << "is running on PUs : \n";
    os << einsums::threads::detail::to_string(get_used_processing_units())
#ifdef EINSUMS_HAVE_MAX_CPU_COUNT
       << " " << std::bitset<EINSUMS_HAVE_MAX_CPU_COUNT>(get_used_processing_units())
#endif
       << '\n';
...
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::report_error(std::size_t global_thread_num, std::exception_ptr const &e) {
...
///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
einsums::runtime_state scheduled_thread_pool<Scheduler>::get_state() const {
    ...
    if (thread_count_ != 0) {
        ...
        // up.
        if (num_thread != std::size_t(-1) && num_thread < static_cast<std::size_t>(thread_count_))
...
template <typename Scheduler>
einsums::runtime_state scheduled_thread_pool<Scheduler>::get_state(std::size_t num_thread) const {
...
bool scheduled_thread_pool<Scheduler>::is_busy() {
    // If we are currently on a einsums thread, which runs on the current pool,
    ...
    std::int64_t einsums_thread_offset = (threads::detail::get_self_ptr() && this_thread::get_pool() == this) ? 1 : 0;
    bool         have_einsums_threads  = get_thread_count_unknown(std::size_t(-1), false) > einsums_thread_offset;
    bool         have_polling_work     = sched_->Scheduler::get_polling_work_count() > 0;

    return have_einsums_threads || have_polling_work;
}
...
void scheduled_thread_pool<Scheduler>::wait() {
    einsums::util::detail::yield_while_count([this]() { return is_busy(); }, shutdown_check_count_);
}

template <typename Scheduler>
template <typename Lock>
void scheduled_thread_pool<Scheduler>::stop_locked(Lock &l, bool blocking) {
    EINSUMS_LOG(info, "stop: {} blocking({})", id_.name(), blocking);
    ...
    if (!threads_.empty()) {
        ...
        if (blocking) {
            for (std::size_t i = 0; i != threads_.size(); ++i) {
                ...
                EINSUMS_LOG(info, "stop: {} notify_all", id_.name());

                sched_->Scheduler::do_some_work(std::size_t(-1));

                EINSUMS_LOG(info, "stop: {} join:{}", id_.name(), i);

                {
                    // unlock the lock while joining
                    ::einsums::detail::unlock_guard<Lock> ul(l);
...
template <typename Scheduler>
bool einsums::threads::detail::scheduled_thread_pool<Scheduler>::run(std::unique_lock<std::mutex> &l, std::size_t pool_threads) {
    EINSUMS_ASSERT(l.owns_lock());

    EINSUMS_LOG(info, "run: {} number of processing units available: {}", id_.name(), threads::detail::hardware_concurrency());
    EINSUMS_LOG(info, "run: {} creating {} OS thread(s)", id_.name(), pool_threads);

    if (0 == pool_threads) {
        EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "run", "number of threads is zero");
    }
    ...
    EINSUMS_LOG(info, "run: {} timestamp_scale: {}", id_.name(), timestamp_scale_);

    // run threads and wait for initialization to complete
    std::size_t                                            thread_num = 0;
    std::shared_ptr<einsums::concurrency::detail::barrier> startup =
        std::make_shared<einsums::concurrency::detail::barrier>(pool_threads + 1);
    try {
        ...
        for (/**/; thread_num != pool_threads; ++thread_num) {
            ...
            EINSUMS_LOG(info,
                        ...
                        "within this mask: {}",
                        id_.name(), global_thread_num, einsums::threads::detail::to_string(mask));
        ...
        EINSUMS_ASSERT(pool_threads == std::size_t(thread_count_.load()));
    } catch (std::exception const &e) {
        EINSUMS_LOG(critical, "run: {} failed with: {}", id_.name(), e.what());
    ...
    EINSUMS_LOG(info, "run: {} running", id_.name());
    return true;
}
...
void scheduled_thread_pool<Scheduler>::suspend_internal(error_code &ec) {
    util::yield_while([this]() { return this->sched_->Scheduler::get_thread_count() > 0; }, "scheduled_thread_pool::suspend_internal");

    for (std::size_t i = 0; i != threads_.size(); ++i) {
        einsums::runtime_state expected = runtime_state::running;
        sched_->Scheduler::get_state(i).compare_exchange_strong(expected, runtime_state::pre_sleep);
    }
...
void scheduled_thread_pool<Scheduler>::suspend_direct(error_code &ec) {
    if (threads::detail::get_self_ptr() && einsums::this_thread::get_pool() == this) {
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "scheduled_thread_pool<Scheduler>::suspend_direct",
...
template <typename Scheduler>
void einsums::threads::detail::scheduled_thread_pool<Scheduler>::thread_func(
    std::size_t thread_num, std::size_t global_thread_num, std::shared_ptr<einsums::concurrency::detail::barrier> startup) {
    topology const &topo = get_topology();
    ...
    if (!any(mask)) {
        mask = topo.get_machine_affinity_mask();
    }

    if (EINSUMS_LOG_ENABLED(debug))
        topo.write_to_log();

    error_code ec(throw_mode::lightweight);
    topo.set_thread_affinity_mask(mask, ec);
    if (ec) {
        EINSUMS_LOG(warn, "thread_func: {} setting thread affinity on OS thread {} failed with: {}", id_.name(), global_thread_num,
                    ec.get_message());
    }
    ...
    if (get_scheduler()->has_scheduler_mode(scheduler_mode::reduce_thread_priority)) {
        ...
        if (ec) {
            EINSUMS_LOG(warn, "thread_func: {} reducing thread priority on OS thread {} failed with: {}", id_.name(), global_thread_num,
                        ec.get_message());
        }
    }
    ...
    std::atomic<einsums::runtime_state>    &state    = sched_->Scheduler::get_state(thread_num);
    [[maybe_unused]] einsums::runtime_state oldstate = state.exchange(runtime_state::running);
    EINSUMS_ASSERT(oldstate <= runtime_state::running);

    // wait for all threads to start up before before starting einsums work
    startup->wait();

    EINSUMS_LOG(info, "thread_func: {} starting OS thread: {}", id_.name(), thread_num);
    ...
    try {
        try {
            ...
            // run the work queue
            [[maybe_unused]] einsums::threads::coroutines::detail::prepare_main_thread main_thread;

            // run main Scheduler loop until terminated
            scheduling_counter_data &counter_data = counter_data_[thread_num].data_;

            scheduling_counters counters(counter_data.executed_threads_, counter_data.executed_thread_phases_, counter_data.tfunc_times_,
                                         counter_data.exec_times_, counter_data.idle_loop_counts_, counter_data.busy_loop_counts_,
                                         counter_data.tasks_active_);

            scheduling_callbacks callbacks(util::detail::deferred_call( //-V107
            ...
            // the OS thread is allowed to exit only if no more einsums
            // threads exist or if some other thread has terminated
            EINSUMS_ASSERT((sched_->Scheduler::get_thread_count(thread_schedule_state::suspended, execution::thread_priority::default_,
                                                                thread_num) == 0 &&
            ...
            EINSUMS_LOG(critical,
                        "thread_func: {} thread_num:{} : caught einsums::exception: {}, "
            ...
            einsums::throw_with_info(einsums::exception(einsums::error::unhandled_exception, e.what()));
        }
    } catch (...) {
        EINSUMS_LOG(critical,
    ...
    EINSUMS_LOG(info, "thread_func: {} thread_num: {}, ending OS thread, executed {} einsums threads", id_.name(), global_thread_num,
                counter_data_[global_thread_num].data_.executed_threads_);
}

///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::create_thread(thread_init_data &data, thread_id_ref_type &id, error_code &ec) {
    // verify state
    if (thread_count_ == 0 && !sched_->Scheduler::is_state(runtime_state::running)) {
        // thread-manager is not currently running
        EINSUMS_THROWS_IF(ec, einsums::error::invalid_status, "thread_pool<Scheduler>::create_thread",
...
template <typename Scheduler>
thread_id_ref_type scheduled_thread_pool<Scheduler>::create_work(thread_init_data &data, error_code &ec) {
    // verify state
    if (thread_count_ == 0 && !sched_->Scheduler::is_state(runtime_state::running)) {
        // thread-manager is not currently running
        EINSUMS_THROWS_IF(ec, einsums::error::invalid_status, "thread_pool<Scheduler>::create_work",
                          "invalid state: thread pool is not running");
...
///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
thread_state scheduled_thread_pool<Scheduler>::set_state(thread_id_type const &id, thread_schedule_state new_state,
                                                         thread_restart_state new_state_ex, execution::thread_priority priority,
                                                         error_code &ec) {
    return set_thread_state(id, new_state, //-V107
                            new_state_ex, priority, execution::thread_schedule_hint(static_cast<std::int16_t>(get_local_thread_num_tss())),
                            true, ec);
}
...
OutIter copy_projected(InIter first, InIter last, OutIter dest, ProjSrc &&srcproj, ProjDest &&destproj) {
    while (first != last) {
        EINSUMS_INVOKE(destproj, (dest++)->data_) = EINSUMS_INVOKE(srcproj, (first++)->data_);
    }
    return dest;
}
...
T accumulate_projected(InIter first, InIter last, T init, Proj &&proj) {
    while (first != last) {
        init = std::move(init) + EINSUMS_INVOKE(proj, (first++)->data_);
    }
    return init;
}

#if defined(EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS)
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_executed_threads(std::size_t num, bool reset) {
    ...
    if (num != std::size_t(-1)) {
        ...
        executed_threads =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::executed_threads_);
        reset_executed_threads = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                      &scheduling_counter_data::reset_executed_threads_);

        if (reset) {
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(), &scheduling_counter_data::executed_threads_,
                           &scheduling_counter_data::reset_executed_threads_);
        }
    }

    EINSUMS_ASSERT(executed_threads >= reset_executed_threads);
...
#if defined(EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS)
    std::int64_t reset_executed_threads = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                               &scheduling_counter_data::reset_executed_threads_);
...
#if defined(EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS)
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_executed_thread_phases(std::size_t num, bool reset) {
    ...
    if (num != std::size_t(-1)) {
        ...
        executed_phases       = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                     &scheduling_counter_data::executed_thread_phases_);
        reset_executed_phases = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
        ...
        if (reset) {
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(),
                           &scheduling_counter_data::executed_thread_phases_, &scheduling_counter_data::reset_executed_thread_phases_);
        }
    }

    EINSUMS_ASSERT(executed_phases >= reset_executed_phases);

    return executed_phases - reset_executed_phases;
}

#    if defined(EINSUMS_HAVE_THREAD_IDLE_RATES)
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_thread_phase_duration(std::size_t num, bool reset) {
    ...
    if (num != std::size_t(-1)) {
        ...
        exec_total =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::exec_times_);
        num_phases = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                          &scheduling_counter_data::executed_thread_phases_);

        reset_exec_total = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                &scheduling_counter_data::reset_thread_phase_duration_times_);
        reset_num_phases = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                &scheduling_counter_data::reset_thread_phase_duration_);

        if (reset) {
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(), &scheduling_counter_data::exec_times_,
                           &scheduling_counter_data::reset_thread_phase_duration_times_);
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(),
                           &scheduling_counter_data::executed_thread_phases_, &scheduling_counter_data::reset_thread_phase_duration_);
        }
    }

    EINSUMS_ASSERT(exec_total >= reset_exec_total);
    EINSUMS_ASSERT(num_phases >= reset_num_phases);
...
std::int64_t scheduled_thread_pool<Scheduler>::get_thread_duration(std::size_t num, bool reset) {
    ...
    if (num != std::size_t(-1)) {
        ...
        reset_num_threads = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
    ...
    EINSUMS_ASSERT(exec_total >= reset_exec_total);
    EINSUMS_ASSERT(num_threads >= reset_num_threads);
...
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_thread_phase_overhead(std::size_t num, bool reset) {
    std::int64_t exec_total  = 0;
    ...
    if (num != std::size_t(-1)) {
        ...
        exec_total =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::exec_times_);
        tfunc_total =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::tfunc_times_);
        num_phases = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                          &scheduling_counter_data::executed_thread_phases_);

        reset_exec_total  = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                 &scheduling_counter_data::reset_thread_phase_overhead_times_);
        reset_tfunc_total = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
        ...
        if (reset) {
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(), &scheduling_counter_data::exec_times_,
                           &scheduling_counter_data::reset_thread_phase_overhead_times_);
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(), &scheduling_counter_data::tfunc_times_,
                           &scheduling_counter_data::reset_thread_phase_overhead_times_total_);
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(),
                           &scheduling_counter_data::executed_thread_phases_, &scheduling_counter_data::reset_thread_phase_overhead_);
        }
    }

    EINSUMS_ASSERT(exec_total >= reset_exec_total);
    EINSUMS_ASSERT(tfunc_total >= reset_tfunc_total);
    EINSUMS_ASSERT(num_phases >= reset_num_phases);
    ...
    EINSUMS_ASSERT(tfunc_total >= exec_total);

    return std::int64_t(double((tfunc_total - exec_total) * timestamp_scale_) / double(num_phases));
}
...
std::int64_t scheduled_thread_pool<Scheduler>::get_thread_overhead(std::size_t num, bool reset) {
    ...
    if (num != std::size_t(-1)) {
        ...
        if (reset) {
            ...
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(), &scheduling_counter_data::tfunc_times_,
                           &scheduling_counter_data::reset_thread_overhead_times_total_);
            copy_projected(counter_data_.begin(), counter_data_.end(), counter_data_.begin(),
                           &scheduling_counter_data::executed_thread_phases_, &scheduling_counter_data::reset_thread_overhead_);
        }
    }
    ...
    return std::int64_t(double((tfunc_total - exec_total) * timestamp_scale_) / double(num_threads));
}

template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_cumulative_thread_duration(std::size_t num, bool reset) {
    std::int64_t exec_total       = 0;
    std::int64_t reset_exec_total = 0;
...
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_cumulative_thread_overhead(std::size_t num, bool reset) {
    std::int64_t exec_total        = 0;
    std::int64_t reset_exec_total  = 0;
    std::int64_t tfunc_total       = 0;
    std::int64_t reset_tfunc_total = 0;
...
#    endif // EINSUMS_HAVE_THREAD_IDLE_RATES
#endif     // EINSUMS_HAVE_THREAD_CUMULATIVE_COUNTS

///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_cumulative_duration(std::size_t num, bool reset) {
    std::int64_t tfunc_total       = 0;
    std::int64_t reset_tfunc_total = 0;
    ...
    if (num != std::size_t(-1)) {
        ...
        tfunc_total =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::tfunc_times_);
        reset_tfunc_total =
            accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::reset_tfunc_times_);
...
#if defined(EINSUMS_HAVE_THREAD_IDLE_RATES)
#    if defined(EINSUMS_HAVE_THREAD_CREATION_AND_CLEANUP_RATES)
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::avg_creation_idle_rate(std::size_t, bool reset) {
    double const creation_total = static_cast<double>(sched_->Scheduler::get_creation_time(reset));
    ...
    std::int64_t reset_exec_total  = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
                                                          &scheduling_counter_data::reset_creation_idle_rate_time_);
    std::int64_t reset_tfunc_total = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
...
std::int64_t scheduled_thread_pool<Scheduler>::avg_cleanup_idle_rate(std::size_t, bool reset) {
    double const cleanup_total = static_cast<double>(sched_->Scheduler::get_cleanup_time(reset));
...
#    endif // EINSUMS_HAVE_THREAD_CREATION_AND_CLEANUP_RATES
...
std::int64_t scheduled_thread_pool<Scheduler>::avg_idle_rate_all(bool reset) {
    ...
    std::int64_t reset_exec_total =
        accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::reset_idle_rate_time_);
    std::int64_t reset_tfunc_total = accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
...
std::int64_t scheduled_thread_pool<Scheduler>::avg_idle_rate(std::size_t num, bool reset) {
    ...
    EINSUMS_ASSERT(exec_time >= reset_exec_time);
    EINSUMS_ASSERT(tfunc_time >= reset_tfunc_time);
    ...
    EINSUMS_ASSERT(tfunc_time > exec_time);
...
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_idle_loop_count(std::size_t num, bool /* reset */) {
    if (num == std::size_t(-1)) {
...
template <typename Scheduler>
std::int64_t scheduled_thread_pool<Scheduler>::get_busy_loop_count(std::size_t num, bool /* reset */) {
    if (num == std::size_t(-1)) {
        return accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0),
...
std::int64_t scheduled_thread_pool<Scheduler>::get_scheduler_utilization() const {
    return (accumulate_projected(counter_data_.begin(), counter_data_.end(), std::int64_t(0), &scheduling_counter_data::tasks_active_) *
            100) /
           thread_count_.load();
}
...
///////////////////////////////////////////////////////////////////////////
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::add_processing_unit_internal(std::size_t virt_core, std::size_t thread_num,
                                                                    std::shared_ptr<einsums::concurrency::detail::barrier> startup,
                                                                    error_code                                            &ec) {
    std::unique_lock<typename Scheduler::pu_mutex_type> l(sched_->Scheduler::get_pu_mutex(virt_core));
    ...
    if (threads_[virt_core].joinable()) {
        l.unlock();
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "scheduled_thread_pool<Scheduler>::add_processing_unit",
                          "the given virtual core has already been added to this thread pool");
        return;
    }

    std::atomic<einsums::runtime_state>    &state    = sched_->Scheduler::get_state(virt_core);
    [[maybe_unused]] einsums::runtime_state oldstate = state.exchange(runtime_state::initialized);
    EINSUMS_ASSERT(oldstate == runtime_state::stopped || oldstate == runtime_state::initialized);

    threads_[virt_core] = std::thread(&scheduled_thread_pool::thread_func, this, virt_core, thread_num, std::move(startup));
...
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::remove_processing_unit_internal(std::size_t virt_core, error_code &ec) {
    std::unique_lock<typename Scheduler::pu_mutex_type> l(sched_->Scheduler::get_pu_mutex(virt_core));

    if (threads_.size() <= virt_core || !threads_[virt_core].joinable()) {
        l.unlock();
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "scheduled_thread_pool<Scheduler>::remove_processing_unit",
    ...
    einsums::runtime_state oldstate = state.exchange(runtime_state::stopping);
    ...
    EINSUMS_ASSERT(oldstate == runtime_state::starting || oldstate == runtime_state::running || oldstate == runtime_state::stopping ||
                   oldstate == runtime_state::stopped || oldstate == runtime_state::terminating);
    ...
    if (threads::detail::get_self_ptr() && this == einsums::this_thread::get_pool()) {
        std::size_t thread_num = thread_offset_ + virt_core;

        util::yield_while([thread_num]() { return thread_num == einsums::get_worker_thread_num(); },
...
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::suspend_processing_unit_internal(std::size_t virt_core, error_code &ec) {
    // Yield to other einsums threads if lock is not available to avoid
    // deadlocks when multiple einsums threads try to resume or suspend pus.
    std::unique_lock<typename Scheduler::pu_mutex_type> l(sched_->Scheduler::get_pu_mutex(virt_core), std::defer_lock);

    util::yield_while([&l]() { return !l.try_lock(); }, "scheduled_thread_pool::suspend_processing_unit_internal");

    if (threads_.size() <= virt_core || !threads_[virt_core].joinable()) {
        l.unlock();
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "scheduled_thread_pool<Scheduler>::suspend_processing_unit_internal",
                          "the given virtual core has already been stopped to run on this thread pool");
        return;
    }
    ...
    EINSUMS_ASSERT(expected == runtime_state::running || expected == runtime_state::pre_sleep || expected == runtime_state::sleeping);
...
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::suspend_processing_unit_direct(std::size_t virt_core, error_code &ec) {
    if (!get_scheduler()->has_scheduler_mode(scheduler_mode::enable_elasticity)) {
        EINSUMS_THROWS_IF(ec, einsums::error::invalid_status, "scheduled_thread_pool<Scheduler>::suspend_processing_unit_direct",
                          "this thread pool does not support suspending processing units");
    }

    if (threads::detail::get_self_ptr() && !get_scheduler()->has_scheduler_mode(scheduler_mode::enable_stealing) &&
        einsums::this_thread::get_pool() == this) {
        EINSUMS_THROWS_IF(ec, einsums::error::invalid_status, "scheduled_thread_pool<Scheduler>::suspend_processing_unit_direct",
...
template <typename Scheduler>
void scheduled_thread_pool<Scheduler>::resume_processing_unit_direct(std::size_t virt_core, error_code &ec) {
    // Yield to other einsums threads if lock is not available to avoid
    // deadlocks when multiple einsums threads try to resume or suspend pus.
    std::unique_lock<typename Scheduler::pu_mutex_type> l(sched_->Scheduler::get_pu_mutex(virt_core), std::defer_lock);
    util::yield_while([&l]() { return !l.try_lock(); }, "scheduled_thread_pool::resume_processing_unit_direct");

    if (threads_.size() <= virt_core || !threads_[virt_core].joinable()) {
        l.unlock();
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "scheduled_thread_pool<Scheduler>::resume_processing_unit",
                          "the given virtual core has already been stopped to run on this thread pool");
        return;
    }
