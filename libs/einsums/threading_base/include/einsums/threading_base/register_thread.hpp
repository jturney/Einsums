...
///                   einsums-thread interface, i.e. it takes one argument (a
...
inline thread_id_ref_type register_thread(thread_init_data &data, thread_pool_base *pool, error_code &ec = throws) {
    EINSUMS_ASSERT(pool);
    data.run_now          = true;
...
///                   parameter \a ec. Otherwise it throws an instance
///                   of einsums#exception.
inline thread_id_ref_type register_work(thread_init_data &data, thread_pool_base *pool, error_code &ec = throws) {
    EINSUMS_ASSERT(pool);
    data.run_now = false;
