...
/// \cond NOINTERNAL
struct pool_id_type {
    pool_id_type(std::size_t index, std::string const &name) : index_(index), name_(name) {}
...
struct thread_pool_init_parameters {
    ...
    einsums::threads::callback_notifier  &notifier_;
    ...
    thread_pool_init_parameters(std::string const &name, std::size_t index, scheduler_mode mode, std::size_t num_threads,
                                std::size_t thread_offset, einsums::threads::callback_notifier &notifier,
                                einsums::detail::affinity_data const &affinity_data,
                                std::size_t                           max_idle_loop_count = EINSUMS_IDLE_LOOP_COUNT_MAX,
                                std::size_t max_busy_loop_count = EINSUMS_BUSY_LOOP_COUNT_MAX, std::size_t shutdown_check_count = 10)
        // NOLINTEND(bugprone-easily-swappable-parameters)
        : name_(name), index_(index), mode_(mode), num_threads_(num_threads), thread_offset_(thread_offset), notifier_(notifier),
          affinity_data_(affinity_data), max_idle_loop_count_(max_idle_loop_count), max_busy_loop_count_(max_busy_loop_count),
...
class EINSUMS_EXPORT thread_pool_base {
  public:
    /// \cond NOINTERNAL
    ...
    ///           is pre-initialized to \a einsums#throws the function will
    ...
    /// have been suspended.
    ///
    /// \note A thread pool cannot be suspended from a einsums thread running
    ///       on the pool itself.
    ///
    /// \param ec [in,out] this represents the error status on exit, if this
    ///           is pre-initialized to \a einsums#throws the function will
    ///           throw on error instead.
    ///
    /// \throws einsums::exception if called from a einsums thread which is running
    ///         on the pool itself.
  ...
  public:
    ...
    virtual thread_state set_state(thread_id_type const &id, thread_schedule_state new_state, thread_restart_state new_state_ex,
    ...
    virtual std::int64_t get_executed_threads(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_executed_thread_phases(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
#    if defined(EINSUMS_HAVE_THREAD_IDLE_RATES)
    virtual std::int64_t get_thread_phase_duration(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_thread_duration(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_thread_phase_overhead(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_thread_overhead(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_cumulative_thread_duration(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_cumulative_thread_overhead(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
#    endif
#endif

    virtual std::int64_t get_cumulative_duration(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
...
#    if defined(EINSUMS_HAVE_THREAD_CREATION_AND_CLEANUP_RATES)
    virtual std::int64_t avg_creation_idle_rate(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t avg_cleanup_idle_rate(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
#    endif
#endif

    virtual std::int64_t get_queue_length(std::size_t, bool) { return 0; }

#if defined(EINSUMS_HAVE_THREAD_QUEUE_WAITTIME)
    virtual std::int64_t get_average_thread_wait_time(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_average_task_wait_time(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
#endif

#if defined(EINSUMS_HAVE_THREAD_STEALING_COUNTS)
    virtual std::int64_t get_num_pending_misses(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_num_pending_accesses(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }

    virtual std::int64_t get_num_stolen_from_pending(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_num_stolen_to_pending(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_num_stolen_from_staged(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
    virtual std::int64_t get_num_stolen_to_staged(std::size_t /*thread_num*/, bool /*reset*/) { return 0; }
#endif

    virtual std::int64_t get_thread_count(thread_schedule_state /*state*/, execution::thread_priority /*priority*/,
    ...
    std::int64_t get_thread_count_unknown(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::unknown, execution::thread_priority::default_, num_thread, reset);
    }
    std::int64_t get_thread_count_active(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::active, execution::thread_priority::default_, num_thread, reset);
    }
    std::int64_t get_thread_count_pending(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::pending, execution::thread_priority::default_, num_thread, reset);
    }
    std::int64_t get_thread_count_suspended(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::suspended, execution::thread_priority::default_, num_thread, reset);
    }
    std::int64_t get_thread_count_terminated(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::terminated, execution::thread_priority::default_, num_thread, reset);
    }
    std::int64_t get_thread_count_staged(std::size_t num_thread, bool reset) {
        return get_thread_count(thread_schedule_state::staged, execution::thread_priority::default_, num_thread, reset);
    }
    ...
    virtual einsums::runtime_state get_state() const                       = 0;
    virtual einsums::runtime_state get_state(std::size_t num_thread) const = 0;

    virtual bool has_reached_state(einsums::runtime_state s) const = 0;

    virtual void do_some_work(std::size_t /*num_thread*/) {}

    virtual void report_error(std::size_t global_thread_num, std::exception_ptr const &e) { notifier_.on_error(global_thread_num, e); }
...
template <>
struct fmt::formatter<einsums::threads::detail::thread_pool_base> : fmt::formatter<std::string> {
    template <typename FormatContext>
    auto format(einsums::threads::detail::thread_pool_base const &thread_pool, FormatContext &ctx) const {
        auto id = thread_pool.get_pool_id();
        return fmt::formatter<std::string>::format(fmt::format("{}({})", id.name(), id.index()), ctx);
    }
};
