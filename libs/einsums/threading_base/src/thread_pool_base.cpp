...
thread_pool_base::thread_pool_base(thread_pool_init_parameters const &init)
    : id_(init.index_, init.name_), thread_offset_(init.thread_offset_), affinity_data_(init.affinity_data_), timestamp_scale_(1.0),
...
mask_type thread_pool_base::get_used_processing_units() const {
    ...
    for (std::size_t thread_num = 0; thread_num < get_os_thread_count(); ++thread_num) {
        if (sched->get_state(thread_num).load() <= runtime_state::suspended) {
            used_processing_units |= affinity_data_.get_pu_mask(topo, thread_num + get_thread_offset());
        }
    }
...
void thread_pool_base::init_pool_time_scale() {
    ...
    std::uint64_t base_timestamp = einsums::chrono::detail::timestamp();
    std::uint64_t base_time      = static_cast<std::uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());
    std::uint64_t curr_timestamp = einsums::chrono::detail::timestamp();
    std::uint64_t curr_time      = static_cast<std::uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());

    while ((curr_time - base_time) <= 100000) {
        curr_timestamp = einsums::chrono::detail::timestamp();
        curr_time      = static_cast<std::uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());
    }

    if (curr_timestamp - base_timestamp != 0) {
        timestamp_scale_ = double(curr_time - base_time) / double(curr_timestamp - base_timestamp);
    }
}
