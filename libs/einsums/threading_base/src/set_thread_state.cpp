...
thread_result_type set_active_state(thread_id_ref_type thrd, thread_schedule_state newstate, thread_restart_state newstate_ex,
                                    execution::thread_priority priority, thread_state previous_state) {
    if (EINSUMS_UNLIKELY(!thrd)) {
        EINSUMS_THROW_EXCEPTION(einsums::error::null_thread_id, "threads::detail::set_active_state", "null thread id encountered");
    ...
    execution::thread_schedule_hint schedulehint{static_cast<std::int16_t>(get_thread_id_data(thrd)->get_last_worker_thread_num())};
...
///////////////////////////////////////////////////////////////////////////
thread_state set_thread_state(thread_id_type const &thrd, thread_schedule_state new_state, thread_restart_state new_state_ex,
                              execution::thread_priority priority, execution::thread_schedule_hint schedulehint, bool retry_on_active,
                              error_code &ec) {
    if (EINSUMS_UNLIKELY(!thrd)) {
    ...
    if (new_state == thread_schedule_state::active) {
        EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "invalid new state: {}", new_state);
        return thread_state(thread_schedule_state::unknown, thread_restart_state::unknown);
    }
    ...
    do {
        ...
        case thread_schedule_state::active: {
            if (retry_on_active) {
                ...
                thread_init_data data(
                    util::detail::bind(&set_active_state, thread_id_ref_type(thrd), new_state, new_state_ex, priority, previous_state),
                    "set state for active thread", priority, execution::thread_schedule_hint{}, execution::thread_stacksize::nostack);
                ...
                einsums::execution::this_thread::detail::yield_k(k, "einsums::threads::detail::set_thread_state");
                ++k;
        ...
        case thread_schedule_state::pending_boost:
            if (thread_schedule_state::suspended == new_state) {
                ...
                std::string str = fmt::format("set_thread_state: invalid new state, can't demote a pending "
                                              "thread, thread({}), description({}), new state({})",
                                              thrd, get_thread_id_data(thrd)->get_description(), new_state);

                // NOLINTNEXTLINE(bugprone-branch-clone)
                EINSUMS_LOG(critical, str);

                EINSUMS_THROWS_IF(ec, einsums::error::bad_parameter, "{}", str);
                return thread_state(thread_schedule_state::unknown, thread_restart_state::unknown);
            }
            break;
        ...
        case thread_schedule_state::pending_do_not_schedule:
            [[fallthrough]];
        default: {
            EINSUMS_ASSERT_MSG(false, "set_thread_state: previous state was {}",
        ...
        // NOLINTNEXTLINE(bugprone-branch-clone)
        EINSUMS_LOG(info, "set_thread_state: thread({}), description({}), new state({}), old state({})", thrd,
    ...
    thread_schedule_state previous_state_val = previous_state.state();
    if (!(previous_state_val == thread_schedule_state::pending || previous_state_val == thread_schedule_state::pending_boost) &&
        (new_state == thread_schedule_state::pending || new_state == thread_schedule_state::pending_boost)) {
