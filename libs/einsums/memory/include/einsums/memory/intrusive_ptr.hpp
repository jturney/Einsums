...
#    pragma GCC diagnostic pop
    ...
    // Move support
    constexpr intrusive_ptr(intrusive_ptr &&rhs) noexcept : px(rhs.px) { rhs.px = nullptr; }
    ...
    template <typename U, typename Enable = std::enable_if_t<memory::detail::sp_convertible_v<U, T>>>
    constexpr intrusive_ptr(intrusive_ptr<U> &&rhs) noexcept : px(rhs.px) {
        rhs.px = nullptr;
    }
...
template <typename T>
struct fmt::formatter<einsums::memory::intrusive_ptr<T>> : fmt::formatter<void *> {
    template <typename FormatContext>
    auto format(einsums::memory::intrusive_ptr<T> p, FormatContext &ctx) const {
...
struct hash<einsums::memory::intrusive_ptr<T>> {
    ...
    result_type einsums_STATIC_CALL_OPERATOR(einsums::memory::intrusive_ptr<T> const &p) noexcept { return hash<T *>{}(p.get()); }
};
} // namespace std
