...
#include <einsums/thread_support/thread_name.hpp>
...
#if defined(EINSUMS_HAVE_GPU_SUPPORT)
#    include <einsums/async_cuda_base/cuda_event.hpp>
#endif
...
#    if defined(EINSUMS_HAVE_STACKTRACES)
        if (verbosity >= 1) {
            auto const trace_depth =
                string_util::from_string<std::size_t>(get_config_entry("einsums.trace_depth", EINSUMS_HAVE_THREAD_BACKTRACE_DEPTH));
            std::cerr << "{stack-trace}: " << einsums::debug::detail::trace(trace_depth) << "\n";
        }
#    endif
...
EINSUMS_EXPORT void EINSUMS_CDECL new_handler() {
    EINSUMS_THROW_EXCEPTION(einsums::error::out_of_memory, "new allocator failed to allocate memory");
}

///////////////////////////////////////////////////////////////////////////
// Sometimes the einsums library gets simply unloaded as a result of some
...
void set_signal_handlers() {
    ...
    SetConsoleCtrlHandler(einsums::detail::termination_handler, TRUE);
#else
...
auto get_runtime_state_name(einsums::runtime_state st) -> char const * {
    if (st < einsums::runtime_state::invalid || st >= einsums::runtime_state::last_valid_runtime)
...
///////////////////////////////////////////////////////////////////////////
runtime::runtime(einsums::util::runtime_configuration &rtcfg, bool initialize)
    : rtcfg_(rtcfg), topology_(resource::get_partitioner().get_topology()), state_(einsums::runtime_state::invalid),
      on_start_func_(global_on_start_func), on_stop_func_(global_on_stop_func), on_error_func_(global_on_error_func), result_(0),
      notifier_(), thread_manager_(), stop_called_(false), stop_done_(false) {
...
runtime::runtime(einsums::util::runtime_configuration &rtcfg)
    : rtcfg_(rtcfg), topology_(resource::get_partitioner().get_topology()), state_(einsums::runtime_state::invalid),
      on_start_func_(global_on_start_func), on_stop_func_(global_on_stop_func), on_error_func_(global_on_error_func), result_(0),
      notifier_(), thread_manager_(), stop_called_(false), stop_done_(false) {
    init_global_data();
...
void runtime::set_notification_policies(notification_policy_type &&notifier) {
    ...
    thread_manager_.reset(new einsums::threads::detail::thread_manager(rtcfg_, notifier_));
}
...
void runtime::init() {
    ...
    set_state(einsums::runtime_state::initialized);
}

runtime::~runtime() {
    EINSUMS_LOG(debug, "~runtime(entering)");

    // stop all services
    thread_manager_->stop();
    EINSUMS_LOG(debug, "~runtime(finished)");

    resource::detail::delete_partitioner();

#if defined(EINSUMS_HAVE_GPU_SUPPORT)
    einsums::cuda::experimental::detail::cuda_event_pool::get_event_pool().clear();
#endif
}

void runtime::on_exit(einsums::util::detail::function<void()> const &f) {
    std::lock_guard<std::mutex> l(mtx_);
    on_exit_functions_.push_back(f);
}

void runtime::starting() {
    state_.store(einsums::runtime_state::pre_main);
}

void runtime::stopping() {
    state_.store(einsums::runtime_state::stopped);

    using value_type = einsums::util::detail::function<void()>;
...
bool runtime::stopped() const {
    return state_.load() == einsums::runtime_state::stopped;
}

einsums::util::runtime_configuration &runtime::get_config() {
    return rtcfg_;
}

einsums::util::runtime_configuration const &runtime::get_config() const {
    return rtcfg_;
}

einsums::runtime_state runtime::get_state() const {
    return state_.load();
}

einsums::threads::detail::topology const &runtime::get_topology() const {
    return topology_;
}

void runtime::set_state(einsums::runtime_state s) {
    EINSUMS_LOG(info, "{}", einsums::detail::get_runtime_state_name(s));
    state_.store(s);
}
...
std::chrono::time_point<std::chrono::high_resolution_clock> &runtime_uptime() {
    static std::chrono::time_point<std::chrono::high_resolution_clock> uptime = std::chrono::high_resolution_clock::now();
    return uptime;
}
} // namespace
...
void runtime::init_global_data() {
    ...
    EINSUMS_ASSERT(nullptr == einsums::threads::detail::thread_self::get_self());
...
einsums::threads::callback_notifier::on_startstop_type runtime::on_start_func() const {
    return on_start_func_;
}

einsums::threads::callback_notifier::on_startstop_type runtime::on_stop_func() const {
    return on_stop_func_;
}

einsums::threads::callback_notifier::on_error_type runtime::on_error_func() const {
    return on_error_func_;
}

einsums::threads::callback_notifier::on_startstop_type runtime::on_start_func(einsums::threads::callback_notifier::on_startstop_type &&f) {
    einsums::threads::callback_notifier::on_startstop_type newf = std::move(f);
    std::swap(on_start_func_, newf);
    return newf;
}

einsums::threads::callback_notifier::on_startstop_type runtime::on_stop_func(einsums::threads::callback_notifier::on_startstop_type &&f) {
    einsums::threads::callback_notifier::on_startstop_type newf = std::move(f);
    std::swap(on_stop_func_, newf);
    return newf;
}

einsums::threads::callback_notifier::on_error_type runtime::on_error_func(einsums::threads::callback_notifier::on_error_type &&f) {
    einsums::threads::callback_notifier::on_error_type newf = std::move(f);
    std::swap(on_error_func_, newf);
    return newf;
}

std::size_t runtime::get_num_worker_threads() const {
    EINSUMS_ASSERT(thread_manager_);
    return thread_manager_->get_os_thread_count();
}

///////////////////////////////////////////////////////////////////////////
einsums::threads::callback_notifier::on_startstop_type get_thread_on_start_func() {
...
einsums::threads::callback_notifier::on_startstop_type get_thread_on_stop_func() {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
...
einsums::threads::callback_notifier::on_error_type get_thread_on_error_func() {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
...
einsums::threads::callback_notifier::on_startstop_type
...
einsums::threads::callback_notifier::on_error_type register_thread_on_error_func(einsums::threads::callback_notifier::on_error_type &&f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
...
void report_error(std::size_t num_thread, std::exception_ptr const &e) {
    ...
    if (!thread_manager_is(einsums::runtime_state::running)) {
...
void report_error(std::exception_ptr const &e) {
    ...
    std::size_t num_thread = einsums::get_worker_thread_num();
    get_runtime().get_thread_manager().report_error(num_thread, e);
}

bool register_on_exit(einsums::util::detail::function<void()> const &f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr == rt)
        return false;
...
void set_config_entry_callback(std::string const                                                                     &key,
                               einsums::util::detail::function<void(std::string const &, std::string const &)> const &callback) {
    if (get_runtime_ptr() != nullptr) {
...
void retrieve_commandline_arguments(einsums::program_options::options_description const &app_options,
    ...
    einsums::detail::section &cfg = einsums::detail::get_runtime().get_config();
    if (cfg.has_entry("einsums.cmd_line"))
        cmdline = cfg.get_entry("einsums.cmd_line");

    parse_commandline(cfg, app_options, cmdline, vm, commandline_error_mode::allow_unregistered);
}

///////////////////////////////////////////////////////////////////////////
// retrieve the command line arguments
void retrieve_commandline_arguments(std::string const &appname, einsums::program_options::variables_map &vm) {
...
bool is_running() {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt)
        return rt->get_state() == einsums::runtime_state::running;
    return false;
}
...
bool is_stopped() {
    if (!detail::exit_called) {
        runtime *rt = get_runtime_ptr();
        if (nullptr != rt)
            return rt->get_state() == einsums::runtime_state::stopped;
    }
...
bool is_stopped_or_shutting_down() {
    ...
    if (!detail::exit_called && nullptr != rt) {
        einsums::runtime_state st = rt->get_state();
        return st >= einsums::runtime_state::shutdown;
    }
    return true; // assume stopped
}

bool tolerate_node_faults() {
#ifdef EINSUMS_HAVE_FAULT_TOLERANCE
    return true;
#else
    return false;
#endif
...
bool is_starting() {
    runtime *rt = get_runtime_ptr();
    return nullptr != rt ? rt->get_state() <= einsums::runtime_state::startup : true;
}
...
std::ptrdiff_t get_default_stack_size() {
    return einsums::detail::get_runtime().get_config().get_default_stack_size();
}
...
std::ptrdiff_t get_stack_size(execution::thread_stacksize stacksize) {
    if (stacksize == execution::thread_stacksize::current)
        return einsums::threads::detail::get_self_stacksize();

    return einsums::detail::get_runtime().get_config().get_stack_size(stacksize);
}

void reset_thread_distribution() {
    einsums::detail::get_runtime().get_thread_manager().reset_thread_distribution();
}

void set_scheduler_mode(threads::scheduler_mode m) {
    einsums::detail::get_runtime().get_thread_manager().set_scheduler_mode(m);
}

void add_scheduler_mode(threads::scheduler_mode m) {
    einsums::detail::get_runtime().get_thread_manager().add_scheduler_mode(m);
}

void remove_scheduler_mode(threads::scheduler_mode m) {
    einsums::detail::get_runtime().get_thread_manager().remove_scheduler_mode(m);
}

detail::topology const &get_topology() {
    einsums::detail::runtime *rt = einsums::detail::get_runtime_ptr();
    if (rt == nullptr) {
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "einsums::threads::get_topology",
                                "the einsums runtime system has not been initialized yet");
    }
...
einsums::util::runtime_configuration const &get_config() {
    return get_runtime().get_config();
}
} // namespace einsums::detail

#if defined(_WIN64) && defined(EINSUMS_DEBUG) && !defined(EINSUMS_HAVE_FIBER_BASED_COROUTINES)
#    include <io.h>
#endif
...
void register_pre_startup_function(startup_function_type f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
        if (rt->get_state() > einsums::runtime_state::pre_startup) {
            EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "register_pre_startup_function",
...
void register_startup_function(startup_function_type f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
        if (rt->get_state() > einsums::runtime_state::startup) {
            EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "register_startup_function",
...
void register_pre_shutdown_function(shutdown_function_type f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
        if (rt->get_state() > einsums::runtime_state::pre_shutdown) {
            EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "register_pre_shutdown_function",
...
void register_shutdown_function(shutdown_function_type f) {
    runtime *rt = get_runtime_ptr();
    if (nullptr != rt) {
        if (rt->get_state() > einsums::runtime_state::shutdown) {
            EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "register_shutdown_function",
...
void runtime::call_startup_functions(bool pre_startup) {
    if (pre_startup) {
        set_state(einsums::runtime_state::pre_startup);
        for (startup_function_type &f : pre_startup_functions_) {
            f();
        }
    } else {
        set_state(einsums::runtime_state::startup);
...
void handle_print_bind(std::size_t num_threads) {
    einsums::threads::detail::topology &top = einsums::threads::detail::get_topology();
    auto const                         &rp  = einsums::resource::get_partitioner();
...

    {
        ...
        for (std::size_t i = 0; i != num_threads; ++i) {
            ...
            einsums::threads::detail::mask_cref_type pu_mask = rp.get_pu_mask(i);
            ...
            einsums::threads::detail::mask_type boundcpu = top.get_cpubind_mask(blob, ec);

            // Empty masks are equivalent to full masks, so we first
            // transform possible empty masks to full masks.
            einsums::threads::detail::mask_type boundcpu_never_empty = !einsums::threads::detail::any(boundcpu) ? ~boundcpu : boundcpu;
            einsums::threads::detail::mask_type pu_mask_never_empty  = !einsums::threads::detail::any(pu_mask) ? ~pu_mask : pu_mask;

            // The masks reported by einsums must be the same as the ones
            // reported from HWLOC.
            if (!ec && !einsums::threads::detail::equal(boundcpu_never_empty, pu_mask_never_empty, num_threads)) {
                std::string boundcpu_str = einsums::threads::detail::to_string(boundcpu_never_empty);
                std::string pu_mask_str  = einsums::threads::detail::to_string(pu_mask_never_empty);
                EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "handle_print_bind",
...
runtime::run_helper(einsums::util::detail::function<runtime::einsums_main_function_type> const &func, int &result, bool call_startup) {
    ...
    try {
        {
            einsums::program_options::options_description options;
            result = einsums::detail::handle_late_commandline_options(get_config(), options, &detail::handle_print_bind);
            if (result) {
                EINSUMS_LOG(info, "runtime::run_helper: bootstrap aborted, bailing out");

                set_state(einsums::runtime_state::running);
                finalize();

                return einsums::threads::detail::thread_result_type(einsums::threads::detail::thread_schedule_state::terminated,
                                                                    einsums::threads::detail::invalid_thread_id);
            }
        }

        if (call_startup) {
            call_startup_functions(true);
            EINSUMS_LOG(info, "(3rd stage) run_helper: ran pre-startup functions");

            call_startup_functions(false);
            EINSUMS_LOG(info, "(4th stage) run_helper: ran startup functions");
        }

        EINSUMS_LOG(info, "(4th stage) runtime::run_helper: bootstrap complete");
        set_state(einsums::runtime_state::running);

        // Now, execute the user supplied thread function (einsums_main)
        if (!!func) {
            EINSUMS_LOG(info, "(last stage) runtime::run_helper: about to invoke einsums_main");

            // Change our thread description, as we're about to call einsums_main
            einsums::threads::detail::set_thread_description(einsums::threads::detail::get_self_id(), "einsums_main");

            // Call einsums_main
            result = func();
        }
    } catch (...) {
        // make sure exceptions thrown in einsums_main don't escape
        // unnoticed
        {
            std::lock_guard<std::mutex> l(mtx_);
    ...
    if (caught_exception) {
        ...
        fmt::print(std::cerr, "The einsums runtime caught the following exception in the entry point (typically "
                              "einsums_main). The exception may be rethrown later by einsums::init or einsums::stop. The "
                              "einsums runtime will wait for all tasks to finish, but may be deadlocked because of "
                              "the exception.\n");
...
int runtime::start(einsums::util::detail::function<einsums_main_function_type> const &func, bool blocking) {
#if defined(_WIN64) && defined(EINSUMS_DEBUG) && !defined(EINSUMS_HAVE_FIBER_BASED_COROUTINES)
    ...
    EINSUMS_LOG(info, "cmd_line: {}", get_config().get_cmd_line());

    EINSUMS_LOG(info, "(1st stage) runtime::start");
    ...
    thread_manager_->run();
    EINSUMS_LOG(info, "(1st stage) runtime::start: started thread_manager");
    // }}}

    // {{{ launch main
    // register the given main function with the thread manager
    EINSUMS_LOG(info, "(1st stage) runtime::start: launching run_helper einsums thread");

    einsums::threads::detail::thread_init_data data(einsums::util::detail::bind(&runtime::run_helper, this, func, std::ref(result_), true),
    ...
    einsums::threads::detail::thread_id_ref_type id = einsums::threads::detail::invalid_thread_id;
    ...
    if (blocking) {
        ...
        // wait for at least einsums::runtime_state::running
        einsums::util::yield_while([this]() { return get_state() < einsums::runtime_state::running; }, "runtime::start");
    }

    return 0; // return zero as we don't know the outcome of einsums_main yet
}

int runtime::start(bool blocking) {
    einsums::util::detail::function<einsums_main_function_type> empty_main;
...
void runtime::wait_finalize() {
    std::unique_lock<std::mutex> l(mtx_);
    EINSUMS_LOG(info, "runtime: about to enter wait state");
    wait_condition_.wait(l, [&] { return stop_done_; });
    EINSUMS_LOG(info, "runtime: exiting wait state");
}
...
void runtime::wait_helper(std::mutex &mtx, std::condition_variable &cond, bool &running) {
    ...
    itt_thread_set_name(thread_name.c_str());
...
int runtime::wait() {
    ...
    std::thread t(einsums::util::detail::bind(&runtime::wait_helper, this, std::ref(mtx), std::ref(cond), std::ref(running)));
...
void runtime::stop(bool blocking) {
    EINSUMS_LOG(info, "runtime: about to stop services");
    ...
    if (einsums::threads::detail::get_self_ptr()) {
        ...
        // from a einsums thread, so it would deadlock by waiting for the thread
        // manager
        std::mutex                   mtx;
        std::condition_variable      cond;
        std::unique_lock<std::mutex> l(mtx);

        std::thread t(einsums::util::detail::bind(&runtime::stop_helper, this, blocking, std::ref(cond), std::ref(mtx)));
        cond.wait(l);
        ...
        EINSUMS_LOG(info, "runtime: stopped all services");
    }
...
void runtime::suspend() {
    EINSUMS_LOG(info, "runtime: about to suspend runtime");

    if (state_.load() == einsums::runtime_state::sleeping) {
        return;
    }

    if (state_.load() != einsums::runtime_state::running) {
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "runtime::suspend", "Can only suspend runtime from running state");
    }

    thread_manager_->suspend();

    set_state(einsums::runtime_state::sleeping);
}

void runtime::resume() {
    EINSUMS_LOG(info, "runtime: about to resume runtime");
    ...
    if (state_.load() != einsums::runtime_state::sleeping) {
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "runtime::resume", "Can only resume runtime from suspended state");
    }
...
einsums::threads::detail::thread_manager &runtime::get_thread_manager() {
    return *thread_manager_;
}

///////////////////////////////////////////////////////////////////////////
bool runtime::report_error(std::size_t num_thread, std::exception_ptr const &e, bool /*terminate_all*/) {
    ...
    // Early and late exceptions, errors outside of einsums-threads
    if (!einsums::threads::detail::get_self_ptr() || !thread_manager_is(einsums::runtime_state::running)) {
...
bool runtime::report_error(std::exception_ptr const &e, bool terminate_all) {
    return report_error(einsums::get_worker_thread_num(), e, terminate_all);
}
...
///////////////////////////////////////////////////////////////////////////
int runtime::run(einsums::util::detail::function<einsums_main_function_type> const &func) {
...
///////////////////////////////////////////////////////////////////////////
einsums::threads::callback_notifier runtime::get_notification_policy(char const *prefix) {
    ...
    notifier.add_on_start_thread_callback(einsums::util::detail::bind(&runtime::init_tss_helper, this, prefix, _1, _2, _3, _4));
    notifier.add_on_stop_thread_callback(einsums::util::detail::bind(&runtime::deinit_tss_helper, this, prefix, _1));
    notifier.set_on_error_callback(einsums::util::detail::bind(static_cast<report_error_t>(&runtime::report_error), this, _1, _2, true));

    return notifier;
}

void runtime::init_tss_helper(char const *context, std::size_t local_thread_num, std::size_t global_thread_num, char const *pool_name,
                              char const *postfix) {
    error_code ec(throw_mode::lightweight);
...
void runtime::init_tss_ex(char const *context, std::size_t local_thread_num, std::size_t global_thread_num, char const *pool_name,
                          char const *postfix)
// NOLINTEND(bugprone-easily-swappable-parameters)
{
    std::ostringstream fullname;
    fullname << "einsums/" << context;
    ...
    EINSUMS_ASSERT(detail::thread_name().empty());
    detail::thread_name() = std::move(fullname).str();
...
void runtime::deinit_tss_helper(char const *context, std::size_t global_thread_num) {
    einsums::threads::detail::reset_continuation_recursion_count();
...
/// Register an external OS-thread with einsums
...
/// Unregister an external OS-thread with einsums
bool runtime::unregister_thread() {
    deinit_tss_helper(detail::thread_name().c_str(), einsums::get_worker_thread_num());
    return true;
}

///////////////////////////////////////////////////////////////////////////
einsums::threads::callback_notifier get_notification_policy(char const *prefix) {
...
char const *get_stack_size_name(std::ptrdiff_t size) {
    ...
    einsums::util::runtime_configuration const &rtcfg = einsums::detail::get_config();
...
bool is_runtime_initialized() noexcept {
    return einsums::detail::get_runtime_ptr() != nullptr;
}

std::size_t get_num_worker_threads() {
    einsums::detail::runtime *rt = einsums::detail::get_runtime_ptr();
    if (nullptr == rt) {
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "the runtime system has not been initialized yet");
        return std::size_t(0);
    }
