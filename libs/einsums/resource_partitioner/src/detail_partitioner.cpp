...
init_pool_data::init_pool_data(std::string const &name, scheduling_policy sched, einsums::threads::scheduler_mode mode)
    : pool_name_(name), scheduling_policy_(sched), num_threads_(0), mode_(mode) {
    if (name.empty()) {
...
void init_pool_data::add_resource(std::size_t pu_index, bool exclusive, std::size_t num_threads) {
    if (pu_index >= einsums::threads::detail::hardware_concurrency()) {
        throw_invalid_argument("init_pool_data::add_resource",
                               ...
                               "available number of processing units on this machine is " +
                                   std::to_string(einsums::threads::detail::hardware_concurrency()));
    }
...
void init_pool_data::print_pool(std::ostream &os) const {
    ...
    for (threads::detail::mask_cref_type assigned_pu : assigned_pus_) {
        os << einsums::threads::detail::to_string(assigned_pu) << '\n';
    }
}

void init_pool_data::assign_pu(std::size_t virt_core) {
    EINSUMS_ASSERT(virt_core <= assigned_pu_nums_.size());
    EINSUMS_ASSERT(!std::get<2>(assigned_pu_nums_[virt_core]));
...
partitioner::partitioner()
    : rtcfg_(), first_core_(std::size_t(-1)), mode_(mode_default), topo_(threads::detail::get_topology()),
    ...
    if (++instance_number_counter_ > 1) {
        throw_runtime_error("partitioner::partitioner", "Cannot instantiate more than one resource partitioner");
    }

#if defined(EINSUMS_HAVE_MAX_CPU_COUNT)
    if (EINSUMS_HAVE_MAX_CPU_COUNT < topo_.get_number_of_pus()) {
        throw_runtime_error("partitioner::partioner",
                            fmt::format("Currently, EINSUMS_HAVE_MAX_CPU_COUNT is set to {0} while your system has {1} "
                                        "processing units. Please reconfigure einsums with -DEINSUMS_WITH_MAX_CPU_COUNT={1} "
                                        "(or higher) to increase the maximal CPU count supported by einsums.",
                                        EINSUMS_HAVE_MAX_CPU_COUNT, topo_.get_number_of_pus()));
    }
#endif
    ...
    initial_thread_pools_.push_back(init_pool_data("default", scheduling_policy::unspecified, default_scheduler_mode_));
}
...
// This function is called in einsums_init, before the instantiation of the
...
void partitioner::setup_pools() {
        ...
        for (einsums::resource::core &c : d.cores_) {
            for (einsums::resource::pu &p : c.pus_) {
                if (p.thread_occupancy_count_ == 0) {
                    ...
                    // (to avoid deadlocks).
                    add_resource(p, get_default_pool_name(), first || !(mode_ & mode_allow_dynamic_pools));
                    first = false;
                }
            }
        }
    ...
    if (get_pool_data(l, get_default_pool_name()).num_threads_ == 0) {
        ...
        std::string process_mask_message =
            affinity_data_.using_process_mask()
                ? fmt::format("einsums is using a process mask: {}.",
                              einsums::threads::detail::to_string(get_topology().get_cpubind_mask_main_thread()))
                : "einsums is not using a process mask.";
        auto        omp_proc_bind = std::getenv("OMP_PROC_BIND");
        std::string omp_proc_bind_message =
            omp_proc_bind ? fmt::format("OMP_PROC_BIND is currently set to \"{}\".", omp_proc_bind) : "OMP_PROC_BIND is currently unset.";
        throw_runtime_error("partitioner::setup_pools",
                            fmt::format("The default pool (\"{}\") has no threads assigned. Make sure that the einsums "
                                        "runtime is started with enough threads for all thread pools. This can be "
                                        "caused by the einsums runtime being configured to use additional thread pools "
                                        ...
                                        "einsums can read the mask. einsums is currently configured to use {} thread{} and "
                                        "{} thread pool{}. {} {}",
                                        get_default_pool_name(), get_num_threads(), get_num_threads() != 1 ? "s" : "", get_num_pools(),
                                        get_num_pools() != 1 ? "s" : "", process_mask_message, omp_proc_bind_message));
    ...
    if (check_empty_pools()) {
        ...
        throw_runtime_error("partitioner::setup_pools", "Pools empty of resources are not allowed. Please re-run this application with "
                                                        "allow-empty-pool-policy (not implemented yet)");
    }
    //! FIXME add allow-empty-pools policy. Wait, does this even make sense??
}

// This function is called in einsums_init, before the instantiation of the runtime
...
void partitioner::setup_schedulers() {
    ...
    std::string default_scheduler_str = rtcfg_.get_entry("einsums.scheduler", std::string());
    ...
    if (0 == std::string("local").find(default_scheduler_str)) {
        ...
        throw einsums::detail::command_line_error("Bad value for command line option --einsums:queuing");
    }
...
void partitioner::create_thread_pool(std::string const &pool_name, scheduling_policy sched, einsums::threads::scheduler_mode mode) {
    if (pool_name.empty()) {
    ...
    for (std::size_t i = 1; i < num_thread_pools; i++) {
        if (pool_name == initial_thread_pools_[i].pool_name_) {
            l.unlock();
            throw std::invalid_argument("partitioner::create_thread_pool: there already exists a pool named '" + pool_name + "'.\n");
        }
    }
...
// create a new thread_pool
void partitioner::create_thread_pool(std::string const &pool_name, scheduler_function scheduler_creation) {
    if (pool_name.empty()) {
        throw std::invalid_argument("partitioner::create_thread_pool: cannot instantiate a "
                                    "initial_thread_pool with empty string as a name.");
    }
...
void partitioner::add_resource(pu const &p, std::string const &pool_name, bool exclusive, std::size_t num_threads) {
    ...
    if (p.thread_occupancy_count_ == 0) {
        ...
        std::size_t num_threads = ::einsums::detail::get_entry_as<std::size_t>(rtcfg_, "einsums.os_threads", 0);
        EINSUMS_ASSERT(num_threads != 0);
        ...
        if (detail::init_pool_data::num_threads_overall > num_threads) {
            ...
            throw std::runtime_error("partitioner::add_resource: Creation of " +
                                     ...
                                     " threads requested by the resource partitioner, but only " + std::to_string(num_threads) +
...
void partitioner::add_resource(std::vector<pu> const &pv, std::string const &pool_name, bool exclusive) {
    for (pu const &p : pv) {
...
std::size_t partitioner::get_num_threads() const {
    ...
    EINSUMS_ASSERT(mode_ & mode_allow_oversubscription ||
                   num_threads == ::einsums::detail::get_entry_as<std::size_t>(rtcfg_, "einsums.os_threads", std::size_t(-1)));
...
einsums::threads::scheduler_mode partitioner::get_scheduler_mode(std::size_t pool_index) const {
    std::unique_lock<mutex_type> l(mtx_);
    return get_pool_data(l, pool_index).mode_;
}

detail::init_pool_data const &partitioner::get_pool_data(std::unique_lock<mutex_type> &l, std::size_t pool_index) const {
    if (pool_index >= initial_thread_pools_.size()) {
        l.unlock();
        throw_invalid_argument("partitioner::get_pool_data", "pool index " + std::to_string(pool_index) +
...
std::string const &partitioner::get_pool_name(std::size_t index) const {
    if (index >= initial_thread_pools_.size()) {
        throw_invalid_argument("partitioner::get_pool_name: ",
                               "pool " + std::to_string(index) + " (zero-based index) requested out of bounds. The partitioner owns only " +
...
void partitioner::init(resource::partitioner_mode rpmode, einsums::detail::section rtcfg, einsums::detail::affinity_data affinity_data) {
    mode_          = rpmode;
    rtcfg_         = rtcfg;
...
scheduler_function partitioner::get_pool_creator(std::size_t index) const {
    ...
    if (index >= initial_thread_pools_.size()) {
        l.unlock();
        throw std::invalid_argument("partitioner::get_pool_creator: pool requested out of bounds.");
    }
...
std::size_t partitioner::get_pool_index(std::string const &pool_name) const {
    ...
    throw_invalid_argument("partitioner::get_pool_index", "the resource partitioner does not own a thread pool named '" + pool_name + "'");
}
...
detail::init_pool_data const &partitioner::get_pool_data(std::unique_lock<mutex_type> &l, std::string const &pool_name) const {
    auto pool = std::find_if(initial_thread_pools_.begin(), initial_thread_pools_.end(),
                             [&pool_name](detail::init_pool_data const &itp) -> bool { return (itp.pool_name_ == pool_name); });
    ...
    l.unlock();
    throw_invalid_argument("partitioner::get_pool_data", "the resource partitioner does not own a thread pool named '" + pool_name + "'");
}

detail::init_pool_data &partitioner::get_pool_data(std::unique_lock<mutex_type> &l, std::string const &pool_name) {
    auto pool = std::find_if(initial_thread_pools_.begin(), initial_thread_pools_.end(),
                             [&pool_name](detail::init_pool_data const &itp) -> bool { return (itp.pool_name_ == pool_name); });
...
void partitioner::print_init_pool_data(std::ostream &os) const {
    ...
    //! make this prettier
    os << "the resource partitioner owns " << initial_thread_pools_.size() << " pool(s) : \n"; // -V128
    for (auto itp : initial_thread_pools_) {
        itp.print_pool(os);
    }
}
