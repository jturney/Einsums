...
struct pu {
  ...
  private:
    ...
    friend struct resource::detail::partitioner;
...
struct core {
    explicit core(std::size_t id = invalid_core_id, numa_domain *domain = nullptr) : _id(id), _domain(domain) {}
...
struct numa_domain {
    explicit numa_domain(std::size_t id = invalid_numa_domain_id) : _id(id) {}
...
namespace detail {
resource::partitioner make_partitioner(resource::partitioner_mode rpmode, einsums::detail::section rtcfg,
...
struct partitioner {
    ...
    EINSUMS_EXPORT void add_resource(einsums::resource::numa_domain const &nd, std::string const &pool_name, bool exclusive = true);
    EINSUMS_EXPORT void add_resource(std::vector<einsums::resource::numa_domain> const &ndv, std::string const &pool_name,
                                     bool exclusive = true);

    // Access all available NUMA domains
    EINSUMS_EXPORT std::vector<numa_domain> const &numa_domains() const;
  ...
  private:
    friend resource::partitioner detail::make_partitioner(resource::partitioner_mode rpmode, einsums::detail::section rtcfg,
                                                          einsums::detail::affinity_data affinity_data);

    partitioner(partitioner_mode rpmode, einsums::detail::section rtcfg, einsums::detail::affinity_data affinity_data);
