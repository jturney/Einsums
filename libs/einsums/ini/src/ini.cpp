...
section::section() : _root(this_()) {
}

section::section(std::string const &filename, section *root) : _root(nullptr != root ? root : this), _name(filename) {
    read(filename);
}

section::section(const section &in) : _root(this_()), _name(in.get_name()), _parent_name(in.get_parent_name()) {
...
section &section::operator=(section const &rhs) {
    if (this != &rhs) {
        ...
        _root        = this;
        _parent_name = rhs.get_parent_name();
        _name        = rhs.get_name();
...
// parse file
void section::parse(std::string const &sourcename, std::vector<std::string> const &lines, bool verify_existing, bool weed_out_comments,
    ...
    for (std::vector<std::string>::const_iterator it = lines.begin(); it != end; ++it) {
        ...
        if (weed_out_comments) {
            ...
            if (std::regex_match(line, what_comment, regex_comment)) {
                EINSUMS_ASSERT(3 == what_comment.size());
        ...
        if (assign_pos != std::string::npos) {
            ...
            std::string value   = trim_whitespace(line.substr(assign_pos + 1, line.size() - assign_pos - 1));
            ...
            // Example: einsums.commandline.allow_unknown
            for (std::string::size_type dot_pos = sec_key.find_first_of('.'); std::string::npos != dot_pos;
            ...
            std::unique_lock<mutex_type> l(current->_mutex);
...
void section::parse(std::string const &sourcename, std::string const &line, bool verify_existing, bool weed_out_comments,
                    bool replace_existing) {
    std::vector<std::string> lines;
    lines.push_back(line);
...
void section::add_notification_callback(std::string const &key, entry_changed_func const &callback) {
    std::unique_lock<mutex_type> l(_mutex);
...
///////////////////////////////////////////////////////////////////////////////
void section::add_section(std::unique_lock<mutex_type> & /* l */, std::string const &sec_name, section &sec, section *root) {
    // setting name and root
    sec._name        = sec_name;
    sec._parent_name = get_full_name();

    section &newsec = _sections[sec_name];
    newsec.clone_from(sec, (nullptr != root) ? root : get_root());
}

///////////////////////////////////////////////////////////////////////////
section *section::add_section_if_new(std::unique_lock<mutex_type> &l, std::string const &sec_name) {
    // do we know this one?
...
bool section::has_section(std::unique_lock<mutex_type> &l, std::string const &sec_name) const {
    ...
    if (i != std::string::npos) {
        ...
        section_map::const_iterator it = _sections.find(cor_sec_name);
        if (it != _sections.end()) {
            std::string                                                 sub_sec_name = sec_name.substr(i + 1);
            einsums::detail::unlock_guard<std::unique_lock<mutex_type>> ul(l);
            return (*it).second.has_section(sub_sec_name);
        }
        return false;
    }
    return _sections.find(sec_name) != _sections.end();
}
...
section *section::get_section(std::unique_lock<mutex_type> &l, std::string const &sec_name) {
    ...
    if (i != std::string::npos) {
        std::string           cor_sec_name = sec_name.substr(0, i);
        section_map::iterator it           = _sections.find(cor_sec_name);
        if (it != _sections.end()) {
            std::string                                                 sub_sec_name = sec_name.substr(i + 1);
            einsums::detail::unlock_guard<std::unique_lock<mutex_type>> ul(l);
        ...
        EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "No such section ({}) in section: {}", sec_name, name);
        return nullptr;
    }

    section_map::iterator it = _sections.find(sec_name);
    if (it != _sections.end())
        return &((*it).second);

    EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "No such section ({}) in section: {}", sec_name, get_name());
    return nullptr;
}

section const *section::get_section(std::unique_lock<mutex_type> &l, std::string const &sec_name) const {
    std::string::size_type i = sec_name.find('.');
    if (i != std::string::npos) {
        std::string                 cor_sec_name = sec_name.substr(0, i);
        section_map::const_iterator it           = _sections.find(cor_sec_name);
    ...
    section_map::const_iterator it = _sections.find(sec_name);
    if (it != _sections.end())
        return &((*it).second);
...
void section::add_entry(std::unique_lock<mutex_type> &l, std::string const &fullkey, std::string const &key, std::string val) {
    ...
    if (i != std::string::npos) {
        section *current = _root;

        // make sure all sections in key exist
        std::string sec_name = key.substr(0, i);

        std::string::size_type pos = 0;
        for (std::string::size_type pos1 = sec_name.find_first_of('.'); std::string::npos != pos1;
        ...
        entry_map::iterator it = _entries.find(key);
        if (it != _entries.end()) {
            auto &e = it->second;
            ...
            _entries[key] = entry_type(val, entry_changed_func());
        }
    }
}

void section::add_entry(std::unique_lock<mutex_type> &l, std::string const &fullkey, std::string const &key, entry_type const &val) {
    std::string::size_type i = key.find_last_of('.');
    if (i != std::string::npos) {
        section *current = _root;
        ...
        if (it != _entries.end()) {
            ...
            // just add this entry to the section
            std::pair<entry_map::iterator, bool> p = _entries.insert(entry_map::value_type(key, val));
            EINSUMS_ASSERT(p.second);
...
class compose_callback_impl {
  public:
    ...
    compose_callback_impl(A1 &&f1, A2 &&f2) : f1_(std::forward<A1>(f1)), f2_(std::forward<A2>(f2)) {}
...
template <typename F1, typename F2>
static EINSUMS_FORCEINLINE std::function<void(std::string const &, std::string const &)> compose_callback(F1 &&f1, F2 &&f2) {
    if (!f1)
...
void section::add_notification_callback(std::unique_lock<mutex_type> &l, std::string const &key, entry_changed_func const &callback) {
    std::string::size_type i = key.find_last_of('.');
    if (i != std::string::npos) {
        section *current = _root;
        ...
        if (it != _entries.end()) {
            ...
            _entries[key] = entry_type("", callback);
        }
    }
}
...
bool section::has_entry(std::unique_lock<mutex_type> &l, std::string const &key) const {
    ...
    if (i != std::string::npos) {
        ...
        if (has_section(l, sub_sec)) {
            section_map::const_iterator cit = _sections.find(sub_sec);
            EINSUMS_ASSERT(cit != _sections.end());
            einsums::detail::unlock_guard<std::unique_lock<mutex_type>> ul(l);
...
std::string section::get_entry(std::unique_lock<mutex_type> &l, std::string const &key) const {
    ...
    if (i != std::string::npos) {
        ...
        EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "section::get_entry", "No such key ({}) in section: {}", key, get_name());
        return "";
    }

    if (_entries.find(key) != _entries.end()) {
        entry_map::const_iterator cit = _entries.find(key);
        EINSUMS_ASSERT(cit != _entries.end());
        return expand(l, (*cit).second.first);
    }

    EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "section::get_entry", "No such section ({}) in section: {}", key, get_name());
    return "";
}

// NOLINTBEGIN(bugprone-easily-swappable-parameters)
std::string section::get_entry(std::unique_lock<mutex_type> &l, std::string const &key, std::string const &default_val) const
// NOLINTEND(bugprone-easily-swappable-parameters)
{
    using string_vector = std::vector<std::string>;

    string_vector split_key;
    einsums::string_util::split(split_key, key, einsums::string_util::is_any_of("."));

    std::string sk = split_key.back();
    split_key.pop_back();

    section const *cur_section = this;
    for (string_vector::const_iterator iter = split_key.begin(), end = split_key.end(); iter != end; ++iter) {
        section_map::const_iterator next = cur_section->_sections.find(*iter);
        if (cur_section->_sections.end() == next)
            return expand(l, default_val);
        cur_section = &next->second;
    }

    entry_map::const_iterator entry = cur_section->_entries.find(sk);
    if (cur_section->_entries.end() == entry)
        return expand(l, default_val);
...
void section::dump(int ind, std::ostream &strm) const {
    ...
    entry_map::const_iterator eend = _entries.end();
    for (entry_map::const_iterator i = _entries.begin(); i != eend; ++i) {
        indent(ind, strm);
    ...
    section_map::const_iterator send = _sections.end();
    for (section_map::const_iterator i = _sections.begin(); i != send; ++i) {
        indent(ind, strm);
        strm << "[" << i->first << "]\n";
...
void section::merge(section &second) {
    ...
    for (entry_map::const_iterator i = s_entries.begin(); i != end; ++i)
        _entries[i->first] = i->second;

    // merge subsection known in first section
    section_map::iterator send = _sections.end();
    for (section_map::iterator i = _sections.begin(); i != send; ++i) {
        // is there something to merge with?
        if (second.has_section(l, i->first))
            i->second.merge(second._sections[i->first]);
    }
...
/////////////////////////////////////////////////////////////////////////////////
void section::line_msg(std::string msg, std::string const &file, int lnum, std::string const &line) {
    msg += " " + file;
    if (lnum > 0)
    ...
    EINSUMS_THROW_EXCEPTION(einsums::error::no_success, "section::line_msg", "{}", msg);
}
...
///////////////////////////////////////////////////////////////////////////////
void section::expand(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin) const {
...
void section::expand_bracket(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin) const {
    ...
    if (end != std::string::npos) {
        ...
        if (colon == std::string::npos) {
            value.replace(begin, end - begin + 1, _root->get_entry(l, to_expand, std::string("")));
        } else {
            value.replace(begin, end - begin + 1, _root->get_entry(l, to_expand.substr(0, colon), to_expand.substr(colon + 1)));
        }
    }
}

void section::expand_brace(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin) const {
    // expand all keys embedded inside this key
    expand(l, value, begin);
    ...
    if (end != std::string::npos) {
        ...
        if (colon == std::string::npos) {
            ...
            value.replace(begin, end - begin + 1, nullptr != env ? std::string(env) : to_expand.substr(colon + 1));
        }
    }
}
...
///////////////////////////////////////////////////////////////////////////////
void section::expand_only(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin,
...
void section::expand_bracket_only(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin,
                                  std::string const &expand_this) const {
    // expand all keys embedded inside this key
...
void section::expand_brace_only(std::unique_lock<mutex_type> &l, std::string &value, std::string::size_type begin,
                                std::string const &expand_this) const {
    // expand all keys embedded inside this key
    expand_only(l, value, begin, expand_this);
...
std::string section::expand_only(std::unique_lock<mutex_type> &l, std::string value, std::string const &expand_this) const {
...
void section::set_root(section *r, bool recursive) {
    ...
    if (recursive) {
        section_map::iterator send = _sections.end();
        for (section_map::iterator si = _sections.begin(); si != send; ++si)
