...
#include "einsums/core/NDigits.hpp"
#include "einsums/tensor/Types.hpp"
...
auto println(AType<T, Rank> const &A, TensorPrintOptions options = {}) ->
...
auto get_dim_ranges(TensorType<T, Rank> const &tensor, std::index_sequence<I...>) {
...
auto get_dim_ranges(TensorType<T, Rank> const &tensor) {
...
struct Tensor final : public detail::TensorBase<T, Rank> {
     ...
     */
    auto data() const -> T const * { return _data.data(); }
...
#undef OPERATOR
    ...
    auto strides() const noexcept -> auto const & { return _strides; }
...
#undef OPERATOR
    ...
    auto data() const -> T const * { return static_cast<T const *>(&_data[0]); }
    template <typename... MultiIndex>
...
struct DiskTensor final : public detail::TensorBase<T, Rank> {
    ...
    explicit DiskTensor(h5::fd_t &file, std::string name, Dims... dims)
        requires AllConvertible<size_t, Dims...>
        : _file{file}, _name{std::move(name)}, _dims{static_cast<size_t>(dims)...} {
        static_assert(Rank == sizeof...(dims), "Declared Rank does not match provided dims");
...
struct DiskView final : public detail::TensorBase<T, ViewRank> {
    ...
    operator Tensor<T, ViewRank> const &() const { return _tensor; }
