...
// By default, we return the address of the object which is used to invoke the trait
template <typename F, typename Enable = void>
struct get_function_address {
    static constexpr std::size_t call(F const &f) noexcept { return reinterpret_cast<std::size_t>(std::addressof(f)); }
};

// For global (and static) functions, we return the function address itself
template <typename R, typename... Ts>
struct get_function_address<R (*)(Ts...)> {
    static constexpr std::size_t call(R (*f)(Ts...)) noexcept { return reinterpret_cast<std::size_t>(f); }
};
...
#if defined(__GNUG__) && !defined(__INTEL_COMPILER) && (!defined(__NVCC__) || (__CUDACC_VER_MAJOR__ >= 8))
