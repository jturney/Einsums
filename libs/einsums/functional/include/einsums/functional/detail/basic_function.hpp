...
struct EINSUMS_EXPORT function_base {
    ...
    constexpr explicit function_base(function_base_vtable const *empty_vptr) noexcept : vptr(empty_vptr), object(nullptr), storage_init() {}

    function_base(function_base const &other, vtable const *empty_vtable);
    function_base(function_base &&other, vtable const *empty_vtable) noexcept;
    ~function_base();
...
class basic_function<R(Ts...), Copyable> : public function_base {
  ...
  public:
    ...
    basic_function(basic_function &&other) noexcept : base_type(std::move(other), get_empty_vtable()) {}
    ...
    EINSUMS_FORCEINLINE R operator()(Ts... vs) const {
  ...
  private:
    static constexpr vtable const *get_empty_vtable() noexcept { return detail::get_empty_function_vtable<R(Ts...)>(); }
