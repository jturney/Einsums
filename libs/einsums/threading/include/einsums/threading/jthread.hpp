...
class jthread {
  ...
  public:
    ...
    //          would be exceeded.
    //
    template <typename F, typename... Ts, typename Enable = typename std::enable_if<!std::is_same<std::decay_t<F>, jthread>::value>::type>
        ...
        : ssource_{} // initialize stop_source
          ...
          thread_{
              ...
              [](stop_token st, F &&f, Ts &&...ts) -> void {
                  ...
                  jthread::invoke(use_stop_token{}, std::forward<F>(f), std::move(st), std::forward<Ts>(ts)...);
              },
    ...
    [[nodiscard]] static unsigned int hardware_concurrency() { return einsums::threads::detail::hardware_concurrency(); }

  private:
    stop_source     ssource_;  // stop_source for started thread
    einsums::thread thread_{}; // started thread (if any)
};
