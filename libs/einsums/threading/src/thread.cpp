...
#include <einsums/modules/threading.hpp>
...
thread::thread() noexcept : id_(einsums::threads::detail::invalid_thread_id) {
}
...
thread &thread::operator=(thread &&rhs) noexcept {
    ...
    if (joinable_locked()) {
        l2.unlock();
        l.unlock();
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "thread::operator=", "destroying running thread");
    }
    id_     = rhs.id_;
    rhs.id_ = threads::detail::invalid_thread_id;
    return *this;
...
thread::~thread() {
    if (joinable()) {
        if (detail::thread_termination_handler) {
            try {
                EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "thread::~thread", "destroying running thread");
            } catch (...) {
    ...
    EINSUMS_ASSERT(id_ == threads::detail::invalid_thread_id);
}
...
static void run_thread_exit_callbacks() {
    threads::detail::thread_id_type id = einsums::threads::detail::get_self_id();
    if (id == threads::detail::invalid_thread_id) {
        EINSUMS_THROW_EXCEPTION(einsums::error::null_thread_id, "run_thread_exit_callbacks", "null thread id encountered");
    }
    threads::detail::run_thread_exit_callbacks(id);
    threads::detail::free_thread_exit_callbacks(id);
}

threads::detail::thread_result_type thread::thread_function_nullary(util::detail::unique_function<void()> const &func) {
    try {
        // Now notify our calling thread that we started execution.
        func();
    } catch (einsums::thread_interrupted const &) { //-V565
    ...
    return threads::detail::thread_result_type(threads::detail::thread_schedule_state::terminated, threads::detail::invalid_thread_id);
}
...
unsigned int thread::hardware_concurrency() noexcept {
    return einsums::threads::detail::hardware_concurrency();
}

void thread::start_thread(threads::detail::thread_pool_base *pool, util::detail::unique_function<void()> &&func) {
    EINSUMS_ASSERT(pool);

    threads::detail::thread_init_data data(util::detail::one_shot(util::detail::bind(&thread::thread_function_nullary, std::move(func))),
    ...
    if (ec) {
        EINSUMS_THROW_EXCEPTION(einsums::error::thread_resource_error, "Could not create thread");
        return;
    }
}
...
void thread::join() {
    ...
    if (!joinable_locked()) {
        l.unlock();
        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "trying to join a non joinable thread");
    }

    native_handle_type this_id = einsums::threads::detail::get_self_id();
    if (this_id == id_) {
        l.unlock();
        EINSUMS_THROW_EXCEPTION(einsums::error::thread_resource_error, "einsums::thread: trying joining itself");
        return;
    }
    this_thread::interruption_point();

    // register callback function to be called when thread exits
    if (threads::detail::add_thread_exit_callback(id_.noref(), util::detail::bind_front(&resume_thread, this_id))) {
...
void yield_to(thread::id id) noexcept {
    this_thread::suspend(threads::detail::thread_schedule_state::pending, id.native_handle(), "this_thread::yield_to");
}

void yield() noexcept {
    this_thread::suspend(threads::detail::thread_schedule_state::pending, "this_thread::yield");
}
...
bool interruption_requested() {
    return threads::detail::get_thread_interruption_requested(einsums::threads::detail::get_self_id());
}
...
void sleep_until(einsums::chrono::steady_time_point const &abs_time) {
...
///////////////////////////////////////////////////////////////////////
disable_interruption::disable_interruption() : interruption_was_enabled_(interruption_enabled()) {
    if (interruption_was_enabled_) {
        interruption_was_enabled_ = threads::detail::set_thread_interruption_enabled(einsums::threads::detail::get_self_id(), false);
    }
}
...
disable_interruption::~disable_interruption() {
    ...
    if (p) {
        threads::detail::set_thread_interruption_enabled(einsums::threads::detail::get_self_id(), interruption_was_enabled_);
    }
}

///////////////////////////////////////////////////////////////////////
restore_interruption::restore_interruption(disable_interruption &d) : interruption_was_enabled_(d.interruption_was_enabled_) {
    if (!interruption_was_enabled_) {
        interruption_was_enabled_ = threads::detail::set_thread_interruption_enabled(einsums::threads::detail::get_self_id(), true);
    }
}
