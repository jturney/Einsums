...
#    include <einsums/datastructures/small_vector.hpp>
...
namespace einsums::split_detail {
...
struct value_visitor {
    ...
    void operator()(Ts const &ts) {
        std::apply(einsums::util::detail::bind_front(einsums::execution::experimental::set_value, std::move(receiver)), ts);
    }
};
...
struct split_sender_impl<Sender, Allocator>::split_sender_type {
    ...
    struct value_types_helper {
        using type = einsums::util::detail::transform_t<Tuple, add_const_lvalue_reference>;
    };
    ...
    struct shared_state {
        ...
        using operation_state_type = std::decay_t<einsums::execution::experimental::connect_result_t<Sender, split_receiver>>;
        ...
        using value_type = einsums::util::detail::transform_t<
            typename einsums::execution::experimental::sender_traits<Sender>::template value_types<std::tuple, einsums::detail::variant>,
            value_type_helper>;
        using error_type = einsums::util::detail::unique_t<
            einsums::util::detail::prepend_t<einsums::util::detail::transform_t<typename einsums::execution::experimental::sender_traits<
                                                                                    Sender>::template error_types<einsums::detail::variant>,
                                                                                std::decay>,
                                             std::exception_ptr>>;
        ...
        einsums::detail::small_vector<continuation_type, 1> continuations;

        struct split_receiver {
            einsums::intrusive_ptr<shared_state> state;

            template <typename Error>
            friend void tag_invoke(einsums::execution::experimental::set_error_t, split_receiver r, Error &&error) noexcept {
                r.state->v.template emplace<error_type>(error_type(std::forward<Error>(error)));
                r.state->set_predecessor_done();
            }

            friend void tag_invoke(einsums::execution::experimental::set_stopped_t, split_receiver r) noexcept {
                r.state->set_predecessor_done();
            };
            ...
            using value_type = einsums::util::detail::transform_t<typename einsums::execution::experimental::sender_traits<
                                                                      Sender>::template value_types<std::tuple, einsums::detail::variant>,
                                                                  value_type_helper>;

            template <typename... Ts>
            friend auto tag_invoke(einsums::execution::experimental::set_value_t, split_receiver r, Ts &&...ts) noexcept
        ...
        shared_state(Sender_ &&sender, allocator_type const &alloc) : alloc(alloc) {
            os.emplace(einsums::detail::with_result_of(
                [&]() { return einsums::execution::experimental::connect(std::forward<Sender_>(sender), split_receiver{this}); }));
        }

        ~shared_state() {
            EINSUMS_ASSERT_MSG(start_called, "start was never called on the operation state of split. Did you forget to "
        ...
        struct stopped_error_value_visitor {
            ...
            void operator()(value_type const &ts) { einsums::detail::visit(value_visitor<Receiver>{receiver}, ts); }
        };
        ...
        void add_continuation(Receiver &receiver) {
            if (predecessor_done) {
                ...
                einsums::detail::visit(stopped_error_value_visitor<Receiver>{receiver}, v);
            } else {
                ...
                if (predecessor_done) {
                    ...
                    continuations.emplace_back(
                        [this, &receiver]() mutable { einsums::detail::visit(stopped_error_value_visitor<Receiver>{receiver}, v); });
                }
            }
        }
    ...
    template <typename Receiver>
    friend operation_state<Receiver> tag_invoke(einsums::execution::experimental::connect_t, split_sender_type &&s, Receiver &&receiver) {
        return {std::forward<Receiver>(receiver), std::move(s.state)};
    }

    template <typename Receiver>
    friend operation_state<Receiver> tag_invoke(einsums::execution::experimental::connect_t, split_sender_type const &s,
                                                Receiver &&receiver) {
...
struct is_split_sender_impl<Sender, std::void_t<typename Sender::split_sender_tag>> : std::true_type {};
...
inline constexpr struct split_t final : einsums::functional::detail::tag_fallback<split_t> {
  private:
    template <typename Sender>
        requires(is_sender_v<Sender> && !split_detail::is_split_sender_v<Sender>)
    friend constexpr EINSUMS_FORCEINLINE auto tag_fallback_invoke(split_t, Sender &&sender) {
        return split_detail::split_sender<Sender, einsums::detail::internal_allocator<>>{std::forward<Sender>(sender), {}};
    }

    template <typename Sender, typename Allocator>
        requires(is_sender_v<Sender> && !split_detail::is_split_sender_v<Sender> && einsums::detail::is_allocator_v<Allocator>)
    friend constexpr EINSUMS_FORCEINLINE auto tag_fallback_invoke(split_t, Sender &&sender, Allocator const &allocator) {
        return split_detail::split_sender<Sender, Allocator>{std::forward<Sender>(sender), allocator};
    }

    template <typename Sender, typename Allocator>
        requires(split_detail::is_split_sender_v<Sender> && std::is_same_v<typename Sender::allocator_type, std::decay_t<Allocator>>)
    friend constexpr EINSUMS_FORCEINLINE auto tag_fallback_invoke(split_t, Sender &&sender, Allocator const &) {
    ...
    template <typename Sender>
        requires(split_detail::is_split_sender_v<Sender>)
    friend constexpr EINSUMS_FORCEINLINE auto tag_fallback_invoke(split_t, Sender &&sender) {
        return std::forward<Sender>(sender);
    }
    ...
    friend constexpr EINSUMS_FORCEINLINE auto tag_fallback_invoke(split_t, Allocator const &allocator = {}) {
