...
#    include <einsums/timing/tick_counter.hpp>
#endif
...
class thread_queue {
  private:
    ...
    using thread_heap_type =
        std::vector<threads::detail::thread_id_type, einsums::detail::internal_allocator<threads::detail::thread_id_type>>;
    ...
    using terminated_items_type = typename TerminatedQueuing::template apply<threads::detail::thread_data *>::type;
#endif

  protected:
    template <typename Lock>
    void create_thread_object(threads::detail::thread_id_ref_type &thrd, threads::detail::thread_init_data &data, Lock &lk) {
        EINSUMS_ASSERT(lk.owns_lock());
...
#    endif
#endif
        {
            einsums::detail::unlock_guard<Lock> ull(lk);
            ...
            thrd = threads::detail::thread_id_ref_type(p, threads::detail::thread_id_addref::no);
        }
    }

    static einsums::detail::internal_allocator<task_description> task_description_alloc_;
    ...
    std::size_t add_new(std::int64_t add_count, thread_queue *addfrom, std::unique_lock<mutex_type> &lk, bool steal = false) {
        ...
        if (EINSUMS_UNLIKELY(0 == add_count))
            return 0;
        ...
        while (add_count-- && addfrom->new_tasks_.pop(task, steal)) {
            ...
            if (get_maintain_queue_wait_times_enabled()) {
                ...
                addfrom->new_tasks_wait_ +=
                    duration<std::uint64_t, std::nano>(high_resolution_clock::now().time_since_epoch()).count() - task->waittime;
            ...
            [[maybe_unused]] bool schedule_now = data.initial_state == threads::detail::thread_schedule_state::pending;
            ...
            if (EINSUMS_UNLIKELY(!p.second)) {
                ...
                lk.unlock();
                EINSUMS_THROW_EXCEPTION(einsums::error::out_of_memory, "thread_queue::add_new",
        ...
        if (added) {
            EINSUMS_LOG(debug, "add_new: added {} tasks to queues", added);
        }
        return added;
    }
    ...
    bool add_new_always(std::size_t &added, thread_queue *addfrom, std::unique_lock<mutex_type> &lk, bool steal = false) {
        ...
        if (EINSUMS_LIKELY(parameters_.max_thread_count_)) {
            std::int64_t count = static_cast<std::int64_t>(thread_map_.size());
            if (parameters_.max_thread_count_ >= count + parameters_.min_add_new_count_) { //-V104
                EINSUMS_ASSERT(parameters_.max_thread_count_ - count < (std::numeric_limits<std::int64_t>::max)());
...
#    endif
        ...
        if (delete_all) {
            ...
            while (terminated_items_.pop(todelete)) {
                ...
                if (thread_map_.erase(tid) != 0) {
                    recycle_thread(tid);
                    --thread_map_count_;
                    EINSUMS_ASSERT(thread_map_count_ >= 0);
                }
            }
        } else {
            // delete only this many threads
            std::int64_t delete_count = (std::min)(static_cast<std::int64_t>(terminated_items_count_ / 10),
                                                   static_cast<std::int64_t>(parameters_.max_delete_count_));

            // delete at least this many threads
            delete_count = (std::max)(delete_count, static_cast<std::int64_t>(parameters_.min_delete_count_));
  ...
  public:
    ...
    thread_queue(std::size_t queue_num = std::size_t(-1), thread_queue_init_parameters parameters = {})
        : parameters_(parameters), thread_map_count_(0), work_items_(128, queue_num)
#ifdef EINSUMS_HAVE_THREAD_QUEUE_WAITTIME
          ,
          work_items_wait_(0), work_items_wait_count_(0)
#endif
#ifdef EINSUMS_HAVE_THREAD_STACK_MMAP
          ,
          terminated_items_(128), terminated_items_count_(0)
#endif
          ,
          new_tasks_(128)
#ifdef EINSUMS_HAVE_THREAD_QUEUE_WAITTIME
          ,
          new_tasks_wait_(0), new_tasks_wait_count_(0)
#endif
          ,
          thread_heap_small_(), thread_heap_medium_(), thread_heap_large_(), thread_heap_huge_(), thread_heap_nostack_()
#ifdef EINSUMS_HAVE_THREAD_CREATION_AND_CLEANUP_RATES
          ,
          add_new_time_(0), cleanup_terminated_time_(0)
#endif
#ifdef EINSUMS_HAVE_THREAD_STEALING_COUNTS
          ,
          pending_misses_(0), pending_accesses_(0), stolen_from_pending_(0), stolen_from_staged_(0), stolen_to_pending_(0),
          stolen_to_staged_(0)
#endif
    {
...
#ifdef EINSUMS_HAVE_THREAD_CREATION_AND_CLEANUP_RATES
    std::uint64_t get_creation_time(bool reset) { return ::einsums::detail::get_and_reset_value(add_new_time_, reset); }

    std::uint64_t get_cleanup_time(bool reset) { return ::einsums::detail::get_and_reset_value(cleanup_terminated_time_, reset); }
#endif
...
#ifdef EINSUMS_HAVE_THREAD_STEALING_COUNTS
    std::int64_t get_num_pending_misses(bool reset) { return ::einsums::detail::get_and_reset_value(pending_misses_, reset); }

    void increment_num_pending_misses(std::size_t num = 1) { pending_misses_.fetch_add(num, std::memory_order_relaxed); }

    std::int64_t get_num_pending_accesses(bool reset) { return ::einsums::detail::get_and_reset_value(pending_accesses_, reset); }

    void increment_num_pending_accesses(std::size_t num = 1) { pending_accesses_.fetch_add(num, std::memory_order_relaxed); }

    std::int64_t get_num_stolen_from_pending(bool reset) { return ::einsums::detail::get_and_reset_value(stolen_from_pending_, reset); }

    void increment_num_stolen_from_pending(std::size_t num = 1) { stolen_from_pending_.fetch_add(num, std::memory_order_relaxed); }

    std::int64_t get_num_stolen_from_staged(bool reset) { return ::einsums::detail::get_and_reset_value(stolen_from_staged_, reset); }

    void increment_num_stolen_from_staged(std::size_t num = 1) { stolen_from_staged_.fetch_add(num, std::memory_order_relaxed); }

    std::int64_t get_num_stolen_to_pending(bool reset) { return ::einsums::detail::get_and_reset_value(stolen_to_pending_, reset); }

    void increment_num_stolen_to_pending(std::size_t num = 1) { stolen_to_pending_.fetch_add(num, std::memory_order_relaxed); }

    std::int64_t get_num_stolen_to_staged(bool reset) { return ::einsums::detail::get_and_reset_value(stolen_to_staged_, reset); }

    void increment_num_stolen_to_staged(std::size_t num = 1) { stolen_to_staged_.fetch_add(num, std::memory_order_relaxed); }
#else
    ...
    // pending
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *id, error_code &ec) {
        ...
        EINSUMS_ASSERT(data.stacksize != execution::thread_stacksize::current);

        if (data.run_now) {
            threads::detail::thread_id_ref_type thrd;

            // The mutex can not be locked while a new thread is getting
            // created, as it might have that the current einsums thread gets
            // suspended.
            {
                std::unique_lock<mutex_type> lk(mtx_);

                bool schedule_now = data.initial_state == threads::detail::thread_schedule_state::pending;
                ...
                if (EINSUMS_UNLIKELY(!p.second)) {
                    lk.unlock();
                    EINSUMS_THROWS_IF(ec, einsums::error::out_of_memory, "thread_queue::create_thread",
                ...
                EINSUMS_ASSERT(thread_map_.find(thrd.noref()) != thread_map_.end());
                EINSUMS_ASSERT(&threads::detail::get_thread_id_data(thrd)->get_queue<thread_queue>() == this);
        ...
        if (data.initial_state != threads::detail::thread_schedule_state::pending) {
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "thread_queue::create_thread",
...
#ifdef EINSUMS_HAVE_THREAD_QUEUE_WAITTIME
            if (get_maintain_queue_wait_times_enabled()) {
                std::uint64_t now = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    ...
    /// available
    bool get_next_thread(threads::detail::thread_id_ref_type &thrd, bool allow_stealing = false, bool steal = false) EINSUMS_HOT {
        ...
        if (0 != work_items_count && work_items_.pop(tdesc, steal)) {
            ...
            if (get_maintain_queue_wait_times_enabled()) {
                work_items_wait_ += std::chrono::high_resolution_clock::now().time_since_epoch().count() - tdesc->waittime;
                ++work_items_wait_count_;
            }

            thrd = EINSUMS_MOVE(tdesc->data);
            delete tdesc;
    ...
    void schedule_thread(threads::detail::thread_id_ref_type thrd, bool other_end = false) {
        ...
        using namespace std::chrono;
        work_items_.push(
            new thread_description{EINSUMS_MOVE(thrd),
                                   duration<std::uint64_t, std::nano>(high_resolution_clock::now().time_since_epoch()).count()},
            other_end);
#else
    ...
    void destroy_thread(threads::detail::thread_data *thrd) {
        EINSUMS_ASSERT(&thrd->get_queue<thread_queue>() == this);
    ...
    std::int64_t get_thread_count(threads::detail::thread_schedule_state state = threads::detail::thread_schedule_state::unknown) const {
    ...
    bool enumerate_threads(util::detail::function<bool(threads::detail::thread_id_type)> const &f,
        ...
        if (state == threads::detail::thread_schedule_state::terminated) {
            ...
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "thread_queue::iterate_threads",
    ...
    inline bool wait_or_add_new(bool, std::size_t &added, bool steal = false) EINSUMS_HOT {
        ...
        // stop running after all einsums threads have been terminated
        return !add_new_always(added, this, lk, steal);
    }

    inline bool wait_or_add_new(bool running, std::size_t &added, thread_queue *addfrom, bool steal = false) EINSUMS_HOT {
        ...
        if (0 == work_items_count_.data_.load(std::memory_order_relaxed)) {
            ...
            // this queue
            std::int64_t new_tasks_count = addfrom->new_tasks_count_.data_.load(std::memory_order_relaxed);
            ...
            if (running && !enough_threads) {
                if (new_tasks_count != 0) {
                    EINSUMS_LOG(debug,
                                ...
                                fmt::ptr(addfrom), fmt::ptr(this), new_tasks_count, parameters_.min_tasks_to_steal_staged_);
                }
            ...
            if (!added_new) {
                ...
                // remaining terminated einsums threads
    ...
    bool dump_suspended_threads(std::size_t num_thread, std::int64_t &idle_loop_count, bool running) {
        ...
        EINSUMS_UNUSED(running);
        return false;
#else
        if (get_deadlock_detection_enabled()) {
            std::lock_guard<mutex_type> lk(mtx_);
            return einsums::threads::detail::dump_suspended_threads(num_thread, thread_map_, idle_loop_count, running);
        }
        return false;
#endif
    }
    ...
    void on_start_thread(std::size_t /* num_thread */) {
        ...
        static_assert(execution::thread_stacksize::default_ == execution::thread_stacksize::small_,
        ...
        for (std::int64_t i = 0; i < parameters_.init_threads_count_; ++i) {
            ...
            threads::detail::thread_data *p = threads::detail::thread_data_stackful::create(init_data, this, parameters_.small_stacksize_,
  ...
  private:
    ...
    thread_map_type thread_map_; // mapping of thread id's to einsums-threads
    ...
    // sharing
    einsums::concurrency::detail::cache_line_data<std::atomic<std::int64_t>> new_tasks_count_;

    // count of active work items
    einsums::concurrency::detail::cache_line_data<std::atomic<std::int64_t>> work_items_count_;
};

///////////////////////////////////////////////////////////////////////////
template <typename Mutex, typename PendingQueuing, typename StagedQueuing, typename TerminatedQueuing>
einsums::detail::internal_allocator<typename thread_queue<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>::task_description>
    thread_queue<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>::task_description_alloc_;
} // namespace einsums::threads::detail
