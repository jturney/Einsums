...
static einsums::debug::detail::enable_print<QUEUE_HOLDER_NUMA_DEBUG> nq_deb("QH_NUMA");
}
...
struct queue_holder_numa {
    ...
    // ----------------------------------------------------------------
    inline bool get_next_thread_HP(std::size_t qidx, threads::detail::thread_id_ref_type &thrd, bool stealing, bool core_stealing) {
        ...
        for (std::size_t i = 0; i < num_queues_; ++i, q = fast_mod((qidx + i), num_queues_)) {
            if (queues_[q]->get_next_thread_HP(thrd, (stealing || (i > 0)), i == 0)) {
                // clang-format off
                    einsums::detail::nq_deb.debug(debug::detail::str<>("HP/BP get_next")
    ...
    // ----------------------------------------------------------------
    inline bool get_next_thread(std::size_t qidx, threads::detail::thread_id_ref_type &thrd, bool stealing, bool core_stealing) {
        // loop over queues and take one task,
        ...
        for (std::size_t i = 0; i < num_queues_; ++i, q = fast_mod((qidx + i), num_queues_)) {
            ...
            if (queues_[q]->get_next_thread(thrd, (stealing || (i > 0)))) {
                einsums::detail::nq_deb.debug(debug::detail::str<>("get_next"), "D", debug::detail::dec<2>(domain_), "Q",
                                              debug::detail::dec<3>(q), "Qidx", debug::detail::dec<3>(qidx),
                                              ((i == 0 && !stealing) ? "taken" : "stolen from"),
                                              typename ThreadQueue::queue_data_print(queues_[q]),
    ...
    bool add_new_HP(ThreadQueue *receiver, std::size_t qidx, std::size_t &added, bool stealing, bool allow_stealing) {
        ...
        for (std::size_t i = 0; i < num_queues_; ++i, q = fast_mod((qidx + i), num_queues_)) {
            ...
            if (added > 0) {
                // clang-format off
                    einsums::detail::nq_deb.debug(debug::detail::str<>("HP/BP add_new")
    ...
    bool add_new(ThreadQueue *receiver, std::size_t qidx, std::size_t &added, bool stealing, bool allow_stealing) {
        ...
        for (std::size_t i = 0; i < num_queues_; ++i, q = fast_mod((qidx + i), num_queues_)) {
            ...
            if (added > 0) {
                // clang-format off
                    einsums::detail::nq_deb.debug(debug::detail::str<>("add_new")
                         , "added", debug::detail::dec<>(added)
                         , "D", debug::detail::dec<2>(domain_)
                         , "Q",  debug::detail::dec<3>(q)
                         , "Qidx",  debug::detail::dec<3>(qidx)
    ...
    // ----------------------------------------------------------------
    inline std::int64_t get_thread_count(threads::detail::thread_schedule_state state    = threads::detail::thread_schedule_state::unknown,
  ...
  public:
    ...
    // ------------------------------------------------------------
    bool dump_suspended_threads(std::size_t /* num_thread */, std::int64_t & /* idle_loop_count */, bool /* running */) { return false; }
