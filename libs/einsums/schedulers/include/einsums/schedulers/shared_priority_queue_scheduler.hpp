...
#include <einsums/schedulers/queue_holder_numa.hpp>
#include <einsums/schedulers/queue_holder_thread.hpp>
#include <einsums/schedulers/thread_queue_mc.hpp>
...
struct core_ratios {
    ...
    core_ratios(std::size_t high_priority, std::size_t normal_priority, std::size_t low_priority)
        ...
        : high_priority(high_priority), normal_priority(normal_priority), low_priority(low_priority) {}
...
class shared_priority_queue_scheduler : public scheduler_base {
  public:
    ...
    struct init_parameter {
        init_parameter(std::size_t num_worker_threads, const core_ratios &cores_per_queue,
                       einsums::detail::affinity_data const &affinity_data, const thread_queue_init_parameters &thread_queue_init,
                       char const *description = "shared_priority_queue_scheduler")
            : num_worker_threads_(num_worker_threads), cores_per_queue_(cores_per_queue), thread_queue_init_(thread_queue_init),
              affinity_data_(affinity_data), description_(description) {}

        init_parameter(std::size_t num_worker_threads, const core_ratios &cores_per_queue,
                       einsums::detail::affinity_data const &affinity_data, char const *description)
            : num_worker_threads_(num_worker_threads), cores_per_queue_(cores_per_queue), thread_queue_init_(),
              affinity_data_(affinity_data), description_(description) {}

        std::size_t                           num_worker_threads_;
        core_ratios                           cores_per_queue_;
        thread_queue_init_parameters          thread_queue_init_;
        einsums::detail::affinity_data const &affinity_data_;
    ...
    explicit shared_priority_queue_scheduler(init_parameter const &init)
        : scheduler_base(init.num_worker_threads_, init.description_, init.thread_queue_init_),
          d_lookup_(einsums::threads::detail::hardware_concurrency()), q_lookup_(einsums::threads::detail::hardware_concurrency())
#ifdef EINSUMS_DETAIL_SHARED_PRIORITY_SCHEDULER_LINUX
          ,
          schedcpu_(einsums::threads::detail::hardware_concurrency())
#endif
          ,
          cores_per_queue_(init.cores_per_queue_), num_workers_(init.num_worker_threads_), num_domains_(1),
          affinity_data_(init.affinity_data_), queue_parameters_(init.thread_queue_init_), initialized_(false), debug_init_(false),
          thread_init_counter_(0) {
        set_scheduler_mode(scheduler_mode::default_mode);
        EINSUMS_ASSERT(num_workers_ != 0);
    }
    ...
    void set_scheduler_mode(scheduler_mode mode) override {
        ...
        // clang-format off
            ...
            EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("scheduler_mode")
    ...
    bool cleanup_terminated(bool delete_all) override {
#ifdef EINSUMS_HAVE_THREAD_STACK_MMAP
        using namespace einsums::debug::detail;
        static auto cleanup = spq_deb<3>.make_timer(1, str<>("Cleanup"), "Global version");
        EINSUMS_DETAIL_DP(spq_deb<3>, timed(cleanup));
        ...
        EINSUMS_DETAIL_DP(spq_deb<7>, debug(str<>("cleanup_terminated"), "v1", "D", dec<2>(domain_num), "Q", dec<3>(q_index), "thread_num",
                                            dec<3>(local_num)));

        return numa_holder_[domain_num].thread_queue(static_cast<std::size_t>(q_index))->cleanup_terminated(local_num, delete_all);
#else
        EINSUMS_UNUSED(delete_all);
        return true;
#endif
    }
    ...
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *thrd, error_code &ec) override {
        einsums::threads::detail::increment_global_activity_count();

        // safety check that task was created by this thread/scheduler
        EINSUMS_ASSERT(data.scheduler_base == this);
    ...
    template <typename T>
    // NOLINTBEGIN(bugprone-easily-swappable-parameters)
    bool steal_by_function(std::size_t domain, std::size_t q_index, bool steal_numa, bool steal_core, thread_holder_type *origin, T &var,
                           const char                                                                                   *prefix,
                           util::detail::function<bool(std::size_t, std::size_t, thread_holder_type *, T &, bool, bool)> operation_HP,
                           util::detail::function<bool(std::size_t, std::size_t, thread_holder_type *, T &, bool, bool)> operation)
    // NOLINTEND(bugprone-easily-swappable-parameters)
    {
        using namespace einsums::debug::detail;
        bool result;
        ...
        if (!steal_core) {
            ...
            if (result) {
                EINSUMS_DETAIL_DP(spq_deb<7>, debug(str<>(prefix), "local no stealing", "D", dec<2>(domain), "Q", dec<3>(q_index)));
                return result;
            }
        }
        // High priority tasks first
            ...
            for (std::size_t d = 0; d < num_domains_; ++d) {
                ...
                if (result) {
                    EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_high_priority_first BP/HP",
                                                        (d == 0 ? "taken" : "stolen"), "D", dec<2>(domain), "Q", dec<3>(q_index)));
                    return result;
                }
            ...
            for (std::size_t d = 0; d < num_domains_; ++d) {
                ...
                if (result) {
                    EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_high_priority_first NP/LP",
                                                        (d == 0 ? "taken" : "stolen"), "D", dec<2>(domain), "Q", dec<3>(q_index)));
                    return result;
                }
                // if no numa stealing, skip other domains
        ...
        } else /*scheduler_mode::steal_after_local*/
        {
            ...
            result = result || operation(domain, q_index, origin, var, false, false);
            if (result) {
                EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_after_local local taken", "D", dec<2>(domain),
                                                    "Q", dec<3>(q_index)));
                return result;
            }
            ...
            if (steal_core) {
                if (q_counts_[domain] > 1) {
                    ...
                    if (result) {
                        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_after_local this numa", "stolen", "D",
            ...
            if (steal_numa) {
                ...
                for (std::size_t d = 1; d < num_domains_; ++d) {
                    ...
                    if (result) {
                        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_after_local other numa BP/HP",
                                                            (d == 0 ? "taken" : "stolen"), "D", dec<2>(domain), "Q", dec<3>(q_index)));
                        return result;
                    }
                }
                ...
                for (std::size_t d = 1; d < num_domains_; ++d) {
                    ...
                    if (result) {
                        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>(prefix), "scheduler_mode::steal_after_local other numa NP/LP",
                                                            (d == 0 ? "taken" : "stolen"), "D", dec<2>(domain), "Q", dec<3>(q_index)));
                        return result;
                    }
                }
    ...
    virtual bool get_next_thread(std::size_t /*thread_num*/, bool /*running*/, threads::detail::thread_id_ref_type &thrd,
                                 bool enable_stealing) override {
        using namespace einsums::debug::detail;
        std::size_t this_thread = local_thread_number();
        EINSUMS_ASSERT(this_thread < num_workers_);

        static auto gnt = spq_deb<3>.make_timer(1, str<>("get_next_thread"));

        auto c = get_thread_count(threads::detail::thread_schedule_state::unknown, execution::thread_priority::default_, this_thread);

        EINSUMS_DETAIL_DP(spq_deb<3>, timed(gnt, c));
        ...
        //
        EINSUMS_DETAIL_DP(spq_deb<5>, timed(getnext, dec<>(this_thread)));

        auto get_next_thread_function_HP = [&](std::size_t domain, std::size_t q_index, thread_holder_type * /* receiver */,
                                               threads::detail::thread_id_ref_type &thrd, bool stealing, bool allow_stealing) {
            return numa_holder_[domain].get_next_thread_HP(q_index, thrd, stealing, allow_stealing);
        };

        auto get_next_thread_function = [&](std::size_t domain, std::size_t q_index, thread_holder_type * /* receiver */,
                                            threads::detail::thread_id_ref_type &thrd, bool stealing, bool allow_stealing) {
            return numa_holder_[domain].get_next_thread(q_index, thrd, stealing, allow_stealing);
        };
        ...
        bool result = steal_by_function<threads::detail::thread_id_ref_type>(domain, q_index, numa_stealing_, core_stealing_, nullptr, thrd,
    ...
    virtual bool wait_or_add_new(std::size_t /* thread_num */, bool /* running */, std::int64_t & /* idle_loop_count */,
    ...
    bool just_add_new(std::size_t &added) {
        ...
        auto add_new_function_HP = [&](std::size_t domain, std::size_t q_index, thread_holder_type *receiver, std::size_t &added,
                                       bool stealing, bool allow_stealing) {
            return numa_holder_[domain].add_new_HP(receiver, q_index, added, stealing, allow_stealing);
        };

        auto add_new_function = [&](std::size_t domain, std::size_t q_index, thread_holder_type *receiver, std::size_t &added,
                                    bool stealing, bool allow_stealing) {
            return numa_holder_[domain].add_new(receiver, q_index, added, stealing, allow_stealing);
        };
        ...
        EINSUMS_DETAIL_DP(spq_deb<5>, timed(w_or_add_n, "thread_num", this_thread, "q_index", q_index, "numa_stealing ", numa_stealing_,
                                            "core_stealing ", core_stealing_));

        bool added_tasks = steal_by_function<std::size_t>(domain, q_index, numa_stealing_, core_stealing_, receiver, added, "just_add_new",
    ...
    void schedule_work(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint, bool /*allow_fallback*/,
                       bool other_end, execution::thread_priority priority = execution::thread_priority::normal) {
        using namespace einsums::debug::detail;
        EINSUMS_ASSERT(get_thread_id_data(thrd)->get_scheduler_base() == this);
        ...
        case execution::thread_schedule_hint_mode::none: {
            // Create thread on this worker thread if possible
            EINSUMS_DETAIL_DP(spq_deb<5>, set(msg, "HINT_NONE  "));
            if (local_num == std::size_t(-1)) {
                // This is a task being injected from a thread on another
                // pool - we can schedule on any thread available
            ...
            } else if (!round_robin_) /*assign_parent*/
            {
                domain_num = d_lookup_[thread_num];
                q_index    = q_lookup_[thread_num];
                EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("schedule_thread"), "scheduler_mode::assign_work_thread_parent", "thread_num",
                                                    thread_num, threadinfo<threads::detail::thread_id_ref_type *>(&thrd)));
            } else /*(round_robin_)*/
            {
                domain_num = d_lookup_[thread_num];
                q_index    = q_lookup_[thread_num];
                thread_num = numa_holder_[domain_num].thread_queue(q_index)->worker_next(num_workers_);
                EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("schedule_thread"), "scheduler_mode::assign_work_round_robin", "thread_num",
                                                    thread_num, threadinfo<threads::detail::thread_id_ref_type *>(&thrd)));
            }
        ...
        case execution::thread_schedule_hint_mode::thread: {
            // @TODO. We should check that the thread num is valid
            // Create thread on requested worker thread
            EINSUMS_DETAIL_DP(spq_deb<5>, set(msg, "HINT_THREAD"));
            EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("schedule_thread"), "received HINT_THREAD", dec<3>(schedulehint.hint),
        ...
        case execution::thread_schedule_hint_mode::numa: {
            // Create thread on requested NUMA domain
            EINSUMS_DETAIL_DP(spq_deb<5>, set(msg, "HINT_NUMA  "));
            // TODO: This case does not handle suspended PUs.
        ...
        EINSUMS_DETAIL_DP(
            spq_deb<5>, debug(str<>("thread scheduled"), msg, "Thread", dec<3>(thread_num), "D", dec<2>(domain_num), "Q", dec<3>(q_index)));

        numa_holder_[domain_num].thread_queue(q_index)->schedule_thread(thrd, priority, other_end);
    }

    void schedule_thread(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint, bool allow_fallback,
    ...
    void schedule_thread_last(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint, bool allow_fallback,
                              execution::thread_priority priority = execution::thread_priority::normal) override {
        using namespace einsums::debug::detail;
        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("schedule_thread_last")));
    ...
    void destroy_thread(threads::detail::thread_data *thrd) override {
        using namespace einsums::debug::detail;
        EINSUMS_ASSERT(thrd->get_scheduler_base() == this);
        ...
        EINSUMS_DETAIL_DP(spq_deb<7>,
                          debug(str<>("destroy_thread"), "xthread", xthread, "task owned by", "D", dec<2>(d1), "Q", dec<3>(q1),
                                "this thread", "D", dec<2>(d2), "Q", dec<3>(q2), threadinfo<threads::detail::thread_data *>(thrd)));
        // the cleanup of a task should be done by the original owner
        // of the task, so return it to the queue it came from before it
        // was stolen
        thrd->get_queue<queue_holder_thread<thread_queue_type>>().destroy_thread(thrd, this_thread, xthread);

        einsums::threads::detail::decrement_global_activity_count();
    }
    ...
    std::int64_t get_queue_length(std::size_t thread_num = std::size_t(-1)) const override {
        using namespace einsums::debug::detail;
        EINSUMS_DETAIL_DP(spq_deb<3>, debug(str<>("get_queue_length"), "thread_num ", dec<>(thread_num)));

        EINSUMS_ASSERT(thread_num != std::size_t(-1));
        ...
        EINSUMS_DETAIL_DP(spq_deb<3>, debug(str<>("get_queue_length"), "thread_num ", dec<>(thread_num), dec<>(count)));
        return count;
    }
    ...
    //---------------------------------------------------------------------
    std::int64_t get_thread_count(threads::detail::thread_schedule_state state    = threads::detail::thread_schedule_state::unknown,
        ...
        if (thread_num != std::size_t(-1)) {
            std::size_t  domain_num = d_lookup_[thread_num];
            std::size_t  q_index    = q_lookup_[thread_num];
            std::int64_t count      = numa_holder_[domain_num].thread_queue(q_index)->get_thread_count(state, priority);
            EINSUMS_DETAIL_DP(spq_deb<6>, debug(str<>("get_thread_count"), "thread_num ", dec<3>(thread_num), "count ", dec<4>(count)));
            return count;
        } else {
            std::int64_t count = 0;
            for (std::size_t d = 0; d < num_domains_; ++d) {
    ...
    bool enumerate_threads(util::detail::function<bool(threads::detail::thread_id_type)> const &f,
                           threads::detail::thread_schedule_state state = threads::detail::thread_schedule_state::unknown) const override {
        bool result = true;

        using namespace einsums::debug::detail;
        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("enumerate_threads")));
    ...
    void on_start_thread(std::size_t local_thread) override {
        using namespace einsums::debug::detail;
        EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("start_thread"), "local_thread", local_thread));

        auto const &topo = ::einsums::threads::detail::get_topology();
        ...
        std::sort(locations.begin(), locations.end(), [](const dcp_tuple &a, const dcp_tuple &b) -> bool {
            return (std::get<0>(a) == std::get<0>(b))
                       ? ((std::get<1>(a) == std::get<1>(b)) ? (std::get<2>(a) < std::get<2>(b)) : (std::get<1>(a) < std::get<1>(b)))
        ...
        while (local_thread != std::get<3>(locations[thread_init_counter_])) {
            // std::thread because we cannot suspend einsums threads here
        ...
        for (auto &tup : locations) {
            ...
            if (previous_domain != domain) {
                ...
                if (numa_holder_[domain].size() == 0) {
                    numa_holder_[domain].init(static_cast<std::size_t>(domain), q_counts_[domain]);
                }
            }
            ...
            if (local_thread == local_id) {
                ...
                if (cores_per_queue_.high_priority > 0) {
                    if ((index % cores_per_queue_.high_priority) == 0) {
                        ...
                        EINSUMS_ASSERT(index != 0);
                        hp_queue = numa_holder_[domain].thread_queue(static_cast<std::size_t>(index - 1))->hp_queue_;
                    }
                }
                // Normal priority
                if ((index % cores_per_queue_.normal_priority) == 0) {
                    ...
                    // share the queue with our next lowest neighbor
                    EINSUMS_ASSERT(index != 0);
                    np_queue = numa_holder_[domain].thread_queue(static_cast<std::size_t>(index - 1))->np_queue_;
                }
                // Low priority
                if (cores_per_queue_.low_priority > 0) {
                    if ((index % cores_per_queue_.low_priority) == 0) {
                        ...
                        // share the queue with our next lowest neighbor
                        EINSUMS_ASSERT(index != 0);
                        lp_queue = numa_holder_[domain].thread_queue(static_cast<std::size_t>(index - 1))->lp_queue_;
                    }
                }

                EINSUMS_DETAIL_DP(spq_deb<5>, debug(str<>("thread holder"), "local_thread", local_thread, "domain", domain, "index", index,
                                                    "local_id", local_id, "owner_mask", owner_mask));

                thread_holder = new queue_holder_thread<thread_queue_type>(
                    bp_queue, hp_queue, np_queue, lp_queue, static_cast<std::size_t>(domain), static_cast<std::size_t>(index),
                    static_cast<std::size_t>(local_id), static_cast<std::size_t>(owner_mask), queue_parameters_,
        ...
        while (thread_init_counter_ < num_workers_) {
            // std::thread because we cannot suspend einsums threads yet
            std::this_thread::yield();
        }
  ...
  protected:
    ...
    std::array<std::size_t, EINSUMS_HAVE_MAX_NUMA_DOMAIN_COUNT> q_counts_;
    // index of first queue on each nume domain
    std::array<std::size_t, EINSUMS_HAVE_MAX_NUMA_DOMAIN_COUNT> q_offset_;
    // one item per numa domain of a container for queues on that domain
    std::array<numa_queues, EINSUMS_HAVE_MAX_NUMA_DOMAIN_COUNT> numa_holder_;
...
struct fmt::formatter<einsums::threads::detail::shared_priority_queue_scheduler<Mutex>>
...
#if defined EINSUMS_DETAIL_SHARED_PRIORITY_SCHEDULER_LINUX
#    undef EINSUMS_DETAIL_SHARED_PRIORITY_SCHEDULER_LINUX
#endif
