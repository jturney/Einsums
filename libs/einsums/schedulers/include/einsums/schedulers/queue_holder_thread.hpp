...
static einsums::debug::detail::enable_print<QUEUE_HOLDER_THREAD_DEBUG> tq_deb("QH_THRD");
}
...
EINSUMS_FORCEINLINE std::size_t fast_mod(std::size_t const input, std::size_t const ceil) {
...
struct queue_holder_thread {
    ...
    // we must use OS mutexes here because we cannot suspend a einsums
    ...
    mutable einsums::concurrency::detail::cache_line_data<mutex_type> thread_map_mtx_;

    // every thread maintains lists of free thread data objects
    // sorted by their stack sizes
    using thread_heap_type =
        std::list<threads::detail::thread_id_type, einsums::detail::internal_allocator<threads::detail::thread_id_type>>;
    ...
    mutable einsums::concurrency::detail::cache_line_data<std::tuple<std::size_t, std::size_t>> rollover_counters_;

    // ----------------------------------------------------------------
    // ----------------------------------------------------------------
    // ----------------------------------------------------------------

    static einsums::detail::internal_allocator<threads::detail::thread_data> thread_alloc_;
    ...
    using thread_map_type = std::unordered_set<threads::detail::thread_id_type, std::hash<threads::detail::thread_id_type>,
                                               std::equal_to<threads::detail::thread_id_type>,
                                               einsums::detail::internal_allocator<threads::detail::thread_id_type>>;
    thread_map_type thread_map_;

    mutable einsums::concurrency::detail::cache_line_data<std::atomic<std::int32_t>> thread_map_count_;
    ...
    mutable einsums::concurrency::detail::cache_line_data<std::atomic<std::int32_t>> terminated_items_count_;
#endif
    ...
    struct queue_data_print {
        ...
        friend std::ostream &operator<<(std::ostream &os, const queue_data_print &d) {
            os << "D " << debug::detail::dec<2>(d.q_->domain_index_) << " Q " << debug::detail::dec<3>(d.q_->queue_index_) << " TM "
               << debug::detail::dec<3>(d.q_->thread_map_count_.data_) << " [BP " << queue_mc_print(d.q_->bp_queue_) << "] [HP "
               << queue_mc_print(d.q_->hp_queue_) << "] [NP " << queue_mc_print(d.q_->np_queue_) << "] [LP "
    ...
    queue_holder_thread(QueueType *bp_queue, QueueType *hp_queue, QueueType *np_queue, QueueType *lp_queue, std::size_t domain,
                        std::size_t queue, std::size_t thread_num, std::size_t owner, const thread_queue_init_parameters &init,
                        std::thread::id owner_id)
        // NOLINTEND(bugprone-easily-swappable-parameters)
        : bp_queue_(bp_queue), hp_queue_(hp_queue), np_queue_(np_queue), lp_queue_(lp_queue), domain_index_(domain), queue_index_(queue),
          thread_num_(thread_num), owner_mask_(owner)
#ifdef EINSUMS_HAVE_THREAD_STACK_MMAP
          ,
          terminated_items_(max_thread_count)
#endif
          ,
          parameters_(init), owner_id_(owner_id) {
        rollover_counters_.data_ = std::make_tuple(queue_index_, round_robin_rollover);
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("construct"), "D", debug::detail::dec<2>(domain_index_), "Q",
    ...
    inline std::size_t worker_next(std::size_t const workers) const {
        ::einsums::detail::tq_deb.debug(
            debug::detail::str<>("worker_next"), "Rollover counter ", debug::detail::dec<4>(std::get<0>(rollover_counters_.data_)),
            debug::detail::dec<4>(std::get<1>(rollover_counters_.data_)), "workers", debug::detail::dec<4>(workers));
        if (--std::get<1>(rollover_counters_.data_) == 0) {
            std::get<1>(rollover_counters_.data_) = round_robin_rollover;
            std::get<0>(rollover_counters_.data_) = fast_mod(std::get<0>(rollover_counters_.data_) + 1, workers);
        }
        return std::get<0>(rollover_counters_.data_);
    }

    // ------------------------------------------------------------
    void schedule_thread(threads::detail::thread_id_ref_type thrd, execution::thread_priority priority, bool other_end = false) {
    ...
    bool cleanup_terminated(std::size_t thread_num, bool delete_all) {
        // clang-format off
            if (thread_num!=thread_num_) {
                ::einsums::detail::tq_deb.error(debug::detail::str<>("assertion fail")
        ...
        // clang-format on
        EINSUMS_ASSERT(thread_num == thread_num_);
        ...
        if (delete_all) {
            ...
            while (terminated_items_.pop(todelete)) {
                ...
                ::einsums::detail::tq_deb.debug(debug::detail::str<>("cleanup"), "delete", queue_data_print(this),
            ...
            std::int64_t delete_count = static_cast<std::int64_t>(terminated_items_count_.data_.load(std::memory_order_relaxed) / 2);

            ::einsums::detail::tq_deb.debug(debug::detail::str<>("cleanup"), "recycle", "delete_count",
            ...
            while (delete_count && terminated_items_.pop(todelete)) {
                ...
                ::einsums::detail::tq_deb.debug(debug::detail::str<>("cleanup"), "recycle", queue_data_print(this),
    ...
    // ----------------------------------------------------------------
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *tid, std::size_t thread_num,
                       error_code &ec) {
        EINSUMS_ASSERT((!data.run_now || (data.run_now && thread_num == thread_num_)));
        ...
        if (data.priority == execution::thread_priority::normal) {
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("create_thread "), queue_data_print(this),
            ...
            return bp_queue_->create_thread(data, tid, ec);
                   ...
                   (data.priority == execution::thread_priority::high || data.priority == execution::thread_priority::high_recursive ||
        ...
        ::einsums::detail::tq_deb.error(debug::detail::str<>("create_thread "), "priority?");
        std::terminate();
    }
    ...
    void create_thread_object(threads::detail::thread_id_ref_type &tid, threads::detail::thread_init_data &data) {
        EINSUMS_ASSERT(data.stacksize >= execution::thread_stacksize::minimal);
        EINSUMS_ASSERT(data.stacksize <= execution::thread_stacksize::nostack);
        ...
        if (data.initial_state == threads::detail::thread_schedule_state::pending_do_not_schedule ||
        ...
        if (!heap->empty()) {
            ...
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("create_thread_object"), "rebind", queue_data_print(this),
...
#endif
        {
            ...
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("create_thread_object"), "new", queue_data_print(this),
    ...
    void add_to_thread_map(threads::detail::thread_id_type tid) {
        ...
        if (/*EINSUMS_UNLIKELY*/ (!p.second)) {
            ...
            ::einsums::detail::tq_deb.error(debug::detail::str<>("map add"), "Couldn't add new thread to the thread map",
                                            queue_data_print(this), debug::detail::threadinfo<threads::detail::thread_id_type *>(&tid));

            lk.unlock();
            EINSUMS_THROW_EXCEPTION(einsums::error::out_of_memory, "queue_holder_thread::add_to_thread_map",
                                    "Couldn't add new thread to the thread map {}", map_size);
        }

        ++thread_map_count_.data_;

        ::einsums::detail::tq_deb.debug(debug::detail::str<>("map add"), queue_data_print(this),
                                        debug::detail::threadinfo<threads::detail::thread_id_type *>(&tid));

        // this thread has to be in the map now
        EINSUMS_ASSERT(thread_map_.find(tid) != thread_map_.end());
    }
    ...
    void remove_from_thread_map(threads::detail::thread_id_type tid, bool dealloc) {
        ...
        EINSUMS_ASSERT(thread_map_count_.data_ >= 0);

        bool deleted = thread_map_.erase(tid) != 0;
        EINSUMS_ASSERT(deleted);
        (void)deleted;

        ::einsums::detail::tq_deb.debug(debug::detail::str<>("map remove"), queue_data_print(this),
                                        debug::detail::threadinfo<threads::detail::thread_id_type *>(&tid));
    ...
    // ----------------------------------------------------------------
    bool get_next_thread_HP(threads::detail::thread_id_ref_type &thrd, bool stealing, bool check_new) EINSUMS_HOT {
        // only take from BP queue if we are not stealing
        if (!stealing && bp_queue_ && bp_queue_->get_next_thread(thrd, stealing, check_new)) {
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("next_thread_BP"), queue_data_print(this),
                                            debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&thrd),
        ...
        if (hp_queue_ && hp_queue_->get_next_thread(thrd, stealing, check_new)) {
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("get_next_thread_HP"), queue_data_print(this),
                                            debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&thrd),
    ...
    // ----------------------------------------------------------------
    bool get_next_thread(threads::detail::thread_id_ref_type &thrd, bool stealing) EINSUMS_HOT {
        if (np_queue_->get_next_thread(thrd, stealing)) {
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("next_thread_NP"), queue_data_print(this),
                                            debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&thrd),
        ...
        if (lp_queue_ && lp_queue_->get_next_thread(thrd, stealing)) {
            ::einsums::detail::tq_deb.debug(debug::detail::str<>("next_thread_LP"), queue_data_print(this),
                                            debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&thrd),
    ...
    std::size_t add_new(std::int64_t add_count, thread_holder_type *addfrom, bool stealing) {
        ...
        //
        static auto an_timed = ::einsums::detail::tq_deb.make_timer(1, debug::detail::str<>("add_new"));
        ::einsums::detail::tq_deb.timed(an_timed, "add", debug::detail::dec<3>(add_count), "owns bp, hp, np, lp", owns_bp_queue(),
                                        owns_hp_queue(), owns_np_queue(), owns_lp_queue(), "this", queue_data_print(this), "from",
                                        queue_data_print(addfrom));
        //
        return 0;
    }
    ...
    inline std::size_t get_thread_count_staged(execution::thread_priority priority) const {
        ...
        case execution::thread_priority::unknown: {
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "queue_holder_thread::get_thread_count_staged",
    ...
    inline std::size_t get_thread_count_pending(execution::thread_priority priority) const {
        ...
        default:
        case execution::thread_priority::unknown: {
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "queue_holder_thread::get_thread_count_pending",
                                    "unknown thread priority value (execution::thread_priority::unknown)");
        }
        }
        return 0;
    ...
    // ----------------------------------------------------------------
    inline std::size_t get_thread_count(threads::detail::thread_schedule_state state    = threads::detail::thread_schedule_state::unknown,
...
#ifdef EINSUMS_HAVE_THREAD_STACK_MMAP
            return thread_map_count_.data_.load(std::memory_order_relaxed) + get_thread_count_staged(priority) -
    ...
    void destroy_thread(threads::detail::thread_data *thrd, std::size_t thread_num, bool xthread) {
        // the thread must be destroyed by the same queue holder that created it
        EINSUMS_ASSERT(&thrd->get_queue<queue_holder_thread>() == this);
        //
#ifdef EINSUMS_HAVE_THREAD_STACK_MMAP
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("destroy"), "terminated_items push", "xthread", xthread,
                                        queue_data_print(this), debug::detail::threadinfo<threads::detail::thread_data *>(thrd));
...
#else
        EINSUMS_UNUSED(thread_num);
        EINSUMS_UNUSED(xthread);
    ...
    void abort_all_suspended_threads() {
        ...
        for (thread_map_type::iterator it = thread_map_.begin(); it != end; ++it) {
            if (threads::detail::get_thread_id_data(*it)->get_state().state() == threads::detail::thread_schedule_state::suspended) {
                threads::detail::get_thread_id_data(*it)->set_state(threads::detail::thread_schedule_state::pending,
                                                                    einsums::threads::detail::thread_restart_state::abort);
    ...
    bool enumerate_threads(util::detail::function<bool(threads::detail::thread_id_type)> const &f,
                           threads::detail::thread_schedule_state state = threads::detail::thread_schedule_state::unknown) const {
        ...
        if (state == threads::detail::thread_schedule_state::terminated) {
            ...
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "queue_holder_thread::iterate_threads",
    ...
    // ------------------------------------------------------------
    void debug_info() {
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("details"), "owner_mask_", debug::detail::bin<8>(owner_mask_), "D",
                                        debug::detail::dec<2>(domain_index_), "Q", debug::detail::dec<3>(queue_index_));
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("bp_queue"), debug::detail::hex<12, void *>(bp_queue_), "holder",
                                        debug::detail::hex<12, void *>(bp_queue_->holder_ ? bp_queue_->holder_ : nullptr));
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("hp_queue"), debug::detail::hex<12, void *>(hp_queue_), "holder",
                                        debug::detail::hex<12, void *>(hp_queue_->holder_ ? hp_queue_->holder_ : nullptr));
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("np_queue"), debug::detail::hex<12, void *>(np_queue_), "holder",
                                        debug::detail::hex<12, void *>(np_queue_->holder_ ? np_queue_->holder_ : nullptr));
        ::einsums::detail::tq_deb.debug(debug::detail::str<>("lp_queue"), debug::detail::hex<12, void *>(lp_queue_), "holder",
                                        debug::detail::hex<12, void *>(lp_queue_->holder_ ? lp_queue_->holder_ : nullptr));
    }

    // ------------------------------------------------------------
    void debug_queues(const char *prefix) {
        static auto deb_queues = ::einsums::detail::tq_deb.make_timer(1, debug::detail::str<>("debug_queues"));
        //
        ::einsums::detail::tq_deb.timed(deb_queues, prefix, queue_data_print(this));
    }
};

template <typename QueueType>
einsums::detail::internal_allocator<threads::detail::thread_data> queue_holder_thread<QueueType>::thread_alloc_;
} // namespace einsums::threads::detail
