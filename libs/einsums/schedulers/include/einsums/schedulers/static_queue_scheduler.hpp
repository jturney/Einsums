...
class static_queue_scheduler : public local_queue_scheduler<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing> {
  public:
    using base_type = local_queue_scheduler<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>;
    ...
    /// available
    bool get_next_thread(std::size_t num_thread, bool, threads::detail::thread_id_ref_type &thrd, bool /*enable_stealing*/) override {
        using thread_queue_type = typename base_type::thread_queue_type;

        {
            EINSUMS_ASSERT(num_thread < this->queues_.size());
    ...
    bool wait_or_add_new(std::size_t num_thread, bool running, std::int64_t &idle_loop_count, bool /*enable_stealing*/,
        ...
        if (EINSUMS_UNLIKELY(get_deadlock_detection_enabled() && EINSUMS_LOG_ENABLED(err))) {
            ...
            for (std::size_t i = 0; suspended_only && i != this->queues_.size(); ++i) {
                suspended_only = this->queues_[i]->dump_suspended_threads(i, idle_loop_count, running);
            }

            if (EINSUMS_UNLIKELY(suspended_only)) {
                EINSUMS_LOG(warn, "queue({}): no new work available, are we deadlocked?", num_thread);
            }
        }
#else
...
struct fmt::formatter<einsums::threads::detail::static_queue_scheduler<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>>
