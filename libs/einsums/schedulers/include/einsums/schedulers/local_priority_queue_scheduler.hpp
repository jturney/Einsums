...
#include <einsums/schedulers/thread_queue.hpp>
...
class EINSUMS_EXPORT local_priority_queue_scheduler : public scheduler_base {
  public:
    using has_periodic_maintenance = std::false_type;

    using thread_queue_type = thread_queue<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>;
    ...
    struct init_parameter {
        ...
        init_parameter(std::size_t num_queues, einsums::detail::affinity_data const &affinity_data, char const *description)
            : num_queues_(num_queues), num_high_priority_queues_(num_queues), thread_queue_init_(), affinity_data_(affinity_data),
    ...
    local_priority_queue_scheduler(init_parameter_type const &init, bool deferred_initialization = true)
        : scheduler_base(init.num_queues_, init.description_, init.thread_queue_init_), curr_queue_(0), affinity_data_(init.affinity_data_),
          num_queues_(init.num_queues_), num_high_priority_queues_(init.num_high_priority_queues_),
          low_priority_queue_(init.num_queues_ - 1, _thread_queue_init), queues_(num_queues_), high_priority_queues_(num_queues_),
          victim_threads_(num_queues_) {
        if (!deferred_initialization) {
            EINSUMS_ASSERT(num_queues_ != 0);
            for (std::size_t i = 0; i != num_queues_; ++i) {
                queues_[i].data_ = new thread_queue_type(i, _thread_queue_init);
            }

            EINSUMS_ASSERT(num_high_priority_queues_ != 0);
            EINSUMS_ASSERT(num_high_priority_queues_ <= num_queues_);
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                high_priority_queues_[i].data_ = new thread_queue_type(i, _thread_queue_init);
            }
    ...
    std::int64_t get_num_pending_misses(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_pending_misses += high_priority_queues_[i].data_->get_num_pending_misses(reset);
            }
            for (std::size_t i = 0; i != num_queues_; ++i) {
        ...
        if (num_thread < num_high_priority_queues_) {
            num_pending_misses += high_priority_queues_[num_thread].data_->get_num_pending_misses(reset);
        }
    ...
    std::int64_t get_num_pending_accesses(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_pending_accesses += high_priority_queues_[i].data_->get_num_pending_accesses(reset);
            }
            for (std::size_t i = 0; i != num_queues_; ++i) {
        ...
        if (num_thread < num_high_priority_queues_) {
            num_pending_accesses += high_priority_queues_[num_thread].data_->get_num_pending_accesses(reset);
        }
        if (num_thread == num_queues_ - 1) {
            num_pending_accesses += low_priority_queue_.get_num_pending_accesses(reset);
        }
    ...
    std::int64_t get_num_stolen_from_pending(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_stolen_threads += high_priority_queues_[i].data_->get_num_stolen_from_pending(reset);
            }
            for (std::size_t i = 0; i != num_queues_; ++i) {
        ...
        if (num_thread < num_high_priority_queues_) {
            num_stolen_threads += high_priority_queues_[num_thread].data_->get_num_stolen_from_pending(reset);
        }
        if (num_thread == num_queues_ - 1) {
            num_stolen_threads += low_priority_queue_.get_num_stolen_from_pending(reset);
        }
    ...
    std::int64_t get_num_stolen_to_pending(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_stolen_threads += high_priority_queues_[i].data_->get_num_stolen_to_pending(reset);
            }
            for (std::size_t i = 0; i != num_queues_; ++i) {
        ...
        if (num_thread < num_high_priority_queues_) {
            num_stolen_threads += high_priority_queues_[num_thread].data_->get_num_stolen_to_pending(reset);
        }
        if (num_thread == num_queues_ - 1) {
            num_stolen_threads += low_priority_queue_.get_num_stolen_to_pending(reset);
        }
    ...
    std::int64_t get_num_stolen_from_staged(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_stolen_threads += high_priority_queues_[i].data_->get_num_stolen_from_staged(reset);
            }
        ...
        if (num_thread < num_high_priority_queues_) {
            num_stolen_threads += high_priority_queues_[num_thread].data_->get_num_stolen_from_staged(reset);
        }
        if (num_thread == num_queues_ - 1) {
            num_stolen_threads += low_priority_queue_.get_num_stolen_from_staged(reset);
        }
    ...
    std::int64_t get_num_stolen_to_staged(std::size_t num_thread, bool reset) override {
        ...
        if (num_thread == std::size_t(-1)) {
            for (std::size_t i = 0; i != num_high_priority_queues_; ++i) {
                num_stolen_threads += high_priority_queues_[i].data_->get_num_stolen_to_staged(reset);
            }
            for (std::size_t i = 0; i != num_queues_; ++i) {
        ...
        if (num_thread < num_high_priority_queues_) {
            num_stolen_threads += high_priority_queues_[num_thread].data_->get_num_stolen_to_staged(reset);
        }
        if (num_thread == num_queues_ - 1) {
            num_stolen_threads += low_priority_queue_.get_num_stolen_to_staged(reset);
        }
    ...
    bool cleanup_terminated(std::size_t num_thread, bool delete_all) override {
        ...
        if (num_thread < num_high_priority_queues_) {
            empty = high_priority_queues_[num_thread].data_->cleanup_terminated(delete_all) && empty;
        }
        return empty;
    }
    ...
    // pending
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *id, error_code &ec) override {
        einsums::threads::detail::increment_global_activity_count();

        // NOTE: This scheduler ignores NUMA hints.
        std::size_t num_thread =
            data.schedule_hint.mode == execution::thread_schedule_hint_mode::thread ? data.schedule_hint.hint : std::size_t(-1);
        ...
        data.schedule_hint.mode = execution::thread_schedule_hint_mode::thread;
        data.schedule_hint.hint = static_cast<std::int16_t>(num_thread);

        // now create the thread
        if (data.priority == execution::thread_priority::high_recursive || data.priority == execution::thread_priority::high ||
            ...
            EINSUMS_LOG(debug,
                        ...
                        "description({})",
                        *this->get_parent_pool(), *this, num, id ? *id : threads::detail::invalid_thread_id, data.priority,
        ...
        if (data.priority == execution::thread_priority::low) {
            ...
            EINSUMS_LOG(debug,
                        ...
                        *this->get_parent_pool(), *this, id ? *id : threads::detail::invalid_thread_id, data.priority,
                        data.get_description());

            return;
        }

        EINSUMS_ASSERT(num_thread < num_queues_);
        ...
        EINSUMS_LOG(debug,
                    ...
                    *this->get_parent_pool(), *this, num_thread, id ? *id : threads::detail::invalid_thread_id, data.priority,
                    data.get_description());
    }

    /// Return the next thread to be executed, return false if none is
    /// available
    bool get_next_thread(std::size_t num_thread, bool running, threads::detail::thread_id_ref_type &thrd, bool enable_stealing) override {
        EINSUMS_ASSERT(num_thread < num_queues_);
...

        {
            ...
            bool have_staged = this_queue->get_staged_queue_length(std::memory_order_relaxed) != 0;
...
#if !defined(EINSUMS_HAVE_THREAD_SANITIZER)
        if (enable_stealing) {
            for (std::size_t idx : victim_threads_[num_thread].data_) {
                EINSUMS_ASSERT(idx != num_thread);
...
#else
        EINSUMS_UNUSED(enable_stealing);
    ...
    void schedule_thread(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint,
                         bool allow_fallback = false, execution::thread_priority priority = execution::thread_priority::normal) override {
        // NOTE: This scheduler ignores NUMA hints.
        ...
        (void)thrdptr;
        if (priority == execution::thread_priority::high_recursive || priority == execution::thread_priority::high ||
            ...
            EINSUMS_LOG(debug,
                        ...
                        *this->get_parent_pool(), *this, num, thrdptr->get_thread_id(), priority, thrdptr->get_description());
            ...
            EINSUMS_LOG(debug,
                        ...
                        "pool({}), scheduler({}), thread({}), priority({}), description({})",
                        *this->get_parent_pool(), *this, thrdptr->get_thread_id(), priority, thrdptr->get_description());
            ...
            EINSUMS_LOG(debug,
                        ...
                        *this->get_parent_pool(), *this, num_thread, thrdptr->get_thread_id(), priority, thrdptr->get_description());

            queues_[num_thread].data_->schedule_thread(thrd);
        }
    }

    void schedule_thread_last(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint,
    ...
    std::int64_t get_thread_count(threads::detail::thread_schedule_state state    = threads::detail::thread_schedule_state::unknown,
        ...
        if (std::size_t(-1) != num_thread) {
            ...
            case execution::thread_priority::unknown: {
                EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "local_priority_queue_scheduler::get_thread_count",
    ...
    bool is_core_idle(std::size_t num_thread) const override {
        ...
        if (num_thread < num_high_priority_queues_ && high_priority_queues_[num_thread].data_->get_queue_length() != 0) {
            return false;
        }
        return true;
    }
    ...
    std::int64_t get_average_thread_wait_time(std::size_t num_thread = std::size_t(-1)) const override {
        ...
        if (std::size_t(-1) != num_thread) {
            ...
            if (num_thread < num_high_priority_queues_) {
                wait_time = high_priority_queues_[num_thread].data_->get_average_thread_wait_time();
                ++count;
            }
    ...
    std::int64_t get_average_task_wait_time(std::size_t num_thread = std::size_t(-1)) const override {
        ...
        if (std::size_t(-1) != num_thread) {
            ...
            if (num_thread < num_high_priority_queues_) {
                wait_time = high_priority_queues_[num_thread].data_->get_average_task_wait_time();
                ++count;
            }
    ...
    bool wait_or_add_new(std::size_t num_thread, bool running, std::int64_t &idle_loop_count, bool enable_stealing,
        ...
        if (enable_stealing) {
            for (std::size_t idx : victim_threads_[num_thread].data_) {
                ...
                if (idx < num_high_priority_queues_ && num_thread < num_high_priority_queues_) {
                    thread_queue_type *q = high_priority_queues_[idx].data_;
                    result               = this_high_priority_queue->wait_or_add_new(true, added, q) && result;
        ...
        if (EINSUMS_UNLIKELY(get_deadlock_detection_enabled() && EINSUMS_LOG_ENABLED(err))) {
            bool suspended_only = true;

            for (std::size_t i = 0; suspended_only && i != num_queues_; ++i) {
                suspended_only = queues_[i].data_->dump_suspended_threads(i, idle_loop_count, running);
            }

            if (EINSUMS_UNLIKELY(suspended_only)) {
                if (running) {
                    EINSUMS_LOG(err,
    ...
    void on_start_thread(std::size_t num_thread) override {
        einsums::threads::detail::set_local_thread_num_tss(num_thread);
        einsums::threads::detail::set_thread_pool_num_tss(_parent_pool->get_pool_id().index());

        if (nullptr == queues_[num_thread].data_) {
            queues_[num_thread].data_ = new thread_queue_type(num_thread, _thread_queue_init);

            if (num_thread < num_high_priority_queues_) {
                high_priority_queues_[num_thread].data_ = new thread_queue_type(num_thread, _thread_queue_init);
            }
        }
        ...
        std::vector<::einsums::threads::detail::mask_type> numa_masks(num_threads);
        std::vector<::einsums::threads::detail::mask_type> core_masks(num_threads);
        ...
        ::einsums::threads::detail::mask_cref_type pu_mask   = topo.get_thread_affinity_mask(num_pu);
        ::einsums::threads::detail::mask_cref_type numa_mask = numa_masks[num_thread];
        ::einsums::threads::detail::mask_cref_type core_mask = core_masks[num_thread];

        // we allow the thread on the boundary of the NUMA domain to steal
        ::einsums::threads::detail::mask_type first_mask = ::einsums::threads::detail::mask_type();
        ::einsums::threads::detail::resize(first_mask, ::einsums::threads::detail::mask_size(pu_mask));

        std::size_t first = ::einsums::threads::detail::find_first(numa_mask);
        if (first != std::size_t(-1))
            ::einsums::threads::detail::set(first_mask, first);
        else
            first_mask = pu_mask;

        auto iterate = [&](einsums::util::detail::function<bool(std::size_t)> f) {
            // check our neighbors in a radial fashion (left and right
            // alternating, increasing distance each iteration)
            std::ptrdiff_t i = 1;
            for (/**/; i < radius; ++i) {
                std::ptrdiff_t left = (static_cast<std::ptrdiff_t>(num_thread) - i) % static_cast<std::ptrdiff_t>(num_threads);
                if (left < 0)
        ...
        iterate([&](std::size_t other_num_thread) { return ::einsums::threads::detail::any(core_mask & core_masks[other_num_thread]); });

        // check for threads which share the same NUMA domain...
        iterate([&](std::size_t other_num_thread) {
            return !::einsums::threads::detail::any(core_mask & core_masks[other_num_thread]) &&
                   ::einsums::threads::detail::any(numa_mask & numa_masks[other_num_thread]);
        });

        // check for the rest and if we are NUMA aware
        if (has_scheduler_mode(scheduler_mode::enable_stealing_numa) && ::einsums::threads::detail::any(first_mask & pu_mask)) {
            iterate(
                [&](std::size_t other_num_thread) { return !::einsums::threads::detail::any(numa_mask & numa_masks[other_num_thread]); });
        }
    }
    ...
    void reset_thread_distribution() override { curr_queue_.store(0, std::memory_order_release); }
  ...
  protected:
    ...
    std::vector<einsums::concurrency::detail::cache_line_data<thread_queue_type *>>      queues_;
    std::vector<einsums::concurrency::detail::cache_line_data<thread_queue_type *>>      high_priority_queues_;
    std::vector<einsums::concurrency::detail::cache_line_data<std::vector<std::size_t>>> victim_threads_;
};
} // namespace einsums::threads::detail

template <typename Mutex, typename PendingQueuing, typename StagedQueuing, typename TerminatedQueuing>
struct fmt::formatter<einsums::threads::detail::local_priority_queue_scheduler<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>>
