...
static einsums::debug::detail::enable_print<false> tqmc_deb("_TQ_MC_");
}
...
class thread_queue_mc {
  ...
  public:
    ...
    std::size_t add_new(std::int64_t add_count, thread_queue_type *addfrom, bool stealing) {
        [[maybe_unused]] auto scp =
            ::einsums::detail::tqmc_deb.scope(debug::detail::ptr(this), __func__, "from", debug::detail::ptr(addfrom), "std::thread::id",
                                              debug::detail::hex<6>(holder_->owner_id_), stealing);
        EINSUMS_ASSERT(holder_->owner_id_ == std::this_thread::get_id());
        ...
        while (add_count-- && addfrom->new_task_items_.pop(task, stealing)) {
            ...
            ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("add_new"), "stealing", stealing,
                                              debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&tid));

            // insert the thread into work-items queue assuming it is in
            // pending state
            EINSUMS_ASSERT(data.initial_state == threads::detail::thread_schedule_state::pending);
  ...
  public:
    explicit thread_queue_mc(const thread_queue_init_parameters &parameters, std::size_t queue_num = std::size_t(-1))
        : parameters_(parameters), queue_index_(static_cast<int>(queue_num)), holder_(nullptr), new_task_items_(1024), work_items_(1024) {
    ...
    void set_holder(queue_holder_thread<thread_queue_type> *holder) {
        holder_ = holder;
        ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("set_holder"), "D", debug::detail::dec<2>(holder_->domain_index_), "Q",
    ...
    std::int64_t get_queue_length() const {
        return work_items_count_.data_.load(std::memory_order_relaxed) + new_tasks_count_.data_.load(std::memory_order_relaxed);
    }

    // ----------------------------------------------------------------
    // This returns the current length of the pending queue
    std::int64_t get_queue_length_pending() const { return work_items_count_.data_.load(std::memory_order_relaxed); }
    ...
    std::int64_t get_thread_count() const {
        EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "get_thread_count", "use get_queue_length_staged/get_queue_length_pending");
        return 0;
    }
    ...
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *id, error_code &ec) {
        ...
        if (data.initial_state != threads::detail::thread_schedule_state::pending) {
            EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "thread_queue_mc::create_thread",
    ...
    bool get_next_thread(threads::detail::thread_id_ref_type &thrd, bool other_end, bool check_new = false) EINSUMS_HOT {
        [[maybe_unused]] auto scp                    = ::einsums::detail::tqmc_deb.scope(debug::detail::ptr(this), __func__);
        std::int64_t          work_items_count_count = work_items_count_.data_.load(std::memory_order_relaxed);
        ...
        if (0 != work_items_count_count && work_items_.pop(thrd, other_end)) {
            ...
            ::einsums::detail::tqmc_deb.debug(
                debug::detail::str<>("get_next_thread"), "stealing", other_end, "D", debug::detail::dec<2>(holder_->domain_index_), "Q",
    ...
    void schedule_work(threads::detail::thread_id_ref_type thrd, bool other_end) {
        ++work_items_count_.data_;
        ::einsums::detail::tqmc_deb.debug(
            debug::detail::str<>("schedule_work"), "stealing", other_end, "D", debug::detail::dec<2>(holder_->domain_index_), "Q",
            debug::detail::dec<3>(queue_index_), "n", debug::detail::dec<4>(new_tasks_count_.data_), "w",
            debug::detail::dec<4>(work_items_count_.data_), debug::detail::threadinfo<threads::detail::thread_id_ref_type *>(&thrd));
        //
    ...
    void debug_queue(work_items_type &q) {
        ...
        ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("debug_queue"), "Pop work items");
        while (q.pop(thrd)) {
            ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("debug_queue"), x++,
                                              debug::detail::threadinfo<threads::detail::thread_data *>(thrd));
            work_items_copy_.push(thrd);
        }
        ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("debug_queue"), "Push work items");
        while (work_items_copy_.pop(thrd)) {
            q.push(thrd);
            ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("debug_queue"), --x,
                                              debug::detail::threadinfo<threads::detail::thread_data *>(thrd));
        }
        ::einsums::detail::tqmc_deb.debug(debug::detail::str<>("debug_queue"), "Finished");
    }
#endif
  ...
  public:
    ...
    einsums::concurrency::detail::cache_line_data<std::atomic<std::int32_t>> new_tasks_count_;
    einsums::concurrency::detail::cache_line_data<std::atomic<std::int32_t>> work_items_count_;
