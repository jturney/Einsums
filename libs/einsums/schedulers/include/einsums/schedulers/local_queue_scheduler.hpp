...
class EINSUMS_EXPORT local_queue_scheduler : public scheduler_base {
  public:
    ...
    struct init_parameter {
        init_parameter(std::size_t num_queues, einsums::detail::affinity_data const &affinity_data,
                       thread_queue_init_parameters thread_queue_init = {}, char const *description = "local_queue_scheduler")
            : num_queues_(num_queues), thread_queue_init_(thread_queue_init), affinity_data_(affinity_data), description_(description) {}

        init_parameter(std::size_t num_queues, einsums::detail::affinity_data const &affinity_data, char const *description)
            : num_queues_(num_queues), thread_queue_init_(), affinity_data_(affinity_data), description_(description) {}
    ...
    local_queue_scheduler(init_parameter_type const &init, bool deferred_initialization = true)
        : scheduler_base(init.num_queues_, init.description_, init.thread_queue_init_), queues_(init.num_queues_), curr_queue_(0),
          affinity_data_(init.affinity_data_), steals_in_numa_domain_(), steals_outside_numa_domain_(),
          numa_domain_masks_(init.num_queues_, ::einsums::threads::detail::get_topology().get_machine_affinity_mask()),
          outside_numa_domain_masks_(init.num_queues_, ::einsums::threads::detail::get_topology().get_machine_affinity_mask()),
          init_barrier(init.num_queues_) {
        ::einsums::threads::detail::resize(steals_in_numa_domain_, threads::detail::hardware_concurrency());
        ::einsums::threads::detail::resize(steals_outside_numa_domain_, threads::detail::hardware_concurrency());

        if (!deferred_initialization) {
            EINSUMS_ASSERT(init.num_queues_ != 0);
            for (std::size_t i = 0; i < init.num_queues_; ++i)
                queues_[i] = new thread_queue_type(i, _thread_queue_init);
        }
    }
    ...
    void create_thread(threads::detail::thread_init_data &data, threads::detail::thread_id_ref_type *id, error_code &ec) override {
        ...
        EINSUMS_ASSERT(num_thread < queue_size);
        ...
        EINSUMS_LOG(debug,
                    ...
                    *this->get_parent_pool(), *this, num_thread, id ? *id : threads::detail::invalid_thread_id, data.get_description());
    }

    /// Return the next thread to be executed, return false if none is
    /// available
    virtual bool get_next_thread(std::size_t num_thread, bool running, threads::detail::thread_id_ref_type &thrd,
                                 bool /*scheduler_mode::enable_stealing*/) override {
        [[maybe_unused]] std::size_t queues_size = queues_.size();

        {
            EINSUMS_ASSERT(num_thread < queues_size);
        ...
        if (!numa_stealing) {
            ...
            if (::einsums::threads::detail::test(steals_in_numa_domain_,
                                                 pu_number)) //-V600 //-V111
            {
                ::einsums::threads::detail::mask_cref_type this_numa_domain = numa_domain_masks_[num_thread];
                ...
                for (std::size_t i = 1; i != queues_size; ++i) {
                    ...
                    if (!::einsums::threads::detail::test(this_numa_domain,
            ...
            if (::einsums::threads::detail::test(steals_outside_numa_domain_,
                                                 pu_number)) //-V600 //-V111
            {
                ::einsums::threads::detail::mask_cref_type numa_domain = outside_numa_domain_masks_[num_thread];
                ...
                for (std::size_t i = 1; i != queues_size; ++i) {
                    ...
                    std::size_t pu_num = affinity_data_.get_pu_num(idx);
                    if (!::einsums::threads::detail::test(numa_domain,
                                                          pu_num)) //-V560 //-V600 //-V111
                        continue;
    ...
    void schedule_thread(threads::detail::thread_id_ref_type thrd, execution::thread_schedule_hint schedulehint, bool allow_fallback,
        ...
        EINSUMS_ASSERT(num_thread < queues_.size());

        EINSUMS_LOG(debug,
                    "local_queue_scheduler::schedule_thread: pool({}), scheduler({}), "
                    "worker_thread({}), thread({}), description({})",
                    *this->get_parent_pool(), *this, num_thread, get_thread_id_data(thrd)->get_thread_id(),
    ...
    std::int64_t get_thread_count(threads::detail::thread_schedule_state state    = threads::detail::thread_schedule_state::unknown,
        ...
        if (std::size_t(-1) != num_thread) {
            ...
            case execution::thread_priority::unknown: {
                EINSUMS_THROW_EXCEPTION(einsums::error::bad_parameter, "local_queue_scheduler::get_thread_count",
    ...
    bool is_core_idle(std::size_t num_thread) const override { return queues_[num_thread]->get_queue_length() == 0; }
    ...
    virtual bool wait_or_add_new(std::size_t num_thread, bool running, std::int64_t &idle_loop_count,
                                 bool /* scheduler_mode::enable_stealing */, std::size_t &added) override {
        ...
        if (!numa_stealing_) {
            ...
            if (::einsums::threads::detail::test(steals_in_numa_domain_,
                                                 pu_number)) //-V600 //-V111
            {
                ::einsums::threads::detail::mask_cref_type numa_domain_mask = numa_domain_masks_[num_thread];
                for (std::size_t i = 1; i != queues_size; ++i) {
                    // FIXME: Do a better job here.
                    std::size_t const idx = (i + num_thread) % queues_size;
                    ...
                    if (!::einsums::threads::detail::test(numa_domain_mask,
                                                          affinity_data_.get_pu_num(idx))) //-V600
                    {
                        continue;
                    }
                    result = queues_[num_thread]->wait_or_add_new(running, added, queues_[idx]) && result;
                    if (0 != added) {
            ...
            if (::einsums::threads::detail::test(steals_outside_numa_domain_,
                                                 pu_number)) //-V600 //-V111
            {
                ::einsums::threads::detail::mask_cref_type numa_domain_mask = outside_numa_domain_masks_[num_thread];
                for (std::size_t i = 1; i != queues_size; ++i) {
                    // FIXME: Do a better job here.
                    std::size_t const idx = (i + num_thread) % queues_size;
        ...
        if (EINSUMS_UNLIKELY(get_deadlock_detection_enabled() && EINSUMS_LOG_ENABLED(err))) {
            ...
            for (std::size_t i = 0; suspended_only && i != queues_.size(); ++i) {
                suspended_only = queues_[i]->dump_suspended_threads(i, idle_loop_count, running);
            }
    ...
    void on_start_thread(std::size_t num_thread) override {
        ...
        if (nullptr == queues_[num_thread]) {
            queues_[num_thread] = new thread_queue_type(num_thread, _thread_queue_init);
        }
        ...
        ::einsums::threads::detail::mask_cref_type machine_mask = topo.get_machine_affinity_mask();
        ::einsums::threads::detail::mask_cref_type core_mask    = topo.get_thread_affinity_mask(num_pu);
        ::einsums::threads::detail::mask_cref_type node_mask    = topo.get_numa_node_affinity_mask(num_pu);

        if (::einsums::threads::detail::any(core_mask) && ::einsums::threads::detail::any(node_mask)) {
            {
                std::lock_guard l{init_mtx};
                ::einsums::threads::detail::set(steals_in_numa_domain_, num_pu);
            }
        ...
        ::einsums::threads::detail::resize(first_mask, ::einsums::threads::detail::mask_size(core_mask));

        std::size_t first = ::einsums::threads::detail::find_first(node_mask);
        ...
        bool numa_stealing = has_scheduler_mode(scheduler_mode::enable_stealing_numa);
        if (numa_stealing && ::einsums::threads::detail::any(first_mask & core_mask)) {
            {
                std::lock_guard l{init_mtx};
                ::einsums::threads::detail::set(steals_outside_numa_domain_, num_pu);
            }

            outside_numa_domain_masks_[num_thread] = ::einsums::threads::detail::not_(node_mask) & machine_mask;
        }
    ...
    void on_stop_thread(std::size_t num_thread) override { queues_[num_thread]->on_stop_thread(num_thread); }

    void on_error(std::size_t num_thread, std::exception_ptr const &e) override { queues_[num_thread]->on_error(num_thread, e); }
  ...
  protected:
    ...
    ::einsums::threads::detail::mask_type              steals_in_numa_domain_;
    ::einsums::threads::detail::mask_type              steals_outside_numa_domain_;
    std::vector<::einsums::threads::detail::mask_type> numa_domain_masks_;
    std::vector<::einsums::threads::detail::mask_type> outside_numa_domain_masks_;

    einsums::concurrency::detail::barrier init_barrier;
    std::mutex                            init_mtx;
};
} // namespace einsums::threads::detail

template <typename Mutex, typename PendingQueuing, typename StagedQueuing, typename TerminatedQueuing>
struct fmt::formatter<einsums::threads::detail::local_queue_scheduler<Mutex, PendingQueuing, StagedQueuing, TerminatedQueuing>>
