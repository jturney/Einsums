...
lock_data::lock_data(std::size_t trace_depth)
    : ignore_(false), user_data_(nullptr)
#    ifdef EINSUMS_HAVE_VERIFY_LOCKS_BACKTRACE
      ,
      backtrace_(einsums::debug::detail::trace(trace_depth))
#    endif
{
#    ifndef EINSUMS_HAVE_VERIFY_LOCKS_BACKTRACE
    EINSUMS_UNUSED(trace_depth);
#    endif
}

lock_data::lock_data(register_lock_data *data, std::size_t trace_depth)
    : ignore_(false), user_data_(data)
#    ifdef EINSUMS_HAVE_VERIFY_LOCKS_BACKTRACE
      ,
      backtrace_(einsums::debug::detail::trace(trace_depth))
#    endif
...
struct held_locks_data_ptr {
    held_locks_data_ptr() : data_(new held_locks_data) {}
...
struct register_locks {
    ...
    static bool get_ignore_all_locks() { return !get_held_locks().data_->ignore_all_locks_; }

    static void set_ignore_all_locks(bool enable) { get_held_locks().data_->ignore_all_locks_ = enable; }
};

bool        register_locks::lock_detection_enabled_     = false;
std::size_t register_locks::lock_detection_trace_depth_ = EINSUMS_HAVE_THREAD_BACKTRACE_DEPTH;

struct reset_lock_enabled_on_exit {
    reset_lock_enabled_on_exit() : old_value_(register_locks::get_lock_enabled()) { register_locks::set_lock_enabled(false); }
...
bool register_lock(void const *lock, util::register_lock_data *data) {
    ...
    if (register_locks::lock_detection_enabled_ && (!register_locks_predicate || register_locks_predicate())) {
        ...
        if (!data) {
            p = held_locks.insert(std::make_pair(lock, detail::lock_data(register_locks::lock_detection_trace_depth_)));
        } else {
            p = held_locks.insert(std::make_pair(lock, detail::lock_data(data, register_locks::lock_detection_trace_depth_)));
        }
        return p.second;
    }
    return true;
...
// unregister the given lock from this einsums-thread
...
// verify that no locks are held by this einsums-thread
namespace detail {
...
void verify_no_locks() {
    ...
    if (enabled && register_locks::lock_detection_enabled_ && (!register_locks_predicate || register_locks_predicate())) {
        register_locks::held_locks_map &held_locks = register_locks::get_lock_map();
...
void force_error_on_lock() {
    ...
    //    // this OS-thread
    //    if (!held_locks.empty()) {
    //        EINSUMS_THROW_EXCEPTION(einsums::error::invalid_status, "force_error_on_lock",
