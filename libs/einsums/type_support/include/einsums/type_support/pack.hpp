...
struct make_index_pack_join<index_pack<Left...>, index_pack<Right...>> : index_pack<Left..., (sizeof...(Left) + Right)...> {};

template <typename... Ts>
using decayed_pack = pack<std::decay_t<Ts>...>;

#define EINSUMS_MAKE_INDEX_PACK_INTEGER_PACK                                                                                               \
    template <std::size_t N>                                                                                                               \
    struct make_index_pack : index_pack<__integer_pack(N)...> {};

#define EINSUMS_MAKE_INDEX_PACK_MAKE_INTEGER_SEQ                                                                                           \
    template <std::size_t N>                                                                                                               \
    struct make_index_pack : __make_integer_seq<pack_c, std::size_t, N> {};

#define EINSUMS_MAKE_INDEX_PACK_FALLBACK                                                                                                   \
    template <std::size_t N>                                                                                                               \
    struct make_index_pack : make_index_pack_join<typename make_index_pack<N / 2>::type, typename make_index_pack<N - N / 2>::type> {};

#if defined(__has_builtin)
#    if __has_builtin(__integer_pack)
EINSUMS_MAKE_INDEX_PACK_INTEGER_PACK
#    elif __has_builtin(__make_integer_seq)
EINSUMS_MAKE_INDEX_PACK_MAKE_INTEGER_SEQ
#    else
EINSUMS_MAKE_INDEX_PACK_FALLBACK
#    endif
#else
EINSUMS_MAKE_INDEX_PACK_FALLBACK
#endif

#undef EINSUMS_MAKE_INDEX_PACK_INTEGER_PACK
#undef EINSUMS_MAKE_INDEX_PACK_MAKE_INTEGER_SEQ
#undef EINSUMS_MAKE_INDEX_PACK_FALLBACK
...
#define EINSUMS_AT_INDEX_IMPL_TYPE_PACK_ELEMENT                                                                                            \
...
#define EINSUMS_AT_INDEX_IMPL_FALLBACK                                                                                                     \
    ...
    static indexed<J, T> at_index_check(indexed<J, T> const &);                                                                            \
                                                                                                                                           \
    template <std::size_t I, typename Ts>                                                                                                  \
    struct at_index_impl : decltype(detail::at_index_check<I>(indexer<Ts, typename make_index_pack<Ts::size>::type>())) {};

#if defined(__has_builtin)
#    if __has_builtin(__type_pack_element)
EINSUMS_AT_INDEX_IMPL_TYPE_PACK_ELEMENT
#    else
EINSUMS_AT_INDEX_IMPL_FALLBACK
#    endif
#else
EINSUMS_AT_INDEX_IMPL_FALLBACK
#endif

#undef EINSUMS_AT_INDEX_IMPL_TYPE_PACK_ELEMENT
#undef EINSUMS_AT_INDEX_IMPL_FALLBACK
...
struct unique_helper<Pack<Ts...>, Pack<U, Us...>> : std::conditional<contains<U, Ts...>::value, unique_helper<Pack<Ts...>, Pack<Us...>>,
...
template <template <typename...> class NewPack, template <typename...> class OldPack, typename... Ts>
