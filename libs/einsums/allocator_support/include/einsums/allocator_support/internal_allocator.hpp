...
struct internal_allocator {
    ...
    [[nodiscard]] pointer allocate(size_type n, void const *hint = nullptr) {
        ...
        pointer p = reinterpret_cast<pointer>(EINSUMS_PP_CAT(EINSUMS_HAVE_JEMALLOC_PREFIX, malloc)(n * sizeof(T)));
        if (p == nullptr) {
            throw std::bad_alloc();
        }
        return p;
    ...
    void deallocate(pointer p, size_type n) { EINSUMS_PP_CAT(EINSUMS_HAVE_JEMALLOC_PREFIX, free)(p); }
