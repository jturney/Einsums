...
#if defined(EINSUMS_HAVE_JEMALLOC_PREFIX)
...
inline void *__aligned_alloc(std::size_t alignment, std::size_t size) noexcept {
    return EINSUMS_PP_CAT(EINSUMS_HAVE_JEMALLOC_PREFIX, aligned_alloc)(alignment, size);
}

// NOLINTNEXTLINE(bugprone-reserved-identifier)
inline void __aligned_free(void *p) noexcept {
    return EINSUMS_PP_CAT(EINSUMS_HAVE_JEMALLOC_PREFIX, free)(p);
}

#elif defined(EINSUMS_HAVE_CXX17_STD_ALIGNED_ALLOC)
...
#elif defined(EINSUMS_HAVE_C11_ALIGNED_ALLOC)
...
#else // !EINSUMS_HAVE_CXX17_STD_ALIGNED_ALLOC && !EINSUMS_HAVE_C11_ALIGNED_ALLOC
