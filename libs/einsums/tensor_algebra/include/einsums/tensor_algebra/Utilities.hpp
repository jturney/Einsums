...
auto order_indices(std::tuple<Args...> const &combination, std::array<size_t, Rank> const &order, std::index_sequence<I...>) {
...
auto construct_index(std::tuple<TargetCombination...> const &target_combination, std::tuple<TargetPositionInC...> const &,
                     std::tuple<LinkCombination...> const   &link_combination, std::tuple<LinkPositionInLink...> const &) {
...
auto construct_index_from_unique_target_combination(std::tuple<UniqueTargetIndices...> const & /*unique_target_indices*/,
                                                    ...
                                                    std::tuple<TargetPositionInC...> const &,
...
template <typename... AIndices, typename... TargetCombination, typename... TargetPositionInC, typename... LinkCombination,
          typename... LinkPositionInLink>
constexpr auto construct_indices(std::tuple<AIndices...> const &, std::tuple<TargetCombination...> const &target_combination,
                                 std::tuple<TargetPositionInC...> const  &target_position_in_C,
...
constexpr auto product_dims(std::tuple<PositionsInX...> const &indices, XType<T, XRank> const &X, std::index_sequence<I...>) -> size_t {
    return (X.dim(std::get<2 * I + 1>(indices)) * ... * 1);
}

template <template <typename, size_t> typename XType, size_t XRank, typename... PositionsInX, std::size_t... I, typename T = double>
constexpr auto is_same_dims(std::tuple<PositionsInX...> const &indices, XType<T, XRank> const &X, std::index_sequence<I...>) -> bool {
