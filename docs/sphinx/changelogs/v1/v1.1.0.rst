...
* :cpp:any:`~einsums::TensorView`\ s can be made from :cpp:any:`~einsums::RuntimeTensor`\ s and :cpp:any:`~einsums::RuntimeTensorView`\ s
...
* The dispatching function for :cpp:func:`~einsums::tensor_algebra::einsum` is now more modular, so it is hopefully easier to maintain.
...
* ``sort`` is no longer a valid alias for :cpp:func:`~einsums::tensor_algebra::permute`.
