...
What's New
----------
* Tensor storage order can now be selected per tensor at construction. Tensors are column-major by default, so the
  first index varies fastest.

    * Pass the order ahead of the name to build one row-major tensor, as in
      ``Tensor<double, 2> A(/*row_major=*/true, "A", 3, 3);``. The order belongs to the tensor, so views and copies
      taken from it keep it.
