...
These are added in Python — not the C++ surface — so they are invisible to
...
.. py:class:: RuntimeTensor
    ...
    .. py:method:: sum()
        ...
        Sum of all elements (:py:func:`einsums.linalg.sum`). Eager returns a
        Python scalar; inside ``cg.capture`` returns a graph ``[1]`` tensor.
    ...
    .. py:method:: max()
        ...
        Maximum element (real dtypes only — complex has no natural ordering;
        use ``einsums.linalg.norm(Norm.MAXABS, A)`` for the largest magnitude).
    ...
    .. py:method:: __matmul__(other)
        ...
        two vectors (inner product) raise — use :py:func:`einsums.linalg.dot`.

    .. py:method:: __add__(other)

        Element-wise add. ``other`` may be another Einsums tensor or a scalar;
