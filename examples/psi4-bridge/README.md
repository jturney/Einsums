...
| `df_mp2_laplace.py` | The same DF-MP2 energy written over all four indices instead of pair by pair, so the denominator is a four-axis `1/(e_i + e_j - e_a - e_b)` and can be tagged for `LaplaceTransform`. The pass replaces it with a quadrature and pushes one exponential per axis onto the operands of the contraction that forms `(ia\|jb)`, which is the substitution SOS-MP2 is built on. At water/cc-pVDZ the energy lands inside every bound the pass records, at 16, 30 and 60 points for 1e-3, 1e-5 and 1e-8, and the emitted node count does not move with the point count. `--naive` offers the dense-integral form to the density fit and the transform and reports what each declines; `--response` shows the two composing on the orbital-response shape, where the integral is a contraction operand. Integrals come from psi4, or from the DLPNO fixture with `--fixture`. |
