...
#----------------------------------------------------------------------------------------------
"""The local MP2 iteration, as a solver over an explicit problem.
...
replayed to convergence (see :meth:`LMP2Solver.iterate`).
...
**Why this is a solver over an :class:`LMP2Problem` rather than methods on
DLPNOMP2.** psi4 runs the same iteration twice in a DLPNO-CCSD calculation:
once as ``DLPNOMP2::lmp2_iterations`` for the MP2 energy, and again inside
``DLPNOCCSD::pno_lmp2_iterations``, where it converges the amplitudes that the
strong/weak pair split and ``recompute_pnos`` are then derived from. psi4
duplicates ~800 lines to do it. The two differ only in which PNOs they run in
and what is done with the answer, so here they are one solver handed two
problems - which is also why the problem carries a :class:`~dlpno.layout.PairLayout`
rather than reading bucket fields off a calculation object: the CC run has two
...
__all__ = ["LMP2Problem", "LMP2Solver"]


@dataclass(frozen=True)
class LMP2Problem:
    """Everything the local MP2 iteration reads, as values.

    The pair list, the PNO set and the threshold bundle of design decision 2,
    plus the stores the iteration runs on. Nothing here is derived: a caller
    that wants the iteration in a different PNO basis builds a second problem
    rather than mutating a calculation object.
    """

    #: Number of active occupied LMOs.
    naocc: int
    #: Per surviving pair ordinal, its ``(i, j)``.
    ij_to_i_j: list
    #: ``naocc x naocc`` array of pair ordinals, ``-1`` where a pair was
    #: screened away.
    i_j_to_ij: object
    #: Per pair ordinal, the ordinal of its transpose.
    ij_to_ji: list

    #: Per pair, its PNO count; zero means the pair is dead.
    n_pno: list
    #: Per pair, its canonical PNO orbital energies.
    e_pno: list
    #: Per pair, the PAO-domain-to-PNO transform.
    X_pno: list

    #: Occupied Fock matrix in the LMO basis, ``(naocc, naocc)``.
    F_lmo: object
    #: PAO overlap matrix, ``(npao, npao)``.
    S_pao: object
    #: Per pair, the PAO indices its domain covers.
    lmopair_to_paos: list

    #: How the per-pair blocks are bucketed and padded.
    layout: PairLayout

    #: The exchange, amplitude and antisymmetrized-amplitude stores, one tensor
    #: per bucket each. The solver writes ``T_all`` and ``Tt_all`` in place.
    K_all: list
    T_all: list
    Tt_all: list

    #: Truncation and convergence parameters. The solver reads ``f_cut``,
    #: ``n_width_groups``, ``maxiter``, ``e_convergence``, ``r_convergence`` and
    #: ``diis_max_vecs``; the PNO cutoffs were spent building the problem.
    cut: object
...
class LMP2Solver:
    """The local MP2 equations as a ComputeGraph, over one :class:`LMP2Problem`."""

    def __init__(self, problem, use_diis=True, verbose=True):
        self.p = problem
        self.use_diis = use_diis
        self.verbose = verbose

        self.plan = None
        self._S_cls = self._S_T = None
        self.e_lmp2 = 0.0
        self.e_lmp2_os = 0.0
        self.e_lmp2_ss = 0.0
        self.n_iterations = 0
    ...
    def compute_overlaps(self):
        ...
        **The overlaps are pre-scaled by the square root of their Fock
        ...
        # every entry point, not only run_phases. Imported here rather than at
        # module scope because ``stages`` imports the solvers.
        from .stages import compute_pno_overlaps as overlap_stage

        if self.plan is None:
            self.plan_couplings()
        self.take_overlaps(overlap_stage(*self.overlap_args()))
        return self

    def overlap_args(self):
        """The contracted overlap stage's arguments, from the problem."""
        p = self.p
        return (p.X_pno, p.S_pao, p.lmopair_to_paos, p.n_pno,
                p.layout.bucket_of, p.layout.bucket_dims, self.plan)

    def take_overlaps(self, overlaps):
        """Accept the overlap stage's result, whichever backend produced it."""
        self._S_cls, self._S_T = overlaps.S_cls, overlaps.S_T
        self._print(
            f"  overlaps: {len(self.plan.pair)} couplings, "
            f"{len(self.plan.classes)} shape classes"
        )
        return self

    def plan_couplings(self):
        ...
        p = self.p
        naocc = p.naocc
        F_lmo = ten.view(p.F_lmo)
        f_cut = p.cut.f_cut

        # Which partners each pair couples to, in the order they are laid out.
        partners = [[] for _ in range(p.n_lmo_pairs)]  # (partner, k, is_ik, factor)
        for ij, (i, j) in enumerate(p.ij_to_i_j):
            if p.n_pno[ij] == 0:
                continue
            for k in range(naocc):
                kj = int(p.i_j_to_ij[k, j])
                if kj != -1 and i != k and abs(F_lmo[i, k]) > f_cut and p.n_pno[kj] > 0:
                    partners[ij].append((kj, k, False, -float(F_lmo[i, k])))
                ik = int(p.i_j_to_ij[i, k])
                if ik != -1 and j != k and abs(F_lmo[k, j]) > f_cut and p.n_pno[ik] > 0:
                    partners[ij].append((ik, k, True, -float(F_lmo[k, j])))

        self._n_partners = sum(len(q) for q in partners)
    ...
    def _plan_couplings(self, partners):
        ...
        * ``_cls_perm`` - for each destination slot in pair order, the source slot
        ...
        ``_cls_partner_groups``, ``_pair_slot``) stay on the solver: they are
        read by :meth:`iterate`, which is not being promoted.
        """
        bucket_of = self.p.layout.bucket_of
        n_groups = max(1, int(self.p.cut.n_width_groups))
        ...
        for ij, plist in enumerate(partners):
            ...
            by_cls = {}
            for entry in plist:
                by_cls.setdefault(bucket_of[entry[0]], []).append(entry)
            per_pair[ij] = by_cls

        counts = {}  # class -> {pair: how many couplings}
        for ij, by_cls in per_pair.items():
            for b_p, lst in by_cls.items():
                counts[bucket_of[ij], b_p] = counts.get((bucket_of[ij], b_p), {})
                counts[bucket_of[ij], b_p][ij] = len(lst)
        ...
        for ij in self._coupled_pairs:
            ...
            for b_p in sorted(by_cls):
                ci = self._cls_index[bucket_of[ij], b_p]
                for c, (q, k, is_ik, factor) in enumerate(by_cls[b_p]):
                    c_pair.append(ij)
                    c_partner.append(q)
                    c_cls.append(ci)
        ...
        for n, (q, ci) in enumerate(zip(c_partner, c_cls)):
            by_partner[ci].setdefault(q, []).append(n)
    ...
    def _allocate_iteration_tensors(self):
        ...
        self.R_all = layout.new_stores("R")
        ...
        self.D_all = layout.new_stores("D")
        for D in self.D_all:
            ten.view(D)[...] = 1.0
        for ij, (i, j) in enumerate(p.ij_to_i_j):
            n = p.n_pno[ij]
            ...
            la.outer_sum(D, [p.e_pno[ij], p.e_pno[ij]], [1.0, 1.0])
            la.shift(-float(F_lmo[i, i] + F_lmo[j, j]), D)
            layout.block(self.D_all, ij)[:n, :n] = ten.view(D)
        ...
        self.step_all = layout.new_stores("step")
        ...
        self._R_view = [layout.view(self.R_all, ij) if p.n_pno[ij] else None
                        for ij in range(P)]
        self._T_view = [layout.view(p.T_all, ij) if p.n_pno[ij] else None
                        for ij in range(P)]
        ...
        dims = layout.bucket_dims
        self._V = [
            ten.zeros(f"V (class {cls})",
                      [dims[cls[1]], dims[cls[0]], self._cls_slots_partner[ci]])
            for ci, cls in enumerate(self._classes)
        ]
        self._W = [
            ten.zeros(f"W (class {cls})",
                      [dims[cls[1]], self._cls_slots_pair[ci], dims[cls[0]]])
            for ci, cls in enumerate(self._classes)
        ]
        ...
        # freed memory.
        self._V_flat = [V.reshape_view([dims[cls[1]],
                                        dims[cls[0]] * self._cls_slots_partner[ci]])
                        for ci, (cls, V) in enumerate(zip(self._classes, self._V))]
        self._ST_flat = [S.reshape_view([dims[cls[1]],
                                         dims[cls[0]] * self._cls_slots_partner[ci]])
                         for ci, (cls, S) in enumerate(zip(self._classes, self._S_T))]
        self._W_flat = [W.reshape_view([dims[cls[1]] * self._cls_slots_pair[ci],
                                        dims[cls[0]]])
    ...
    def _emit_repack(self):
        """Record this phase's ops into the ambient capture."""
        dims = self.p.layout.bucket_dims
        for ci, cls in enumerate(self._classes):
            M_p, M_b = dims[cls[1]], dims[cls[0]]
    ...
    def _capture_residual(self):
        ...
        ``8e4174d48``, which is what made the fold in :meth:`iterate` possible.
        """
    ...
    def _emit_prologue(self):
        """Record this phase's ops into the ambient capture."""
        for D, T, R, K in zip(self.D_all, self.p.T_all, self.R_all, self.p.K_all):
    ...
    def _emit_energy(self):
        ...
        for K, R, Tt in zip(self.p.K_all, self.R_all, self.p.Tt_all):
    ...
    def _emit_step(self):
        ...
        """
        for R, D, S, T in zip(self.R_all, self.D_all, self.step_all, self.p.T_all):
    ...
    def _emit_antisymmetrize(self):
        """Record this phase's ops into the ambient capture."""
        for Tt, T in zip(self.p.Tt_all, self.p.T_all):
            einsums.permute("abp <- bap", Tt, T)
            la.axpby(2.0, T, -1.0, Tt)

    def iterate(self, optimize=True):
        ...
        def advance(iteration):
            ...
            r_curr = 0.0
            for ij, n in enumerate(p.n_pno):
                if n:
                    block = p.layout.block(self.R_all, ij)[:n, :n]
            ...
            state["converged"] = (
                iteration > 0
                and abs(state["curr"] - state["prev"]) < p.cut.e_convergence
                and abs(r_curr) < p.cut.r_convergence
            )
            return not state["converged"]

        if self.use_diis:
            acc = cg.diis(list(zip(p.T_all, self.step_all)), k=p.cut.diis_max_vecs)
            predicate = acc.wrap(advance)
        else:
            predicate = advance

        g = cg.Graph("lmp2")
        body = g.add_loop("lmp2 iteration", p.cut.maxiter + 1, predicate)
        ...
        # Leave Tt consistent with T. The body computes Tt at its TOP (see the
        # docstring), so on exit Tt lags T by the final Jacobi step and by
        # whatever DIIS did after it. Nothing in the MP2 path reads Tt again -
        # which is why this went unnoticed - but the coupled-cluster cascade
        # does: ``recompute_pnos`` builds its pair density from (T, Tt) and its
        # weak-pair energy from K . Tt, and a stale Tt there is worth 2e-6 Eh on
        # the water dimer, which reads as a PNO truncation difference rather
        # than as the bookkeeping slip it is.
        g_final = cg.Graph("lmp2 final antisymmetrization")
        with cg.capture(g_final):
            self._emit_antisymmetrize()
        g_final.execute()
        ...
        # sum over pairs.
        self.e_lmp2_os = sum(ten.vector_dot(K, T) for K, T in zip(p.K_all, p.T_all))
        ...
        skew = max(
            (np.abs(p.layout.block(p.T_all, ij)[:n, :n]
                    - p.layout.block(p.T_all, p.ij_to_ji[ij])[:n, :n].T).max()
             for ij, n in enumerate(p.n_pno) if n),
            default=0.0,
        )
    ...
    def pair_energies(self):
        """Per-pair correlation energies ``e_ij = K_ij . Tt_ij``, as an array.

        psi4's ``e_ij_`` after ``lmp2_iterations``, and what its ``filter_pairs``
        splits strong from weak on. The MP2 driver has no use for it - the total
        is what it reports - so it is computed on request rather than during the
        iteration, where it would be one more node per replay for a number read
        once.

        Indexed by pair ordinal. Padding contributes nothing, so the dot is over
        each pair's whole padded block.
        """
        p = self.p
        e_pair = np.zeros(p.n_lmo_pairs)
        for ij, n in enumerate(p.n_pno):
            if not n:
                continue
            K = p.layout.block(p.K_all, ij)
            Tt = p.layout.block(p.Tt_all, ij)
            e_pair[ij] = float(np.vdot(K, Tt).real)
        return e_pair
