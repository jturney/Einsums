...
function(einsums_add_module libname modulename)
    ...
    set(EINSUMS_ENABLED_MODULES ${EINSUMS_ENABLED_MODULES} ${modulename}
    ...
    list(REMOVE_ITEM zombie_generated_headers ${generated_headers} ${compat_headers}
         ${CMAKE_CURRENT_BINARY_DIR}/include/einsums/config/modules_enabled.hpp
    )
    ...
    target_link_libraries(
        einsums_${modulename} ${module_public_keyword} ${${modulename}_MODULE_DEPENDENCIES}
    )
    target_link_libraries(
        einsums_${modulename} ${module_public_keyword} ${${modulename}_DEPENDENCIES}
    )

    target_link_libraries(
        einsums_${modulename} ${module_public_keyword} einsums_public_flags einsums_base_libraries
    )

    target_include_directories(
        einsums_${modulename} ${module_public_keyword} $<BUILD_INTERFACE:${HEADER_ROOT}>
        $<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}/include>
        $<BUILD_INTERFACE:${PROJECT_BINARY_DIR}> $<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>
    )
    ...
    einsums_add_source_group(
        NAME einsums_${modulename} ROOT ${HEADER_ROOT}/einsums CLASS "Header Files"
        TARGETS ${headers}
    )
    einsums_add_source_group(
        NAME einsums_${modulename} ROOT ${SOURCE_ROOT} CLASS "Source Files" TARGETS ${sources}
    )

    if(${modulename}_GLOBAL_HEADER_GEN OR ${modulename}_CONFIG_FILES)
        einsums_add_source_group(
            NAME einsums_${modulename} ROOT ${CMAKE_CURRENT_BINARY_DIR}/include/einsums
            CLASS "Generated Files" TARGETS ${generated_headers}
        )
    endif()
    einsums_add_source_group(
        NAME einsums_${modulename} ROOT ${CMAKE_CURRENT_BINARY_DIR}/include/einsums
        CLASS "Generated Files" TARGETS ${config_header}
    )
    ...
    if(NOT module_is_interface_library)
        set_target_properties(
            einsums_${modulename} PROPERTIES FOLDER "Core/Modules/${libname_cap}"
    ...
    install(DIRECTORY include/ DESTINATION ${CMAKE_INSTALL_INCLUDEDIR} COMPONENT ${modulename})

    # Installing the generated header files from the build dir
    install(DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/include/einsums
            DESTINATION ${CMAKE_INSTALL_INCLUDEDIR} COMPONENT ${modulename}
    )
    ...
    if(MSVC)
        foreach(cfg DEBUG;RELWITHDEBINFO)
            ...
            einsums_get_target_property(
                _pdb_dir einsums_${modulename} COMPILE_PDB_OUTPUT_DIRECTORY_${cfg}
            )
            install(FILES ${_pdb_dir}/${_pdb_file}.pdb DESTINATION ${CMAKE_INSTALL_LIBDIR}
                    CONFIGURATIONS ${cfg} OPTIONAL
            )
        endforeach()
    endif()
    ...
    einsums_create_configuration_summary(
