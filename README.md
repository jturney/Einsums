...
The PackedGemm engines instead fold the permutation into the packing step, in the spirit of BLIS-based tensor libraries such as TBLIS: they pack cache-sized blocks directly out of the scrambled operands and contract them on the best kernel the machine offers, on Apple M4, SME FMOPA outer-product tiles for real types and the same real tiles via the 1m method for complex types, selected at runtime through the SIMD dispatch ladder.
